...
- **MySQL / PlanetScale-MySQL target:** the same probe-bounded auto:N as Postgres. MySQL has implemented the connection-budget probe (`ProbeTargetConnectionBudget`, ADR-0116) since v0.100.0, so the lane count is the auto request of 4 clamped to the target's measured connection budget and to `--max-target-connections`; a failed probe degrades to serial apply rather than refusing. (An earlier revision of this line said MySQL had no probe and a fixed ceiling — false since v0.100.0; the flag is inert only on SQLite/D1 and the trigger-CDC targets, which the `INERT-FLAG` WARN names.) PlanetScale per-branch connection limits are generous relative to 4 lanes + 4 dedicated backends across every tier.
