...
- **`SLUICE-E-TARGET-CONTROL-TABLE-PLACEMENT`** — sluice's own control tables must live in an unsharded shard group on a sharded database, sluice places them in the authoritative group itself, and this fires when it cannot (the role may not write the topology, or the topology has no usable authoritative group). Refused at control-table creation, before any data moves, naming the entry to add by hand.

- **`SLUICE-E-TARGET-SHARD-KEY-MISSING`** — the platform refused an `INSERT` that does not carry the table's shard-key column (SQLSTATE `NK306`). Terminal on the statement's shape: for a control table it means the placement above did not happen; for a user table it means the source rows carry no column by the target's shard-key name.
...
- **sluice places its own control tables on a sharded database.** Once a database is sharded, the default shard group covers `public`, so *every* table there inherits the shard-key requirement — including sluice's own bookkeeping tables, which have nothing to do with your sharding scheme and would otherwise be refused with `shard-key column … required but missing from INSERT` (SQLSTATE `NK306`). At control-table creation sluice assigns them to the topology's `authoritative_shard_group` — the group PlanetScale's guidance reserves for unsharded metadata — by writing the data topology itself, only when a shard index would route them, and only once (an unchanged topology write still mints a revision, so sluice compares before it writes). The write is attributable: `__neki.list_data_topology_changes()` shows it with the comment `sluice: place control tables in the authoritative shard group`. The connecting role needs to be able to call `__neki.set_data_topology`; if it cannot, or the topology names no authoritative group, sluice refuses with `SLUICE-E-TARGET-CONTROL-TABLE-PLACEMENT` before any data moves and names the exact entry to add by hand. A topology that cannot be *read* is a warning, not a refusal, so an unsharded database whose role lacks that privilege keeps working. See [`operator/planetscale-postgres-to-neki.md`](operator/planetscale-postgres-to-neki.md) for the manual recipe.
...
- **`max_slot_wal_keep_size` scales with the COMPUTE tier, not PITR** — 512 MB on Micro, 2048 MB on Small+ (**live-validated 2026-07-17, PG 17.6**). This is the real, measured runway, not a paper setting: a detached logical slot on **Small survived 1377 MB of retained WAL still `wal_status='reserved'`** (≈2.7× the Micro bound), while the **identical detached slot on Micro was invalidated** once retained WAL crossed ~512 MB — it went `wal_status='unreserved'` at 551 MB (negative `safe_wal_size`) and then `wal_status='lost'` / `invalidation_reason='wal_removed'` after the next checkpoint, forcing a re-snapshot. So a detached or badly lagging stream has exactly the compute tier's WAL runway before the slot is lost. sluice's ADR-0059 slot-health probe pages at 70/85% of whatever the live bound is; keep detach windows short. The lever for a WIDER retention window is a compute-tier bump (~$15/mo Small → 2 GB), **not** the PITR add-on (~$100/mo) — PITR only reaches 2 GB transitively because it *requires* Small compute. (Aside: bulk-generating multi-GB of WAL against a 1 GB-RAM Micro in a tight loop can push it into a brief crash-recovery restart, and the Management API still reports `ACTIVE_HEALTHY` during it — probe backend liveness with a real query, not the status field.)
...
**CDC and backups: point `sync start` and `backup` at the PRIMARY, never the replica.** (Since v0.139.0 `backup full` refuses a standby up front too: it cannot record the CDC end position every manifest needs, and it used to discover that only after copying every row. v0.140.0 made that refusal reach a standby at any `wal_level` — before it, the `wal_level` check won on the default setting and the copy still happened.) CDC manages the sluice publication on the source and `CREATE`/`ALTER PUBLICATION` cannot run on a standby; sluice refuses up front with the coded `SLUICE-E-CDC-STANDBY-SOURCE` steer (pre-fix this surfaced as a raw SQLSTATE 25006 at publication ensure). PG 16+ standbys can technically host logical slots, but creation blocks on the primary's next running-xacts record and Supabase denies the `pg_log_standby_snapshot()` nudge — the primary is the supported CDC source.
