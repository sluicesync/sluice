...
- **A multi-schema `sync start` refuses a table with no usable replica identity, before it publishes anything (since v0.139.0).** Postgres will not `UPDATE` or `DELETE` a published table that lacks one, so publishing first and checking later would break your own application's writes. The refusal carries `SLUICE-E-SOURCE-REPLICA-IDENTITY` and names the offending tables; the remedy is a primary key or a `REPLICA IDENTITY FULL`. **`--exclude-table` is NOT a remedy here** — it is on a single-schema sync, whose publication is scoped, but this path's publication is `FOR ALL TABLES` because the slot is database-wide, so an excluded table is still published and its writes still break. Since v0.141.0 that case is warned about rather than silent: `UNSELECTED-NAMESPACE-EXPOSURE` names every at-risk table the refusal did not grade, which includes the ones you excluded. **The same marker also fires when the check could not RUN** — most often because the sync role cannot read `pg_publication_tables` — and that warning says "could not check" rather than naming tables. Read it as exposure UNKNOWN, not exposure absent: until v0.148.3 that case logged at debug level, so an operator at the default level saw nothing and an absent warning was indistinguishable from a clean audit.
- **The fan-out runs migrate's target-side preflights per selected database (since v0.154.0).** `sync start --all-schemas` / `--all-databases` opens its own writer per namespace rather than going through the single-namespace cold start's, and so reached none of them: it created every schema, opened the slot, and died **mid-copy** on a raw `0A000` where `migrate` and a single-schema `sync start` against the same target both refused up front. Per database, in migrate's order, it now clears stale backends, both sharded-target refusals (placement, and the shard-key/upsert-key mismatch), the direct-DDL check, row-level security on the target side, and the target-ownership advisory, before the populated-target refusal it already ran. A source-side `TINYINT(1)`-range probe runs per database alongside the schema read, for the same parity reason. The operator-visible change: into an RLS-forced target whose role lacks `BYPASSRLS`, the run now refuses before it copies anything, naming the offending tables, the connecting role and `ALTER ROLE … BYPASSRLS`. That refusal is deliberately **uncoded** — it is a structured preflight error, not a `SLUICE-E-*` one, so there is no error-codes row to look it up by; match it on the message.
