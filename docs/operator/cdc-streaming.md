...
Since v0.148.2 every MySQL-family lane — binlog GTID, binlog file/pos (`@@server_uuid`), MariaDB (lineage anchor and domain), and VStream (a foreign keyspace) — treats a **different lineage** as terminal. The refusal is `SLUICE-E-CDC-LINEAGE-MISMATCH` from the engine and, from the pipeline, a line marked `FOREIGN-LINEAGE-REFUSED` that says nothing on the target was touched. It fires on both paths: a warm resume refuses before streaming, and the reactive path — a position that went invalid mid-stream — asks the source the same question before it drops anything. What is *not* foreign, and still recovers automatically: a purged position; a VStream errant GTID, whose remedy is on the source and which keeps its existing route; and, on MySQL, an **empty** `@@gtid_executed` or a source that is merely **behind** the position — *provided the server's `@@server_uuid` is one the position names*, which is what makes it the same instance after a `RESET MASTER` or a rollback. An empty or behind set on a server the position has never seen is the rebuilt-node shape — a restore followed by `RESET MASTER`, or one seeded through `gtid_purged` from an older backup — and is refused (the 2026-09-15 audit measured the automatic re-copy reducing a target to the rebuild's stale rows on that shape). The refusal names the uuid; if the server is a lagging replica behind a failover endpoint, waiting for it to catch up makes the position contained and the resume proceeds with the target intact. The uuid is a check of *identity*, not of safety, and one residual is stated plainly: an **in-place restore on the same server** — `RESET MASTER`, then `mysql < dump.sql` on the same box with `auto.cnf` untouched — presents the same empty or behind set under the position's own uuid and STILL takes the automatic re-copy, reducing a target that was ahead of the restored source to the dump's rows. The witness cannot see that case; whether a behind or empty set under the server's own uuid should be terminal too is filed as an open policy question. If you restore a source in place, stop the stream before the restore and decide afterwards, deliberately, whether the restored source or the target is the truth — the automatic path will not ask. **MariaDB has no instance identity**, so an empty `@@gtid_binlog_state` there still takes the automatic re-copy whether the server is the same one reset or a rebuilt one — a known gap, stated rather than closed, because refusing every empty state would also refuse every legitimate same-server reset; if you rebuild a MariaDB source, stop the stream first and restart it with `--restart-from-scratch` deliberately.
...
What refuses, with no marker, and takes the position-invalid (auto-resnapshot) route — the verdicts that mean *the same source has moved past the position*: a MySQL GTID resume set not contained in the source's `@@global.gtid_executed` because the source is merely *behind* it (every source UUID present, sequence numbers lower — a primary rolled back past the position, or a hole in its set) or because the executed set is *empty* (a `RESET MASTER`), **in both cases only when the server's `@@server_uuid` is one the resume set names** (since v0.153.2; the message says which shape it saw — and this is an identity check, not a safety one: an in-place same-server restore, `RESET MASTER` then the dump loaded on the same box, presents exactly this shape and still re-copies, see the `FOREIGN-LINEAGE-REFUSED` section); a MariaDB resume set naming a replication domain absent from an *empty* `@@gtid_binlog_state`; a VStream errant GTID (the position shares UUIDs with the shard and names one it never executed — the remedy is on the source, and it stays on this route). What refuses TERMINALLY as a *different lineage* — `SLUICE-E-CDC-LINEAGE-MISMATCH` from the engine, `FOREIGN-LINEAGE-REFUSED` from the pipeline, since v0.148.2: a MySQL GTID resume set sharing no source UUID with the executed set, and, since v0.153.2, an empty or behind executed set on a server whose `@@server_uuid` the position has never named (the rebuilt-node shape — a restore followed by `RESET MASTER`, or a `gtid_purged` seeded from an older backup; if it is a lagging replica behind a failover endpoint instead, wait for it to catch up and retry); a MariaDB anchor that answers a different GTID state, or no event at the anchor with no retained file above it that could have purged it on the same lineage, and a resume domain absent from a *non-empty* `@@gtid_binlog_state`; a VStream shard sharing no source UUID with the position (a shard that is merely behind is replica lag, logged at INFO, and left to vtgate's tablet picker). Lineage is checked before retention on every arm, so a foreign source that has also purged binlogs is diagnosed as foreign rather than with a recovery hint that does not apply. Postgres is out of scope here — it has its own source-identity pin ([ADR-0051](../adr/adr-0051-pg-cdc-source-identity-pinning.md)).
