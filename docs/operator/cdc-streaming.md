...
Every backup chunk — the rows of `backup full`, and the changes of `backup incremental` and `backup stream`, including before-images and the `ADD COLUMN` fill — is JSON. JSON cannot hold a byte that is not part of valid UTF-8, and Go's encoder writes each such byte as U+FFFD (`�`) without an error. Through the release before this refusal existed that is what a backup did: the chunk sealed, `backup verify` rehashed it and agreed, and a restore landed `�` where the source held a value, at exit 0. It was reached in practice by a MySQL, MariaDB or PlanetScale/Vitess column in a legacy character set (latin1, cp1250, sjis, …), whose change-stream values reached the backup as raw bytes.

A backup now refuses such a value at capture: the run fails with `BACKUP-VALUE-NOT-UTF8`, naming the table, the column, which image of a change held it, and where inside a list or JSON document it sits. The refusal is terminal, so `backup stream` stops rather than retrying. Nothing is committed for the refused window, and the source value is intact. Binary columns are unaffected — they are carried as bytes, never as text. Text must reach sluice already decoded from its column's character set; a refusal therefore means the source's reader did not do that for this column, which is a sluice defect worth reporting. To proceed meanwhile, exclude the column's table, or store the value as a binary type on the source. **If you restored a chain captured before this refusal from such a source**, compare the affected text columns against the source: any `�` in them is a lost value, and so is any other character a legacy-charset value turned into.
