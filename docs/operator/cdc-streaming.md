...
- **The same copy-shaping flags.** `--where`, the table scope, `--type-override`, `--redact`, `--inject-shard-column` and `--target-schema` are fingerprinted when the copy starts, and a re-run whose flags differ is refused (`COLD-START-SHAPE-CHANGED`, below).
