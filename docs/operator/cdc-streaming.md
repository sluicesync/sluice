...
## Chain restore: a column added with a non-constant DEFAULT (`ADD-COLUMN-FILL-NOT-REPRODUCIBLE`)

When a source runs `ALTER TABLE t ADD COLUMN c … DEFAULT d` between two links of a chain, it fills every row it already holds with `d` — and that fill writes no row event, so no link carries a value of `c` for those rows. Chain restore and `sync from-backup` replay the recorded `ADD COLUMN`, and the target fills the same rows by evaluating `d` itself. For a constant `d` (a string, number, boolean, date, bytes, JSON, UUID literal) that is exactly the source's value, on Postgres and MySQL sources alike, because the recorded schema change is a catalog read and carries the DEFAULT. One exception, the same one `migrate` has: toward a MySQL target, a DEFAULT on a `TEXT`, `BLOB` or `JSON` column is dropped by the target's DDL emitter (with its own WARN naming the column), so those rows land NULL.

For a DEFAULT that depends on when or where it is evaluated — `now()`, `CURRENT_TIMESTAMP`, `clock_timestamp()`, `gen_random_uuid()`, `CURRENT_USER`, a sequence, or any function sluice cannot prove constant — the target evaluates it at restore time, so the restored rows hold the restore's clock (or a fresh UUID) where the source holds its `ALTER`'s. The replay logs a WARN marked `ADD-COLUMN-FILL-NOT-REPRODUCIBLE` naming the table, the column and the DEFAULT, and carries on: the true values are in no link, and refusing would throw away every later link of the chain for one column's pre-existing rows. To repair, copy that column's values for the rows that pre-date the `ALTER` from the source, or take a fresh full backup after the `ALTER` and restore from it.

**Not flagged, stated rather than implied:** a DEFAULT changed or dropped later in the **same** window as the `ADD COLUMN` — `ADD COLUMN c … DEFAULT 'v'` then `ALTER COLUMN c DROP DEFAULT`, which is what Django emits for every `AddField` with a default. The chain records the window-end DEFAULT (none), so the restored rows that pre-date the `ALTER` land NULL where the source holds `'v'`, with no warning. If your migrations follow that pattern, take a full backup after them, or compare the column against the source after a restore.
