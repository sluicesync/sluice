...
**What the resume requires, in full, because it refuses rather than guessing.** The source must be PostgreSQL (a slot is what makes the anchor provable; MySQL, MariaDB, Vitess/PlanetScale, SQLite, D1 and the trigger engines have no equivalent and still refuse). The stopped run must have **finished copying every in-scope table** — sluice checks the per-table progress it recorded, not merely which phase it reached, so a stop *during* the copy is not resumable and re-runs exactly as before. And the slot must still be **exactly** where that snapshot was taken: unattached, with its `confirmed_flush_lsn` equal to the recorded anchor. If any of those fails you get the old behaviour, which is a loud refusal, never a silent partial copy.
