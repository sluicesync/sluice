...
**A replaced source is caught too, from this release onward.** The snapshot's recorded position now carries the source's `system_identifier` and timeline from the moment the slot is created, so a resume whose `--source` has been repointed at a physical clone, a restored backup or a promoted standby is refused by the existing source-identity check rather than streaming from it. Positions recorded by earlier releases carry no such pin — and they carry no copy-shape fingerprint either, so the resume declines them for that reason first.
