...
| `sluice_sync_lag_seconds` | gauge | `stream_id` | Seconds the target trails the source's latest applied commit, frozen at each apply (~0 while caught up and flowing). Omitted until the first source-timestamped change is applied, and omitted again after ~30 s with no applied change — sustained idle is UNKNOWN, not zero, because this seam cannot tell a quiet source from a wedged pipeline. | The primary lag alert: > 60 s sustained. A fired `--notify-sync-lag-seconds` alert stays latched through idle (it clears only when changes flow again at low lag). Pair with `sluice_seconds_since_last_apply` (which ages on a quiet stream) for "not applying at all"; `sluice_target_replica_lag_seconds` is target-internal. |
...
| `sluice_target_router_cpu_util` | gauge | `stream_id` | CPU utilisation of the **busiest routing-layer pod**, fraction in [0,1] — the layer that terminates client connections and routes queries to shards. PlanetScale Neki's routers; the same role Vitess gives VTGate. Absent on an unsharded target. | **Watch alongside `sluice_target_cpu_util`, not instead of it.** This layer is in sluice's own connection path — a Neki connection is an ordinary port-5432 Postgres connection that lands on a router first, with no way around it — so when it saturates, every statement sluice issues queues behind one, and no number in the primary's readings explains why. Measured on a live Neki branch pegged at 100% while the database pods were fine. Busiest rather than average: connections spread across router pods, so one pegged pod is a real stall for its share of traffic. **The remedy is a larger router tier, which is a separate control from the database's size.** |
| `sluice_target_router_mem_util` | gauge | `stream_id` | Memory utilisation of the busiest routing-layer pod, fraction in [0,1]. | As above. |
| `sluice_target_pgbouncer_cpu_util` | gauge | `stream_id` | CPU utilisation of the **busiest PgBouncer connection pooler**, fraction in [0,1], from PgBouncer's own per-peer metric. PlanetScale Postgres runs one as a sidecar per database pod. | **This is not sluice's path.** sluice connects to PlanetScale Postgres directly, and logical replication cannot traverse a transaction pooler at all, so nothing sluice does passes through here. Read it as a window on *your application's* traffic to the same database — which is worth watching during a migration, because the app and the migration are contending for one instance. A quiet pooler says nothing about sluice's throughput, and a busy one is not sluice queueing. Note PgBouncer is single-threaded per peer process, so a peer at 100% is saturated even while its pod looks idle. |
| `sluice_target_pgbouncer_mem_util` | gauge | `stream_id` | Memory utilisation of the busiest PgBouncer pooler, fraction in [0,1]. | As above. |
| `sluice_target_pgbouncer_client_wait_seconds` | gauge | `stream_id` | Seconds the **longest-waiting client** has been queued at the pooler without yet being handed a backend connection. | The least ambiguous of the pooler signals — CPU at 90% may be fine, but a client that has waited two seconds is being made to wait. It measures a **different ceiling** from `sluice_target_max_connections`: on a measured PlanetScale Postgres branch PgBouncer admitted 400 clients in front of 25 Postgres backends, so application clients can queue here long before the backend count looks stressed. |
