...
| `sluice_target_front_door_cpu_util` | gauge | `stream_id` | CPU utilisation of the **busiest front-door instance**, fraction in [0,1] — the hop client connections land on before a database backend. On PlanetScale Neki that is a router pod; on PlanetScale Postgres it is the PgBouncer sidecar. Absent where the platform has no separate front door. | **Watch this alongside `sluice_target_cpu_util`, not instead of it.** The front door can saturate while every database pod is comfortable, and when it does, throughput collapses for a reason nothing in the primary's numbers explains — measured on a live Neki branch with the routing layer pegged at 100%. Busiest rather than average: connections spread across instances, so one pegged instance is a real stall for its share of traffic. The remedy differs by platform — a bigger router tier on Neki, a bigger instance on Postgres — but in neither case is it a bigger *database* for the same size. |
| `sluice_target_front_door_mem_util` | gauge | `stream_id` | Memory utilisation of the busiest front-door instance, fraction in [0,1]. | As above. |
| `sluice_target_front_door_wait_seconds` | gauge | `stream_id` | Seconds the **longest-waiting client** has been queued at the front door without yet being handed a backend connection. Emitted where the platform publishes a queue wait (PlanetScale Postgres, via PgBouncer's per-pool maxwait); absent on Neki. | **The least ambiguous of the three — alert on this first.** CPU at 90% may be fine; a client that has waited two seconds for a connection is being made to wait, full stop. It is also the signal sluice is most likely to cause itself: a bulk copy opens many concurrent connections and a pooled front door is where that shows up first. Note this is a *different ceiling* from `sluice_target_max_connections`, which reports the Postgres backend limit — on a measured PlanetScale Postgres branch PgBouncer admitted 400 clients in front of 25 backends, so connections can queue here long before the backend count looks stressed. |
