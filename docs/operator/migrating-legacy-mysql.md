...
- **MariaDB under its default `binlog_row_metadata=NO_LOG`, and PlanetScale / Vitess,** record no written charset — vttablet reports a replayed row's *current* collation (measured on vttestserver). Rows are decoded by the column's current charset, and sluice logs `CHARSET-HISTORY-UNRECORDED` once per table with non-UTF-8 text columns. When the replay reaches the `ALTER` itself, sluice compares the charset it names with the shape it has been decoding the table by: a live stream still holds the *old* charset there, a replaying one already holds the *new* one — and that refuses with `SLUICE-E-CDC-SCHEMA-REPLAY-MISMATCH`. Re-snapshot the table (`sync … --restart-from-scratch`). **The rows of that table the stream emitted before the refusal were decoded by the new charset and may already be on the target**; the re-snapshot re-copies them, which is why it is the remedy and not optional.
  ...
  What that guard cannot see, on those sources — re-snapshot yourself after such a change if the stream was behind it:
  ...
  - a statement sluice's SQL parser cannot parse, or one naming a collation it cannot name;
  ...
  One false positive, loud: an `ALTER` that sets a column to the charset it *already* has looks exactly like a replayed one, and refuses.
