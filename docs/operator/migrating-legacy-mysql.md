...
- On PlanetScale / Vitess, a non-UTF-8 `ENUM`/`SET` value that could be read two ways: vttablet sends such a cell as UTF-8 label text during CDC but as its stored bytes during the COPY phase, with nothing to tell the two apart, so sluice takes whichever reading is one of the column's labels — and refuses when both are, which happens only when two labels are each other's mis-encoding (a latin1 `ENUM('é','Ã©')`). Rename one of the labels, or exclude the table.

To proceed past a refusal, convert the column to `utf8mb4` on the source (`ALTER TABLE … MODIFY … CHARACTER SET utf8mb4`) **and re-snapshot the table** (`sync … --restart-from-scratch`), or exclude the table. Do not convert and resume: the stream would replay binlog history written in the old charset.

**A charset change on the source while the stream is behind it.** A binlog value is decoded by the charset it was *written* in, which each row event's TABLE_MAP states — MySQL 8 writes it under its default `binlog_row_metadata=MINIMAL`, MariaDB under `MINIMAL` or `FULL`. So when a stream replays history recorded before an `ALTER … CHARACTER SET` (a warm resume after downtime, a stream that was lagging, a cold start whose copy spanned the `ALTER`), it refuses with `SLUICE-E-CDC-SCHEMA-REPLAY-MISMATCH` naming the column and both charsets; re-snapshot the table. Two gaps, stated rather than implied:

- **MariaDB under its default `binlog_row_metadata=NO_LOG`** writes no charsets, so such a replay is decoded by the column's *current* charset — a `utf8mb4` `é` replayed after a change to `latin1` arrives as `Ã©`. Set `binlog_row_metadata=MINIMAL` (or `FULL`) on the source to close it; until then, re-snapshot any table whose charset you change while a stream is behind it.
- **PlanetScale / Vitess** reports the column's *current* collation for a replayed row (measured on vttestserver), with no independent record of the charset it was written in, so the same replay is decoded by the new charset. Re-snapshot a table after changing its charset whenever the stream was behind the change.

**Backup chains captured before the fix** (`LEGACY-CHARSET-INCREMENT`). Chain restore and `sync from-backup` WARN, naming the table and columns, for every incremental that sluice v0.156.2 or earlier captured from a MySQL-family source with a non-UTF-8 text column: those incrementals stored every non-ASCII value they changed in such a column as `�`, and the chain cannot give the value back. Take a fresh full backup with v0.156.3 or later, and repair rows restored from the old chain against the source. Rows the full backup copied are exact.
