...
> **Correction (v0.148.1).** v0.148.0 shipped this section claiming the cold-start block generally. It was not: on a target that had never run `migrate` the progress tables were never created, so the feature was a no-op in its most common case, and it reached only the PostgreSQL fast-path copy. v0.148.1 fixes the table creation and the phantom "in progress" row that outlived a finished copy. It left one limitation, which **v0.149.0 closes**: the block appeared for PostgreSQL sources only, because the recording lived inside the fast parallel copy lane that only a shareable-snapshot source is eligible for. A MySQL, MariaDB, PlanetScale or Vitess source took the serial copy path and recorded nothing — including the PlanetScale MySQL move this page is written from. Every serial cold start now records the same phase and per-table rows through the same primitives. That was audit A0909-P2b.

On **every source**, `sync status` reports the cold start from the progress rows the copy writes as it goes:
