...
`--no-drain` has two live paths, dispatched by engine pair: a **Postgres source** (any target — the new table joins the source publication, ADR-0030) and a **MySQL-family binlog source writing to a MySQL-family target** (the running streamer polls `sluice_cdc_state.live_added_tables` on the target and widens its table filter, ADR-0034, shipped v0.27.0). Any other pair — a MySQL source with a Postgres target, or a PlanetScale/Vitess (VStream) source, whose live path is a demand-gated roadmap item — refuses loudly and names the **drained workflow**: `sluice sync stop --wait`, then `schema add-table`, then `sluice sync start` again (a re-run with the same `--stream-id` warm-resumes; it does not re-snapshot). The Postgres live path is strict zero-loss since v0.32.0 (ADR-0036); the MySQL filter-flip path keeps ADR-0034's best-effort caveat for writes landing on the new table during the streamer's poll lag, so on MySQL the drained flow is the strict zero-loss one.
