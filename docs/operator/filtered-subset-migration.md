...
**2. A faithfully-evaluable predicate.** A binlog / logical-replication CDC stream has no server-side row filter, so sluice evaluates the predicate itself over each decoded change — and it accepts only a **restricted grammar** where its client-side evaluation is guaranteed to agree with the source's own `WHERE`. (On the PlanetScale/Vitess VStream flavor the cold-start *snapshot* copy additionally pushes a server-side `WHERE` down, ADR-0174 Piece 2, so the copy phase transfers only in-scope rows; the steady-state CDC leg is still evaluated client-side.)
