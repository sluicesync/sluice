...
- **VStream (PlanetScale/Vitess) caveat — a PAD-SPACE-collation column is refused (v0.99.282).** The cold-start + warm-resume server-side push-down (below) evaluates the pushed `WHERE` **NO-PAD** regardless of the column's real `PAD_ATTRIBUTE`, so on a VStream source a `--where` on a **PAD-SPACE legacy collation** (e.g. `utf8mb4_general_ci`) can't be pushed faithfully and is refused at sync-start. NO-PAD `utf8mb4_0900_*` collations, non-string predicates, and the non-VStream flavors (vanilla MySQL binlog, Postgres — which push through the source's own PAD-faithful `=`) are unaffected. Use a NO-PAD collation, filter on a different column, or `migrate --where`. (A client-side fallback that lets such a filter work on VStream is planned.)
