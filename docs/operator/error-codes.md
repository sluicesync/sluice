...
| `SLUICE-E-VSTREAM-FLOAT-LOSSY` | refusal | `backup full` on a PlanetScale/Vitess (VStream) source with `--strict-float`, when the source schema has single-precision `FLOAT` columns: vttablet's rowstreamer renders FLOAT at mysqld's 6-significant-digit display precision, so the archived snapshot would hold display-rounded floats and there is no CDC leg on the backup path to heal it. sluice refuses rather than archive silently-rounded data. | Drop `--strict-float` to archive anyway (rounded, with a WARN), snapshot via a non-VStream path, or wait for an upstream Vitess rowstreamer fix. A target-side `--type-override` to DOUBLE does NOT help — the source value is already rounded on the wire. |
