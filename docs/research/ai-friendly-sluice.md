...
2. **JSON result envelopes on the primary verbs (M).** `--format json` on `migrate`/`sync run`/`backup`/`restore` emitting one terminal object: `status` (`completed`/`refused`/`failed`; `aborted` was added in v0.99.205 for an operator-declined destructive confirmation), per-table rows/bytes/duration, warnings emitted, resume position/token, and `next_steps` (e.g. after `migrate`: "run `sluice verify --source … --target …`"). Follow the seven existing `Format` fields' pattern; render at the same code point that prints today's human summary.
