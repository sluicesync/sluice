...
5. **Format versioning across sluice releases.** Manifest carries `sluice_backup_version: 1`; future versions add fields. Old sluice refuses newer manifests with a clear error; new sluice reads older ones — **as far as the format version goes, which is not the whole story.** Corrected 2026-08-07 (invariant sweep): this line said "…always reads older" of new sluice, and the unqualified form is false. A newer binary REFUSES an older chain across a schema-fingerprint epoch, because `verifySchemaHashes` recomputes each link's `SchemaHash` with the current build's `ComputeSchemaHash`, which keys off no recorded algorithm version; three such boundaries have shipped and two of them sit inside format version 8, where the version gate cannot see them. See `BackupFormatVersion`'s doc in `internal/ir/backup/manifest.go` ("What newer-reads-older does not cover") and `TestBackup_CrossVersionChainCompat` cells 5 and 6, which pin both directions against real binaries built from real tags. <!-- forward-compat-caveats: schema-fingerprint-epoch -->
