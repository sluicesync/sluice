...
Whether the gated suite runs **weekly on a schedule** or **on dispatch only**. Dispatch-only is the zero-cost default and what this design ships with. Weekly costs whatever a few minutes of a live PS-10 Neki pair costs, and buys early warning when the platform moves — which, for a product in preview, is the whole point. The `schedule:` block is written and commented out either way.
