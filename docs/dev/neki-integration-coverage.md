...
5. **`restore` INTO a sharded target, and `backup full` FROM one.** Added 2026-09-14, after ADR-0187's control-table placement unblocked it. Restore runs the same phases migrate does — schema-apply, bulk copy, indexes, constraints — so it meets every hazard the others meet, and the only Neki reference in the whole backup package is a comment in `restore_table_pool.go` recording a defect code review caught rather than a run (a literal `0` dropping `CopyConcurrencyCeiling`, letting a restore open table × chunk concurrent COPYs against a router that admits four). The backup direction is the lower-priority half and is included because nothing has ever measured a sharded READ.

6. **Shard-targeted sessions (`SET __neki.shard`).** Added 2026-09-15, after PlanetScale's query-planning page documented a per-session switch that forwards DML to one shard, bypassing shard-key routing. A PROBE rather than a premise check — sluice has no behaviour here yet and every outcome is useful — measuring the two things that would change what sluice can offer against a sharded Neki, and the one that would make it worse. **Per-shard consistent reads:** whether a `REPEATABLE READ` transaction on a targeted session is a real Postgres snapshot of that shard, whether it can export one, and whether per-shard counts sum to the router's — which together decide whether `migrate` and `backup` FROM a Neki could read shard-by-shard instead of through a scatter-gather that is not consistent even within one table. **Composite positions:** whether `pg_current_wal_lsn()` answers per shard (the router refuses it outright — that is what killed `backup full` FROM a Neki at the finalize phase) and whether a replication connection can be pointed at one shard through a startup parameter, since `SET` cannot precede a replication handshake. Together those are the VGTID shape sluice already carries for Vitess, which would turn "a Neki has no position" into "a Neki has N". **And the hazard, measured then undone:** a targeted `INSERT` whose shard key routes elsewhere, graded on which reads can then see the row — the evidence behind the rule that sluice must never WRITE under targeting, so any per-shard design coming out of this is read-only.

**Item 5's status, 2026-09-14 — BUILT, NOT YET RUN LIVE.** `nekiRestoreIntoShardedTarget` and `nekiBackupFromShardedSource` (`internal/engines/postgres/nekiverify_backup_restore_test.go`) ride item 1's fixture, in that order, between the CDC arm and the MoveTables arm. What is unusual about this item — and worth copying for the next one — is that its non-router half runs on **every PR**: the shared core lives under `//go:build integration || nekiverify` and `TestPostgresSuite_NekiBackupRestorePlumbing` drives it against an ordinary PostgreSQL container with the flavor forced. That per-PR run caught two harness faults before the live arm was ever dispatched (an expected row set left in generation order while the read-back is `ORDER BY tenant_id, id`, and an index build that on Neki is not `CREATE INDEX` at all). The suite's history is six MoveTables dispatches that each failed on a harness question at the cost of a cluster; this is the cheapest available answer to that.

One leg is **NOT COVERED** per-PR and is named rather than implied: a Neki target submits an online-DDL workflow through the `__neki` schema instead of running `CREATE INDEX` (ADR-0184), so a forced-flavor server answers `3F000` and the index-build leg only exists on the live arm. And the arm takes a **single-full** restore, which never opens a change applier — so the control-table creation door (`ChainRestore.Run` step 2.9) is reached only by a multi-segment chain restore, which this arm does not build. The arm measures which control tables the restore created and prints the answer, and grades placement as a statement about the target's state.

**Item 6's status, 2026-09-15 — BUILT, NOT YET RUN LIVE.** `nekiShardTargetedSessionProbe` (`internal/engines/postgres/nekiverify_shard_targeting_probe_test.go`) rides item 1's fixture, registered after the backup arm and before the MoveTables arm, with measurements (a)–(g) as subtests. Two things about its shape are worth copying. Each targeted session is ONE connection taken from a pool of its own rather than from the suite's shared pool, so a pin cannot be recycled into a later arm even if `RESET __neki.shard` failed — `RESET` is the courtesy, the private pool is the guarantee. And its anti-vacuity foundation is a tenant-placement check that runs before every measurement: if the `SET` silently did nothing, both pinned reads would return the router's scatter-gather result and every tenant would appear to live on both shards, which is the one failure that would make every answer below it a confident statement about the router while claiming to be about a shard. Unlike item 5 there is **no per-PR half**: every measurement here is a question about a router, so nothing in it can run against an ordinary PostgreSQL container with the flavor forced.

**Status, 2026-09-11 — ALL FOUR [of the original] ITEMS ARE BUILT.** Item 1 is BUILT and has run green on a live cluster (`TestNekiverify_ShardedRefusalPremises`). Items 2, 3 and 4 are BUILT but **have not yet run against a live cluster** — they are written and type-checked under the tag and wait on the first scheduled run; grade them on that run rather than assuming they pass. All three ride item 1's fixture rather than provisioning their own database.
