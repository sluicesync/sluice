...
**Backup and restore, measured 2026-09-15 (nekiverify run 34926553073).** `restore` INTO a sharded Neki works end to end — schema, a six-chunk copy, two secondary indexes through the online-DDL workflow, 20,500 rows scattered evenly across both shards, content byte-exact. A single-full restore creates no control tables (only a chain restore reaches the applier door), and it resolves to a single cross-table writer on a Neki (`target connection budget allows only one writer`) — correct on the COPY ceiling, filed as a perf item. `backup full` FROM a sharded Neki copied every row and then **died at the finalize phase**: the end-position capture reads `pg_current_wal_lsn()`, which the router does not implement (NK013). Fixed after the run: on a Neki the capturer answers `ErrPositionUnavailable`, the manifest's EndPosition stays empty and a WARN says no incremental can chain off that full (a Neki cannot be a CDC source in any case). Details in the audit backlog, 2026-09-15.
...
**The obvious fix is a silent-corruption bug.** Leaving the shard key out of the `SET` list makes the statement legal and makes the write wrong: `ON CONFLICT (k)` evaluates its conflict only on the shard the *incoming* row routes to, so a row whose shard key differs from the stored row's finds no conflict and INSERTS. Measured: `SELECT count(*) … WHERE id = 3001` returned **2**, the two rows physically on different shards, at exit 0. A guard predicate does not help — there is no conflict for it to skip. This reaches the same harm as the separately-reported "topology without reshard silently splits a table" finding, with no topology mistake at all — so that finding's diagnosis was narrower than the hazard.
