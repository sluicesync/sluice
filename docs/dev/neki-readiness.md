...
The guard on sluice's side is worth building regardless, and Neki hands us the pieces: `__neki.neki_xxh3_64_*` computes a row's routing key client-side and `__neki.get_data_topology()` returns the key ranges, so a preflight can compare where a sampled row **is** against where it should **route** — or, more cheaply, compare a keyed count against a scattering count for one sampled key. A target in that state should be refused before CDC apply rather than quietly accumulating duplicates. Filed as a gate proposal rather than built.
