...
4. **Backup-chain incompatibility** — ~~a backup created without redactions then restored to a target with redactions configured: the redactions apply at restore-time (since restore goes through the same IR pipeline)~~. **Corrected 2026-09-06 (audit W1-3, sibling of the line 57 correction — the same false claim appeared twice in this document and only one instance was fixed on the first pass).** `restore` has no `--redact` flag and never had one; redaction happens at chunk-*write* time and only `backup full` performs it, so nothing is applied on the way out. A backup taken without `--redact` holds plaintext and restores plaintext, whatever the target is configured with. Operators who want PII-clean backups must re-create them with `--redact` rules. Phase 4+ explores "lazy" redaction stored alongside the chain.
