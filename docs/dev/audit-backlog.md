...
### HIGH — found by the B-9/B-4/CDC-4 sibling sweep (2026-08-05), not yet fixed

| ID | Finding | Location |
|---|---|---|
| B-4b | **The B-4 shape, unfixed, on the BACKUP snapshot.** `openBackupSnapshotSerial` opens the consistent-snapshot transaction and *then* captures the chain anchor position, holding no lock at all — and it is the fallback every coordinated-FTWRL failure lands on, i.e. every RDS backup. A commit in that window is above the read view and below the recorded position, so it is in neither the full nor an incremental taken from it. The B-4 fix next door does NOT cover it; a comment at the site says so. Not flipped in the same change because the anchor is load-bearing for incremental lineage and the ADR-0088 replay-window invariants, which that change did not validate. The coordinated (FTWRL) path is fine — it captures under the lock. | `mysql/backup_snapshot.go` (`openBackupSnapshotSerial`) |
