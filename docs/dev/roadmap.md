...
2. **Lighter capture payload (changed-columns-only) — next piece.** The capture log stores the full before/after row image as JSONB (self-contained, replay-safe — the poller never re-reads the source). That costs ~10.8x source-write amplification vs Bucardo's ~2x (Bucardo stores only the changed PK + re-reads the live row at sync). The biggest remaining win for write-heavy sources is a **changed-columns-only capture payload for the `postgres-trigger` UPDATE path** (cut the ~10.8x source-write overhead), trading away some replay self-containment. Keep the full-image mode the default (the gap-free contract).
...
- The changed-columns-only mode changes the replay contract — needs an ADR (interacts with the ADR-0066 capture-log design + the gapless-handoff anchor). Default must stay the self-contained full-image mode.
...
**Sizing.** (1) is done (~small diff: one const per engine + the latency-clock move + pins). (2) lighter payload is an ADR + capture-schema option (~400-700 LOC), demand-gated on a write-heavy-source operator. (3) is a small config surface, partly free.
