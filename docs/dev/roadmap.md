...
- **Phase 4.5 (backup-as-broker / `sync from-backup`) still deferred** — the stream is the producer side; `sync from-backup` is the future "watch a chain and replay incrementals into a target" feature.
...
- **Cross-engine chain restore is refused** loudly (Phase 5+ topic; the same cross-engine machinery `migrate` and `schema diff` use translates types but doesn't yet handle replay-of-changes-with-translation).
