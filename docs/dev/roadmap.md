...
- **(b) vttestserver harness expansion.** Drive the vttestserver image's `ENABLE_ONLINE_DDL`, `NUM_SHARDS`, `FOREIGN_KEY_MODE`, `KEYSPACES` env vars as gated CI scenarios — the lever that makes (c) testable deterministically.
- **(c) online-DDL / `_vt_*` table-lifecycle robustness (CORRECTNESS-GATING).** Exclude Vitess-internal tables from COPY + CDC via `schema.IsInternalOperationTableName()` — the naming is *version-dependent* (legacy `_vt_HOLD/PURGE/EVAC/DROP` in Vitess 19–20 → the unified `_vt_<op>_<uuid>_<timestamp>_` form, op codes `hld/prg/evc/drp/vrp/gho/ghc/del`, in 20+; [vitessio/vitess#14582](https://github.com/vitessio/vitess/issues/14582)), which is exactly why we anchor to the Vitess helper rather than a hand-kept list (and must confirm the vendored Vitess version recognizes what real clusters emit). Pin "sync survives an online `ALTER` + shadow swap with zero loss" against vttestserver under `ENABLE_ONLINE_DDL`. See [ADR-0073](../adr/adr-0073-vitess-internal-and-online-ddl-tables.md).

**Gotchas.** The reproduced bug: during the Bug-125 hunt, sluice's COPY filter grabbed a `_vt_vrp_*` shadow during an in-flight deploy and tripped the ADR-0071 scope-mismatch refusal (zero rows) — so the exclusion must cover BOTH the COPY filter and CDC dispatch. Anchor the internal-table matcher to Vitess's own helper (not a static prefix list) + pin the recognized set so a Vitess naming change fails a test. (c) is the silent-loss-tier piece — "the first real schema change on PlanetScale breaks the sync" if it's wrong. Sequence: **Bug 126 (SHIPPED v0.99.6)** → (b) harness → (c) exclusion + DDL-survival pins → (a) folds in once concrete capability differences surface.
