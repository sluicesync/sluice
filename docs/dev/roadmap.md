...
What remains are the **harder-frontier, demand-gated items** that have always been here as design-first work — multi-source Shape A, MySQL multi-source native parity, ADR-0032 Tier 3 (uuid-ossp + pgcrypto function-defaults), the remaining 5 deferred translator rules, View Phase 3, and the analytics-export / Arrow research items pending operator demand. The **Vitess/vttestserver hardening track** below (operator-flagged; the make-or-break PlanetScale path) is now essentially complete — exclusion shipped, the full-cluster harness + cutover-survival + chaos suites are built and green — leaving only the demand-gated `vitess` self-hosted flavor.

### 1. Vitess / vttestserver hardening track ([ADR-0073](../adr/adr-0073-vitess-internal-and-online-ddl-tables.md)) — *operator-flagged; (b1)/(b2)/(c)/(d) SHIPPED — only (a) `vitess` flavor remains (demand-gated)*
...
- **(a) `vitess` self-hosted flavor.** A sibling to the PlanetScale flavor (same VStream engine code, different defaults + `Capabilities`): sane self-hosted defaults (`vstream_transport=plaintext`, `vstream_auth=none`, explicit shards) so `--source-driver=vitess` "just works" without hand-set `vstream_*`. Start its `Capabilities` identical to `planetscale`; diverge only on evidence. The cheap UX part; not load-bearing until a concrete capability difference surfaces.
...
**Gotchas.** The reproduced bug (now fixed): during the Bug-125 hunt, sluice's COPY filter grabbed a `_vt_vrp_*` shadow during an in-flight deploy and tripped the ADR-0071 scope-mismatch refusal (zero rows) — so the exclusion covers BOTH the COPY filter and CDC dispatch. Anchor the matcher to Vitess's helper (not a static prefix list) + pin the recognized set AND a non-internal-lookalike battery (a false positive = a user table silently dropped from the migration). (c) was silent-loss-tier — "the first real schema change on PlanetScale breaks the sync" if wrong. **Sequence (all done bar (a)): Bug 126 (v0.99.6) → (b1) vttestserver harness + (c) exclusion (v0.99.7) → (b2) compose harness BUILT → (c) full-cutover validation on (b2) PASS + primary-only wedge fixed (v0.99.7 arc) → (d) chaos suite green → (a) `vitess` flavor when a concrete capability difference surfaces (demand-gated).** The tier-2 cluster + chaos suites are gated/manual (not per-PR CI); a scheduled CI job to run them periodically is the open follow-on.
...
### 3. Cross-table copy concurrency for many-table schemas (the pgcopydb many-table gap)
...
- **(a) Cross-table copy concurrency.** A bounded worker pool over the per-table copy loop (N tables copied concurrently), composed *with* the existing within-table `--bulk-parallelism` so the two axes don't oversubscribe (a shared connection/parallelism budget, à la the ADR-0042/0043 copy-pool gate). New knob (e.g. `--table-parallelism`) or fold into a single total-parallelism budget the orchestrator splits across the two axes.
- **(b) Adaptive `--bulk-parallel-min-rows`.** Drop the within-table-split threshold as the table count rises, so a many-medium-table schema auto-engages within-table parallelism (8-way *within* ≈ 4-way *across* for total throughput — the benchmark measured 6.97 s when the threshold was lowered by hand, within ~14 % of pgcopydb). Cheaper than (a); closes most of the gap with no new concurrency surface, but leaves the residual serial-table scheduling overhead.
