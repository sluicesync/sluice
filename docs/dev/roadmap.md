...
**Gotchas.** The reproduced bug (now fixed): during the Bug-125 hunt, sluice's COPY filter grabbed a `_vt_vrp_*` shadow during an in-flight deploy and tripped the ADR-0071 scope-mismatch refusal (zero rows) — so the exclusion covers BOTH the COPY filter and CDC dispatch. Anchor the matcher to Vitess's helper (not a static prefix list) + pin the recognized set AND a non-internal-lookalike battery (a false positive = a user table silently dropped from the migration). (c) is silent-loss-tier — "the first real schema change on PlanetScale breaks the sync" if wrong. **Sequence: Bug 126 (SHIPPED v0.99.6) → (b1) vttestserver harness + (c) exclusion (SHIPPED to `main`, unreleased) → (b2) compose harness (NEXT) → finish the (c) full-cutover validation on (b2) → release → (a) flavor when capability differences surface.**
