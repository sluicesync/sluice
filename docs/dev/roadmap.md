...
- **Path C — pre-release validation VM (LANDED; host migrated 2026-05-19).** Runs `integration vstream` (vttestserver-based VStream coverage) on every release-validation pass without burning CI minutes. **Originally the always-on paid Vultr box; that is being RETIRED (idle + stale) and replaced by a local runner-less Hyper-V validation VM** (`scripts/hyperv-runner/New-ValidationVM.ps1`, cloned from the golden — zero recurring cost, ~1-2 min to provision on demand). Runbook (host-agnostic `go test` block + the new local-Hyper-V provisioning/Go-bootstrap) in `docs/dev/notes/release-validation-on-vultr.md`. Reference timing: ~4 min per run. Captures the Vitess-side coverage gap CI explicitly skips for cost. The remaining gap vs `psverify` is real-PlanetScale-specific behavior (TLS, vendor pgwire-proxy quirks); Path A still covers that surface.
...
### 14. Backup chain retention, pruning, and compaction (GitHub #20) — **SHIPPED (v0.47.0 → Task #16)**

Closed end-to-end. See "Recently landed: Backup chain retention — chunks 14a–14e complete" for the per-chunk summary covering the keystone catalog (14a), rotation (14b Phase 1 / Phase 2), prune (14c), naive compact (14d), and smart compaction (14e). The only forward-looking piece is the `--smart-compaction` default flip, gated on operator field data confirming the safety profile.
