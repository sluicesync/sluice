...
Items previously listed here that have been addressed:
- `BIGINT UNSIGNED` (MySQL) → Postgres — already widens cleanly: non-auto-inc emits `NUMERIC(20,0)` (preserves full unsigned range), auto-inc emits `BIGINT GENERATED BY DEFAULT AS IDENTITY` with documented overflow caveat. Test in `internal/engines/postgres/ddl_emit_test.go`.
- PG arrays → MySQL friendlier error — landed in v0.3.x with copy-paste-ready YAML mappings hint in the error message.
- Generated columns — currently in flight (read-side capture + write-side emission).
