...
What remains are the **harder-frontier, demand-gated items** that have always been here as design-first work — multi-source Shape A, MySQL multi-source native parity, ADR-0032 Tier 3 (uuid-ossp + pgcrypto function-defaults), the remaining 5 deferred translator rules, View Phase 3, and the analytics-export / Arrow research items pending operator demand — plus the **active Vitess/vttestserver hardening track** below (operator-flagged; the make-or-break PlanetScale path).

### 1. Vitess / vttestserver hardening track ([ADR-0073](../adr/adr-0073-vitess-internal-and-online-ddl-tables.md)) — *operator-flagged, active; (c) is correctness-gating*
...
- **(b2) full Vitess compose-cluster harness (NEXT — the unblocker for the (c) cutover proof).** A testcontainers-compose harness around Vitess's `examples/compose` (etcd + vtctld + vtgate + vttablets + MySQL) that runs the **real** online-DDL scheduler — so it performs the VReplication copy + atomic rename cutover vtcombo can't. Wire sluice's planetscale/`vitess` DSN to the compose vtgate. A separate, gated/manual job (heavier boot/resources); the durable vehicle for the cutover-survival proof **and** future resharding / FK-mode / multi-keyspace tests. Chosen over real PlanetScale (free, deterministic, automatable, reusable, validates self-hosted Vitess directly). See ADR-0073's "test-harness tiering" decision.
- **(c) online-DDL / `_vt_*` table-lifecycle robustness (CORRECTNESS-GATING).** **Exclusion: SHIPPED to `main`** (`e9d5544`, validated on vttestserver) — Vitess-internal tables dropped from COPY + CDC via `schema.IsInternalOperationTableName()` (version-dependent naming, [vitessio/vitess#14582](https://github.com/vitessio/vitess/issues/14582): legacy `_vt_HOLD/PURGE/EVAC/DROP` in 19–20 → unified `_vt_<op>_<uuid>_<timestamp>_`, op codes `hld/prg/evc/drp/vrp/gho/ghc/del`, in 20+), anchored to the helper not a static list. **Full cutover-survival: implemented + partially pinned, but GATED on (b2)** before a release is cut — the rename-swap-with-rows can't be proven on vttestserver. See [ADR-0073](../adr/adr-0073-vitess-internal-and-online-ddl-tables.md).
- **(d) chaos harness on (b2) — resilience under real infra failures (NEXT, operator-flagged 2026-06-06).** A `chaos`-tagged layer on the full Vitess-cluster harness injecting **primary failover (PRS + ERS)**, **vttablet/mysqld kill mid-cold-start**, **vtgate restart**, and **rolling Vitess upgrade** — each asserting one invariant: after the fault + recovery, target `COUNT == source` (zero loss/dup) **OR** sluice fails LOUDLY; never a silent partial/hang. Validates the v0.99.8/v0.99.9 resume hardening + the Gap-1 gRPC-transient retry under *real* crashes (not the SIGKILL sim), and is the vehicle to prove F3 below. Local/on-demand (heavy); possible scheduled CI job later.

**Gotchas.** The reproduced bug (now fixed): during the Bug-125 hunt, sluice's COPY filter grabbed a `_vt_vrp_*` shadow during an in-flight deploy and tripped the ADR-0071 scope-mismatch refusal (zero rows) — so the exclusion covers BOTH the COPY filter and CDC dispatch. Anchor the matcher to Vitess's helper (not a static prefix list) + pin the recognized set AND a non-internal-lookalike battery (a false positive = a user table silently dropped from the migration). (c) is silent-loss-tier — "the first real schema change on PlanetScale breaks the sync" if wrong. **Sequence: Bug 126 (SHIPPED v0.99.6) → (b1) vttestserver harness + (c) exclusion (SHIPPED to `main`, unreleased) → (b2) compose harness (NEXT) → finish the (c) full-cutover validation on (b2) → release → (a) flavor when capability differences surface → (d) chaos harness.**
