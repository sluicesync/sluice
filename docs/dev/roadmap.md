...
- **14b. Rotation.** **Phase 1 — rotation-EXIT thresholds — SHIPPED v0.51.0; SUPERSEDED by Phase 2.** `--exit-after-age=DUR` and `--exit-after-chain-length=N` on `backup stream run`: at the configured chain age (from `chain.json`'s `CreatedAt`) or incremental count, commit the current rollover and exit cleanly; chain.json gets `RotatedAt` + `RotationReason` markers. Phase 2 supersedes this — the Phase 1 flags now error with a clear migration message pointing to `--retain-rotate-at*` (clean break per the zero-users tenet).
  **Phase 2 — `--retain-rotate-at=DUR` in-process inline rotation — SHIPPED v0.67.0 (ADR-0046; hardened v0.67.1 / Bug 66).** When the threshold trips, the *same goroutine* opens a new snapshot stream, bulk-copies source state into a sibling chain root, writes the `SucceededBy` auto-stitch pointer, and resumes streaming — no operator wrapper. The snapshot/CDC overlap during rotation (the hard part) is the bounded-segment lineage model in ADR-0046, refined by **ADR-0067 (v0.88.0, this session)** which made rotation born-contiguous (the `(P_N, S]` overlap) so rotated chains are compactable (closes Bug 95).

- **14d. `sluice backup compact --from-dir CHAIN --merge-window DUR` — SHIPPED v0.77.0 (Task #15).** Merges K consecutive incrementals into a single combined incremental with the same end-position but smaller file/inode footprint. v1 is the **naive concat** — read all change events in order, write to a single new `.jsonl.gz` chunk under a new `_changes/<merged-ts>/`, delete sources, tombstone merged manifests in `chain.json`. The new manifest gets the last source's end-position. Restore-side is no-change: merged manifests look like larger incrementals to the existing chain iterator. Stays the default; 14e's smart compaction is the opt-in extension.

- **14e. Smart compaction (same-row event collapsing) — SHIPPED (Task #16).** Within the merge window, collapse same-PK event chains into the net result per the [ADR-0064](adr/adr-0064-backup-smart-compaction.md) policy table: INSERT+UPDATE(s) → INSERT with final values; UPDATE(s) → one UPDATE; INSERT+DELETE → nothing; UPDATE(s)+DELETE → just DELETE; DELETE+INSERT (row reused) → both verbatim. TRUNCATE/DDL are barriers; tables without a declared PK fall through to naive concat and are listed in the compaction report. Opt-in via `--smart-compaction` (default off in v1 for the cautious-ship reason; flip in a later release once field data confirms the safety profile). Refuses LOUDLY on corrupt PK with a `--smart-compaction-off` recovery hint. Encrypted chains refuse loudly in v1 (no cross-keyset re-encrypt yet). Observed reductions on update-heavy synthetic workloads: 50–95% event drop + matching byte savings.
