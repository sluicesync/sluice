...
| **6.3 — GCP Cloud KMS + Azure Key Vault** (shipped v0.34.0) | Per-cloud SDK wrappers in `internal/crypto/gcp_kms.go` (`KEKModeGCPKMS = "gcp-kms"`) and `internal/crypto/azure_kms.go` (`KEKModeAzureKMS = "azure-kms"`); `--gcp-kms-key-resource` / `--azure-key-vault-id` (+ `--azure-wrap-algorithm`) flags; unit tests with deterministic stubs covering preflight, round-trip, wrong-key refusal, error-message translation; same CEK-caching pattern as Phase 6.2. Flag-name deviation from the original sketch: GCP gets `--gcp-kms-key-resource` rather than `--kms-key-resource` to disambiguate from AWS's `--kms-key-arn` in operator-help output. The Azure surface uses WrapKey/UnwrapKey (not Encrypt/Decrypt) — Key Vault's recommended pattern for AES-256 CEK wrapping; the default algorithm is RSA-OAEP-256, with `--azure-wrap-algorithm=A256KW` for HSM-backed AES keys. | ~700 LOC actual (incl. tests + CLI wiring + 4 mutual-exclusion cases). |
| **6.4 — Operator guide + key-management ADR** (shipped v0.34.0+docs) | `docs/operator/encryption.md` extended with GCP Cloud KMS + Azure Key Vault setup sections (passphrase + AWS already covered in v0.23.0). ADR-0037 captures the cross-provider design decisions + per-provider trade-offs + minimum-privilege IAM rationale. The "Choosing between providers" comparison table lets operators pick the right mode without re-reading every section. | ~600 LOC docs (~200 ADR + ~400 operator guide additions). |
