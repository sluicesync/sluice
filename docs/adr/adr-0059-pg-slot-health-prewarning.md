...
## Implementation note (2026-07-15): terminal conditions, hysteresis, and the probe's own outage (audit 2026-07-15 findings MED-D0-9, MED-D0-11, LOW-D0-17, LOW-D0-18)

The 2026-07-15 repo audit observed, live on PG16, that the alerting net INVERTED at the exact terminal event it exists to precede: an invalidated slot (`wal_status='lost'`) reports NULL lag, the reporter mapped NULL lag to 0 bytes, the evaluator computed 0% pressure, decided "clean", and emitted a false "condition cleared" INFO — paging stopped at the moment of loss. Four changes make the net truthful at the edges:

**Terminal conditions page CRITICAL once and then latch (MED-D0-9, LOW-D0-17).** The evaluator now dispatches on `wal_status` BEFORE the percentage math. `wal_status='lost'` — and a slot row that VANISHES while a condition is outstanding (someone dropped it mid-stream; pre-fix this skipped silently forever) — are unrecoverable within the slot's lifetime: they emit one ERROR log line + one critical page (the lost page names the re-snapshot recovery; the dropped page names the mid-stream drop) and then LATCH. A latched state never re-fires (re-paging an unrecoverable event every 5m adds noise without information) and — load-bearing — never emits "condition cleared", not even for a probe that would otherwise evaluate clean. The latch's lifetime is the probe attachment (one `runOnce` attempt); a streamer restart, including an auto-re-snapshot restart, re-attaches the probe with fresh state. A slot dropped while HEALTHY stays silent (cold-start-race compatibility, and a deliberate operator drop of an idle slot is the operator's own action — the streamer's read path fails loudly if the slot was still needed).

**`wal_status='unreserved'` pages CRITICAL immediately but is NOT terminal.** Unreserved means the cap is exceeded and invalidation lands at the next checkpoint — the last window where intervention can still save the slot, so it pages at critical level right away. But Postgres documents that unreserved can return to reserved/extended if the consumer catches up before the checkpoint, so it keeps normal clear semantics: latching on a recoverable state would silence the paging net for the rest of the stream, recreating the MED-D0-9 inversion from the other direction.

**Sustained probe failure escalates to WARN + page (MED-D0-11).** Probe errors were DEBUG-only forever, so a revoked role or killed connection silently disabled the whole paging net. After `slotHealthProbeFailureEscalateAfter` (5) CONSECUTIVE failures — ~2.5 minutes of blindness at the 30s cadence — the loop WARNs and pages (warning level: the slot may be healthy; the WATCHER is broken) once per outage streak, then logs a recovery INFO and re-arms when a probe succeeds.

**Downgrades are damped by one probe tick (LOW-D0-18).** Kind transitions used to emit unconditionally, so a reading hovering on the 70/85 boundary (or flapping across 70/clean via the cleared-then-re-entered rule) paged on every 30s probe, bypassing the 5m rate limit. A DOWNGRADE (85→70, anything→clean) now commits only if the lower state persists for one extra probe tick; a flap back to the prior kind lands in that kind's normal rate-limit window instead. ESCALATIONS (clean→70, 70→85, anything→lost) still emit immediately — promptness on the way up is the feature. The observable deltas: a genuine downgrade or clear announces one probe tick (~30s) later than before, and a boundary oscillation emits once per rate-limit window instead of once per probe.
