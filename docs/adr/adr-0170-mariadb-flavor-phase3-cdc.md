...
- **Cross-engine `sync start` end-to-end validated (`mariadb → postgres`).** `TestStreamer_MariaDBToPostgres` drives the full pipeline: cold-start snapshot → CDC handoff → INSERT/UPDATE/DELETE convergence on a real PG target → stop → warm-resume of a while-down change. Asserts value fidelity through BOTH the bulk-copy AND CDC paths for int / text / **JSON identity** (carried verbatim — MariaDB JSON is textual, `ir.JSON{Binary:false}`, PG `json` preserves exact bytes) / **temporal** `DATETIME(3)`. Two defects the reader-level pins could not catch surfaced here and were fixed in this chunk:
  - **Warm-resume Bug-142 shape (fixed):** the streamer's retag-for-source stamps a resumed position with the source engine's `Name()` = `"mariadb"`, which `isMySQLFamilyEngine` did not accept → warm-resume crash-looped on `decode binlog position: engine = "mariadb"`. Added `engineNameMariaDB` to the decode-accept family (mirrors the vitess entry); pinned by `TestDecodeBinlogPosAcceptsMariaDBEngine`.
  - **Native uuid / inet6 / inet4 do NOT survive CDC (KNOWN GAP, deliberately deferred — loud, not silent).** Phase 2 (ADR-0169) added these for schema + bulk copy, where the driver hands back formatted TEXT. The binlog carries the RAW storage bytes (16 for uuid/inet6, 4 for inet4), and `decodeValue`'s `ir.UUID`/`ir.Inet` handler assumes the MySQL "stored in a VARCHAR → text" shape and stringifies the raw bytes; on a PG target this fails LOUDLY (`invalid input syntax for type uuid`, SQLSTATE 22P02). The cross-engine corpus therefore excludes native uuid/inet (bulk migrate of them is unaffected). The proper fix is a **flavor+type-aware binlog decode** (format 16 bytes → uuid text, etc.) that FIRST ground-truths MariaDB's uuid byte ORDER — MariaDB reorders the time fields, so a straight big-endian format would produce a *valid-but-wrong* uuid, a Bug-74-class SILENT corruption if done without a live pin — and pins the uuid/inet4/inet6 family on both LTS lines. Filed as a follow-up chunk.
