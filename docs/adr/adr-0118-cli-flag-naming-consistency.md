...
## Implementation note (2026-09-22, GC-19 / census C-4)

The finding 1(b) WARN shipped hand-kept: three flag names in a slice and a `case "mysql", "planetscale", "vitess"` engine allowlist. The census found it reached 3 of ~11 flags whose own `--help` said "inert on" / "-only" (`--bulk-batch-size` sat three lines below the covered three with the identical sentence), and the allowlist made even those three silent on the trigger-CDC, mydumper and flat-file sources. It is now a data-driven registry (`cmd/sluice/inert_flags.go`): one row per (command, flag) with an `inertWhen(source, target)` predicate derived from the capability the pipeline itself dispatches on — `ir.SnapshotImporterOpener` for the ADR-0079 fast-lane knobs (the same optional interface `coldStartFastEligible` type-asserts), `Capabilities().CDC` for the binlog / VStream / trigger families, `ir.TargetConnectionBudgetProber`, `ir.TargetStaleBackendReaper`, `Capabilities().PostgresBackend` (the engine-level mirror of the post-dial `ir.IndexBuildTuner` writer surface), the `mysql.Engine` assertion `applyControlKeyspace` makes, and `Capabilities().SchemaScope`. Every WARN carries the grep-stable `INERT-FLAG` marker and names the flag, the command, the engine and the reason. Three gates hold it: `TestInertFlagWarnCoversEveryInertMarkedFlag` walks the real kong model for inert-marker phrases in help text and requires each hit to be a registry row or a reasoned exemption (floor ≥10; ~100 marked flags today, 37 rows); `TestInertFlagWarnWiringRoster` resolves every `warnInertFlags` call site from the AST and holds the registry's command set to it; `TestInertFlagPredicates_EngineMatrix` pins every row against every registered engine by an explicit applies-set, so an unclassified new engine fails rather than inheriting a verdict. The `sync run` fleet path has no argv and carries only `control-keyspace` of the registry's keys; `warnInertFleetControlKeyspace` is its one sibling, keyed on non-empty.
