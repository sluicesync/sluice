...
6. **Pagination by keyset.** Large tables are read in PK-keyset pages
   ...
   **Amended 2026-09-02 (audit 2026-09-01 LA-2):** "default N tuned under D1's response-size limit" rested on a false premise — D1 has no meaningful response-size limit. Measured on real D1 on 2026-09-02: a 1,000-row page of 16 KiB BLOBs came back whole (HTTP 200, chunked, uncompressed, 32,839,264 bytes in 2.18 s, ~15 MB/s; the audit had already seen a 12,000,669-byte response), the transport ran at its full ~13–15 MB/s for every page of 3 MB or more and fell to ~1.8 MB/s at 328 KB pages (RTT-bound), D1 stores a single value up to ~4 MB (a 4,100,000-char TEXT accepted; a 4 MiB BLOB and a 6 MiB row refused `SQLITE_TOOBIG`), and a 1 MiB BLOB reads back through `hex()` as a 2 MiB response while a 2 MiB BLOB cannot be `hex()`ed at all (`SQLITE_TOOBIG`, a loud read-page error — a residual this amendment records but does not fix). The only cap on a response is therefore sluice's own 8 MiB `LimitReader` (`d1MaxResponseBytes`, a memory bound), and against it the fixed 1,000-row page made every table of rows wider than ~8 KB unmigratable: the reader decoded the truncated body and failed with `unexpected end of JSON input`, naming neither cause nor remedy. The page size is now adaptive (`internal/engines/sqlite/d1_rows.go`): pages are sized in bytes against `pageByteBudget` (half the cap, so row widths may double between two pages before a page overflows) — the first page from a server-side `LENGTH()` probe of the widest row among its candidates (`probeMaxRowWidth`, MAX over the first 1,000 rows in keyset order; 57 ms for 1,000 rows of 16 KiB BLOBs; the estimate came within 0.2% of the measured page), every later page from the previous page's measured bytes per row (`pageRowsFor`), with `d1PageSize` = 1,000 kept as the CEILING so ordinary tables page exactly as before. A page that overflows the cap anyway (row widths grew faster than predicted, or JSON escaping inflated a text — a control character is six bytes on the wire) is halved and re-requested at the same keyset bound, which is idempotent; the one row that still overflows as a page of one is refused with `SLUICE-E-BULKCOPY-ROW-TOO-LARGE`, naming the table, the row's key, its estimated size and the cap. The overflow itself is now a named `d1ResponseTooLargeError` on every query path, so the `d1-trigger` change-log poll — which shares the transport and its cap but NOT the adaptive page (its batch is the CDC poller's; a batch of change-log rows over 8 MiB now fails by name rather than as a JSON decode error, and is a separate follow-up) — no longer misreports the event either. Verified on real D1 at the production cap: 300 rows × 16 KiB migrate in three pages of 127 on both the direct reader and `--stage-local`; a 1,000-row narrow table still reads as one 1,000-row page; a 1 MiB BLOB row is read, not refused; and a single 1.6 MB text of control characters (~9.6 MB on the wire) is refused by name after the halving reaches one row (`TestD1Verify_WideRowsPageByBytes`). Note the refusal is reachable on real D1 only through JSON-escaping inflation or several multi-megabyte columns in one row — D1's own value and row limits keep any BLOB row under the cap.
