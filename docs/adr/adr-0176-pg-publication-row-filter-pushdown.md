...
- **Q1 resolution (2026-07-23, owner call — audit D0-5):** the filtered-replica contract is **the source engine's own comparison semantics**. `rowpredicate.Compile` now normalizes a finer-than-column temporal literal to the engine's coercion (PG: cast-to-column — a DATE truncates the time-of-day, fractional seconds round half-even to µs; MySQL: promote + round half-up; MariaDB: promote + truncate — each OBSERVED on a real server 2026-07-23 and permanently ground-truthed by `internal/rowpredicate`'s three-engine temporal matrix), threaded engine-neutrally via `ir.TemporalLiteralSemantics` on the collation-resolver seam. With the client evaluator source-faithful by construction, the interim M0-3 granularity *exclusions* moved BACK INTO the envelope: the oracle gained date×time-bearing and timestamp×{7-digit, half-even-boundary, carry} equivalence cells, and the `PushdownTerm` granularity flags remain only as the classifier's fail-closed belt for a compile that missed the engine lens. Note the §4 rule is *refined*, not violated: the client evaluator was not bent to match a divergent server cell post-hoc — the CONTRACT was decided to be server semantics, the pre-existing Compile-layer defect (client-authoritative CDC dropping changes for rows the snapshot's server-evaluated `WHERE` had copied, e.g. `d = '2024-01-01 08:30'`) was fixed at its root, and the equivalence oracle stays the gate.
