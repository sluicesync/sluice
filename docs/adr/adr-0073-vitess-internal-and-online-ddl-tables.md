...
**Status consequence:** part (c)'s **exclusion** is validated + shipped to `main`; its **full cutover-survival** is implemented + unit/partial-integration-tested but **gated on tier 2 (the compose harness)** before a release is cut on it. Building that harness is roadmap track item (b2).
