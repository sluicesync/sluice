...
sluice vendors `vitess.io/vitess v0.24.1` — i.e. **Vitess 24** (the Go module `v0.X` maps to the Vitess MAJOR release: `v0.24.x` == Vitess 24.0.x). Its `schema.IsInternalOperationTableName()` (from v0.24.1 / Vitess 24) is importable and **covers every format real PlanetScale / vttestserver (the unified format Vitess 20+ emits, which Vitess 24 still uses) emit — no Vitess bump needed.** Recognized set (pinned in `vstream_internal_table_test.go`):
...
The tier-2 harness is `startVitessCluster(t)` (`internal/engines/mysql/cdc_vstream_cluster_integration_test.go`, build tag `integration && vitesscluster`). It mirrors `startVTTestServer`'s ergonomics (returns the vtgate MySQL DSN + gRPC endpoint + keyspace + cleanup) but boots a **real** cluster via `docker compose` (`testdata/vitesscluster/docker-compose.yml`): etcd + vtctld + a PRIMARY vttablet + a REPLICA vttablet + vtgate, all on `vitess/lite:v24.0.1` — **Vitess 24**, matching the vendored `vitess.io/vitess v0.24.1` client (the Go module `v0.X` maps to the Vitess MAJOR: `v0.24.x` == Vitess 24.0.x; keep the image MAJOR in lockstep with the vendored client so the harness exercises the same online-DDL / `_vt_*` lifecycle as the code under test, and the same baseline real PlanetScale runs). The harness shells to `docker compose` directly rather than adding the testcontainers compose module (zero new dependency).
...
**Robustness finding surfaced by (b2) — primary-only CDC-tail silent wedge — FIXED.** sluice's VStream CDC tail used to hardcode `TabletType_REPLICA` (`cdc_vstream.go` `buildVStreamRequest`; matches PlanetScale's read-from-replica convention). Against a **primary-only** cluster — PlanetScale *development* branches, and minimal self-hosted setups — vtgate has no REPLICA tablet and the stream **wedged silently**: vtgate logs `failed to find a REPLICA tablet for VStream in <ks>/<shard>`, and (empirically, via a primary-only variant of this harness) sluice's reader presented this as a **silent hang** — `Err()` stayed nil and no rows ever flowed — rather than a loud failure: a loud-failure-tenet violation.

*Phase-A ground truth* (primary-only compose variant of this harness, instrumented at the pump's `Recv`, Vitess 24). The earlier (b2) hypothesis "the no-REPLICA stream sends nothing" was **WRONG** — corrected here:

- **DEAD stream (no REPLICA tablet):** `Recv` returns a `[HEARTBEAT]` batch every ~5s (the request's `HeartbeatInterval: 5`), **and nothing else, indefinitely** — no VGTID, no FIELD, no ROW, no error. vtgate keeps heart-beating even though it has no tablet to actually serve the keyspace.
- **HEALTHY stream:** the **very first** `Recv` returns `[VGTID OTHER]` *immediately* on attach — vtgate stamps the starting position before any row and before the first heartbeat — even when the source is completely idle.

So a HEARTBEAT does **not** prove a serving tablet exists (vtgate sends it regardless); **any non-heartbeat event does**. The clean discriminator is therefore "no NON-HEARTBEAT event within the window", NOT "no event of any kind" — the latter would never fire because heartbeats always flow. A healthy idle source disarms the watchdog within the first second (its initial VGTID), so legitimate long-idle workloads never false-time-out.

*The fix* (two parts, both shipped):

1. **Loud liveness timeout (the tenet fix).** A first-event watchdog (`cdc_vstream_liveness.go`, `vstreamLiveness` + `eventsProveLiveness`) keyed on the **absence of any NON-HEARTBEAT event**: if no VGTID / FIELD / ROW / DDL / … arrives within `vstream_liveness_timeout` (default 30s ≈ 6× the 5s heartbeat; DSN-overridable, `0` disables) of opening the stream — even though heartbeats are flowing — the reader records a **loud** error naming the tablet type, keyspace, and remediation (`set vstream_tablet_type=primary if the cluster is primary-only`) and cancels the stream so the parked `Recv` unblocks. Disarmed on the first non-heartbeat `Recv` batch, so a long-idle-but-healthy source (whose initial VGTID + ongoing heartbeats flow) never false-times-out — reconciling with the pre-existing long-idle tolerance. Applied to all three VStream pumps: the standalone CDC tail (`vstreamCDCReader.pump`), the snapshot COPY pump, and the snapshot post-COPY CDC pump.
2. **Configurable tablet type (usability).** A `vstream_tablet_type={primary|replica|rdonly}` DSN param (default `replica` — unchanged for PlanetScale production) selects the tablet for the **pure CDC tail**; the ADR-0072 COPY-resume PRIMARY override (TablePKs cursor present) still wins regardless. A primary-only cluster now works via `vstream_tablet_type=primary`. An unknown value is a loud parse error. RDONLY is selectable for enterprise setups that prefer to offload even replicas.

*Auto replica→primary fallback: DEFERRED.* vtgate returns **no error** on the no-tablet condition (it just goes silent), so an automatic fallback would have to be "timeout-then-retry-as-primary" — which adds a reconnect FSM and a doubled startup latency for a case the explicit `vstream_tablet_type=primary` flag already solves cleanly. Not worth the complexity; the loud timeout + the flag are the right surface. Revisit only if operators report the manual flag is a friction point.

*Pins* (primary-only compose variant `docker-compose.primary-only.yml` + `startVitessClusterPrimaryOnly`, build tag `integration && vitesscluster`): `TestVitessClusterPrimaryOnly_DefaultReplicaTailFailsLoudly` (default REPLICA → the loud timeout fires, `Err() != nil` with the actionable message, NOT a silent hang) and `TestVitessClusterPrimaryOnly_PrimaryTabletTypeTailWorks` (`vstream_tablet_type=primary` → DML events flow, zero loss). Unit pins (`cdc_vstream_test.go`): the `vstream_tablet_type` / `vstream_liveness_timeout` DSN parses (valid + default + invalid→loud), the `buildVStreamRequest` tablet selection (cursor-present → PRIMARY regardless; no cursor → the configured type), the `eventsProveLiveness` heartbeat-vs-serving discriminator (heartbeat-only → no proof; any non-heartbeat → proof), and the watchdog's fire/disarm/disabled contract.
