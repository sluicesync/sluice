...
**Empty-diff fail-closed tripwire (2026-07-16 amendment, audit MEDIUM):** a deployable DR structurally cannot have an empty diff — `waitDeployable` already refused `no_changes` — so the subset-only assertion passing on ZERO decoded entries was the gate's one silent failure mode: with the diff response shape derived-not-captured (point (b) below), a mis-modelled envelope key would decode every real diff as empty and no-op the gate on every leg while every fakePS-backed test stayed green. The runner now refuses (`drFailure`, same coded family) when the diff decodes empty on a leg with a non-empty intended set, naming the derived-shape caveat. This doubles as the live shape-verification tripwire: the first post-fix psverify dispatch proves the envelope decodes non-empty against the real API. Pinned both ways (`TestLegRunner_EmptyDiffWithIntendedSetRefuses`; the happy-path pins serve non-empty diffs).
