...
**Residual risk / recommended follow-up:** the unit pins construct the exact wire shape Vitess produces (NULL-cell after image + packed bitmap), but a full cluster-level validation — a self-hosted Vitess-24 mysqld set to `binlog_row_image=NOBLOB` with the `AllowNoBlobBinlogRowImage` experimental flag, streamed end-to-end — belongs to the heavy gated `vitesscluster` suite (`vitess-cluster-validator`) and is the recommended next validation. PlanetScale/FULL streams are unaffected and already covered by the existing VStream integration suite.
