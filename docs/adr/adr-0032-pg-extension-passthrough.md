...
- **hstore** (Tier 1 — first opaque-text validation) **— shipped together with citext.** Tier 1 type-only entry: catalog `pgHstoreDef` declares the `hstore` udt; same-engine PG → PG emits the bareword `hstore` and round-trips values as PG-canonical text (`"k"=>"v"`). Cross-engine PG → MySQL gets a built-in default translator: `hstore` → MySQL `JSON`, with the writer's `prepareValue` reparsing the hstore wire form into a JSON object string at value-write time (`prepareHstoreToJSON` / `parseHstoreText` in `internal/engines/mysql/row_writer.go`). The flag-validate gate (`validateEnabledPGExtensions`) relaxes for hstore against non-PG targets via the new `ir.CrossEngineExtensionTranslator` optional engine surface — PG declares hstore + citext as cross-engine-translatable; other extensions preserve the strict refusal. **v0.31.0 deferred PG → PG bulk-copy** because the IR carried text-form hstore bytes and PG's binary COPY protocol expects hstore's pair-array wire format; `validateEnabledPGExtensions` refused at preflight with a `--type-override` workaround hint. **v0.32.1 ships the PG → PG COPY binary codec** at `internal/engines/postgres/hstore_codec.go` — `pgHstoreBinaryCodec` mirrors `pgvectorBinaryCodec`'s structural pattern; the writer's `writeViaCopy` gate now registers either codec on conn-per-COPY when the table carries the matching column type (`tableHasPGVectorColumn` / `tableHasHstoreColumn`). The two-codec arrangement is the documented pattern for future Tier 2+ extensions with their own wire formats (e.g. PostGIS EWKB).
...
- **postgis** (Tier 2 — final v1 entry) **— shipped v0.33.0.** Completes the ADR-0032 v1 shortlist. PostGIS is the catalog's first "type-less" entry: `typesByName` is empty because `geometry` and `geography` pre-date ADR-0032 and already have first-class IR shapes — `geometry` rides on `ir.Geometry` per ADR-0035 (v0.28.0), `geography` has no IR shape today but surfaces in the hint path. The load-bearing v1 surface this entry contributes is the **operator-class round-trip path**: `pgPostGISDef.indexOperatorClasses` declares the nine canonical PostGIS opclasses (`gist_geometry_ops_2d` / `_nd`, `gist_geography_ops`, `spgist_geometry_ops_2d` / `_3d` / `_nd`, `brin_geometry_inclusion_ops_2d` / `_4d` / `_nd`) that operators rely on for spatial-index strategy selection. With `--enable-pg-extension postgis`, the schema reader's `extensionOperatorClassEnabled` gate preserves these via `ir.IndexColumn.OperatorClass` so the same-engine writer emits the opclass and PG selects the matching index strategy on the target. Without the flag, the opclass drops with the existing WARN (loud-failure default; mirrors pg_trgm). A new `extensionDef.hintTypeNames` field carries `geometry` / `geography` so `extensionOwningType` surfaces the actionable `--enable-pg-extension postgis` hint for unenabled-extension columns — the IR path stays unchanged. The pre-existing PG → MySQL geometry path (ADR-0035) is untouched: `--enable-pg-extension postgis` is meaningful only on PG → PG; cross-engine targets receive the strict loud-failure default per `validateEnabledPGExtensions` (postgis is not in `crossEngineDefaultTranslatedExtensions`). The catalog framework needed no new shape — the empty-`typesByName` plus `indexOperatorClasses` arrangement is the pg_trgm pattern, and `hintTypeNames` is a small additive field on `extensionDef`. Integration tests live under the `integration postgis` build tag in `internal/pipeline/migrate_pg_postgis_passthrough_integration_test.go`, sharing the `postgis/postgis:16-3.4` image with the ADR-0035 PG → MySQL test.
