...
Accepted — repo-audit task M2.2 (operator-authorized 2026-06-10). Tier (a) merged via PR #170 (`internal/appliershared` helpers); tier (b) — the batched-apply loop extraction this ADR primarily documents — merged via PR #173; tier (c) — the control-table CRUD — implemented 2026-06-10 (notes below). Tier (d) remains an open follow-up. Builds on the metadata convergence in repo-audit M2.1 (PR #169: both engines' appliers use `map[string]*ir.Column` caches and `(string, []any, error)` SQL builders), which made the two engines' batch loops structurally diffable in the first place.
...
- **(c) DONE (2026-06-10):** control-table CRUD (`readPosition`/`listStreams`/`requestStop`/… were near-identical skeletons over dialect-divergent SQL) — same seam style via `ControlTableConfig`; includes the mirrored lease-table CRUD that lived in `control_table.go` (heartbeat/record/finalize/list/select/delete + the row type and scan). See the tier-c implementation notes above.
- **(d) OPEN:** lease/keyset/heartbeat helpers (`shard_consolidation_lease.go`, `keyset_store.go`, `heartbeat_writer.go` pairs) — the largest remaining mirrored surface. Tier (c) already moved the lease SQL primitives those wrappers call; (d) is the wrapper/manager layer.
