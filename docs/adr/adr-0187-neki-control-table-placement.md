...
| `MigrationStateStore.EnsureControlTable` (`migration_state.go`) | `sluice_migrate_state`, `sluice_migrate_table_progress` — migrate's door, also reached by the sync cold-start progress recording, backfill and the expand/contract DDL executor. (An earlier draft of this row, and the fix's commit message, said "AND restore's" — that was wrong: a chain restore never opens this store; it reaches `ChangeApplier.EnsureControlTable` above, which is placed too.) |
