...
- **Status quo — operator watches PlanetScale's metrics in their own Grafana, sluice stays reactive.** Entirely safe (no silent loss) and the correct default until demand appears — which is why this is Proposed/demand-gated. The win it leaves on the table is exactly the proactive-back-off + in-tool observability this ADR scopes.
