...
- **Postgres** (`cdc_reader.go::dispatchWAL`):
  ...
  - `CommitMessage` → `ir.TxCommit{Position: <TransactionEndLSN>}` — the POST-commit point, the first LSN after the commit record. **Corrected 2026-09-01 (audit A2-1, the Postgres sibling of item 132):** this line said `<CommitLSN>`, and the reader did emit it, but `CommitLSN` is the commit record's *start*, and logical decoding skips a transaction on resume only when that start is strictly before the requested LSN — so a resume from a cleanly-persisted `TxCommit` re-delivered the whole transaction it had just acknowledged (its `RelationMessage` rendered from the historic catalog, and its rows). Every mid-stream DDL refusal re-fired on that replay after the operator had applied the DDL on the target as the hint said, and a `RENAME TABLE` recovery routed the replayed rows to the old name into `sluice_cdc_skipped_tables`. Rows and `TxBegin` keep `CommitLSN` (the pre-transaction point) so a position persisted mid-transaction still replays the transaction whole. Pinned on a real server by `TestPGCDC_TxCommitPositionIsPostCommit` (both directions) and `TestPGCDC_DrainedModelRecoveryResumes`.
