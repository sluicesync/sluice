...
The PG `migrate` **overlapped copy+index** path (ADR-0077, `runOverlappedCopyAndIndexPhase` → `ir.IncrementalIndexBuilder.BuildTableIndexesFromChannel`) builds per-table indexes *interleaved with copy* inside the engine method, so a pipeline-level wrap can't reach it; making that path reparent-resilient needs a classify-and-retry **inside** the engine's per-table build (it currently cancels the group on the first build error). The live-failing path (restore) uses the now-wrapped `CreateIndexes`, not the overlap path, so this is a documented follow-up, not a regression. Tracked as roadmap item 42.
