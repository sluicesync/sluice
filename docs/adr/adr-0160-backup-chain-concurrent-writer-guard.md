...
- **The Azure leg is code-path-covered, not emulator-pinned.** The gocloud `IfNotExist` mapping is pinned through three real drivers (fileblob, memblob, s3blob-on-MinIO); no Azurite emulator runs in the suite. The per-driver precondition plumbing is upstream-tested (gocloud drivertest); first real-cloud validation should note it here. (This residual originally covered GCS too — closed by the 2026-07-16 real-GCS probe, see the backend matrix row; Azure is now the only derived row.)
