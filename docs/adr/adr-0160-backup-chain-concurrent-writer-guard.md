...
| S3 (`s3://`) | gocloud `IfNotExist` → `If-None-Match: *` | yes — **real-cloud-verified 2026-07-16** (AWS S3, us-east-1: fresh-key PUT → 200, occupied-key → 412 `PreconditionFailed`, exactly-one-winner under concurrent conditional PUTs; through sluice's own `BlobStore` the interleaved-writer scenario produced the coded `SLUICE-E-BACKUP-CHAIN-CONFLICT` refusal with zero catalog bytes written by the loser. Probe log: sluice-testing `workspace/aws/s3-cas-probe.md`. Previously MinIO-derived; MinIO pin retained) |
| GCS (`gs://`) | gocloud `IfNotExist` → generation-0 precondition | yes — **real-cloud-verified 2026-07-16** (GCS us-east1: fresh-key `ifGenerationMatch=0` PUT → 200, occupied-key → 412 `conditionNotMet`, exactly-one-winner under 2-way AND 8-way concurrent conditional PUTs with zero non-412 loser shapes; through sluice's own `BlobStore` the interleaved-writer scenario produced the coded `SLUICE-E-BACKUP-CHAIN-CONFLICT` refusal with zero catalog bytes written by the loser. gocloud v0.46.0 sends the precondition: `gcsblob.go:658` → storage SDK `ifGenerationMatch=0`. Probe log: sluice-testing `workspace/gcp/gcs-cas-probe.md`) |
...
**The S3 409 loser shape (mapped 2026-07-16, from the real-cloud probe):** a *truly simultaneous* conditional-PUT loser on real S3 can receive 409 `ConditionalRequestConflict` instead of 412, which gocloud does not map to `FailedPrecondition` — pre-fix that loser fell into the degrade-WARN path above, whose "may not support conditional PUTs" wording was wrong in exactly that instant. `BlobStore.PutIfAbsent` now classifies the 409 (smithy `APIError` code), retries the create-only PUT once (a 409 writes nothing, and the racer that caused it may itself have failed), and resolves any still-contended outcome to `ErrPathExists` — the coded conflict refusal, never the degrade path. Pinned: `TestConditionalPutConflictRetry`.

**GCS has no 409-analogue — the conflict-retry classifier is correctly S3-specific.** The 2026-07-16 GCS probe observed ONLY 412 `conditionNotMet` losers even at 8-way truly-simultaneous contention (GCS serializes conditional writes on generation; the S3 409 is an S3-implementation artifact), and the classifier keys on an AWS-smithy error type GCS traffic can never carry — so it never engages on GCS, and that is a correct scoping, not a coverage gap. GCS's only non-412 conditional-write loser shape is 429 `rateLimitExceeded` (sustained >1 mutation/sec on one object), which gocloud maps to `ResourceExhausted` → the degrade-WARN path, not the coded refusal — safe direction, not reproduced even at 8-way concurrency, and structurally unlikely against one-shot per-generation claim markers.
...
## Implementation note (2026-07-29): what this ADR does NOT cover, learned the expensive way

This ADR's Context opens with "two cron'd `backup incremental` runs". **That scenario still produced a permanently unrestorable chain after this guard shipped**, reproduced 2026-07-28 against real AWS S3 *and* a local directory, with **both runs exiting rc=0**. It is worth recording precisely, because the guard was working correctly the whole time.

Both writers resolved the SAME parent — the parent is resolved *before* the CDC window opens, since the window's StartPosition comes from it — and then spent minutes streaming. The winner committed and appended. The loser then loaded THE WINNER'S catalog, appended beside it, and claimed the next generation **cleanly**: no collision, nothing clobbered, `lineage.gen/` advancing 1→2→3 exactly as designed. The result was `segments[0].incrementals = [A, B]` with A and B both recording the same `parent_backup_id` — a FORK. `backup verify` and `restore` refuse it from then on, and the forked chain keeps ACCEPTING incrementals off whichever sibling won the tail.

**The lesson: CAS serialises the WRITES; it never asked whether the LINK belonged at the end of the chain.** Those are different invariants, and no amount of write-serialisation implies the second. Postgres slot exclusivity does not substitute either — the loser blocks on the slot, waits it out, re-reads the same changes from the slot restart position, and commits its sibling reporting the same change count.

The fix (v0.104.7, `internal/pipeline/lineage/chain_tip.go`) is a precondition on the link rather than more CAS: **an incremental may only be appended while its parent is still the open segment's tip.** It is enforced twice — before the manifest write, so the loser writes nothing durable, and inside `UpdateLineageForManifest` against the catalog that call loaded under this ADR's observation, which is the durable gate every chain writer funnels through. Refusals reuse `SLUICE-E-BACKUP-CHAIN-CONFLICT`: same class, same operator action.

This also sharpens the claim→Put residual below. Two writers landing inside that window can both observe the old tip and both pass the tip check, and the generation claim does not arbitrate them either (a writer that observes the *other's* marker claims the generation after it and also "wins"). The outcome there is a LOST UPDATE — one link orphaned, the chain still linear and still contiguous, because both siblings start at the same parent position — never a fork. That is the honest boundary of what is closed today.

## Residuals

- **The claim→Put gap (the create-only-CAS residual).** The claim and the catalog Put are two operations, not one: a competitor whose *observation* lands inside another writer's claim→Put window (normally milliseconds — the claim marker PUT immediately followed by the catalog PUT of an already-marshaled body) reads the pre-Put catalog, claims the NEXT generation, and both writers then Put unconditionally — last-write-wins again, undetected. True closure needs conditional *overwrite* (`If-Match` ETag on `lineage.json` itself). **The 2026-07-16 real-AWS probe confirmed the closure primitive EXISTS upstream**: real S3 honors `If-Match` ETag conditional overwrite (200 on the current ETag, 412 on a stale one), so closure no longer waits on the provider — the gap is gocloud-only (`WriterOptions` has no `IfMatch` in v0.46.0), and is actionable via a per-provider SDK write for the one `lineage.json` PUT if this residual is ever promoted. The same-day GCS probe confirmed GCS's native equivalent (`ifGenerationMatch=<current generation>` → 200, stale generation → 412), so the closure primitive exists on both verified providers (per-provider: `storage.Conditions{GenerationMatch: N}`). Until then the guard shrinks the silently-vulnerable window from the whole seconds-wide load→mutate→Put span (the pre-guard exposure) to that millisecond gap, rather than eliminating it. Accepted for v1 — added at review 2026-07-15; upstream ground truth updated 2026-07-16 (probe log: sluice-testing `workspace/aws/s3-cas-probe.md`).
...
- **RETIRED (2026-07-16): every cloud backend row is now real-cloud-verified.** Azure Blob was the last derived row, closed the same day as GCS: fresh-key `If-None-Match: *` PUT → 201, occupied-key → **409 `BlobAlreadyExists`** (Azure's normal loser shape — NOT S3/GCS's 412; gocloud's azureblob driver explicitly maps it → `FailedPrecondition`, azureblob.go:665-670), exactly-one-winner under 2-way and 8-way concurrent conditional PUTs with zero non-409 loser shapes; through sluice's own `BlobStore` the interleaved-writer scenario produced the coded `SLUICE-E-BACKUP-CHAIN-CONFLICT` refusal with zero catalog bytes written by the loser. gocloud v0.46.0 sends the precondition (azureblob.go:945). Theoretical residual on Azure: unmapped 503 throttling → the degrade-WARN path (safe direction, not reproduced). Probe log: sluice-testing `workspace/azure/blob-cas-probe.md`.
