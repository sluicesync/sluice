...
- **Persist an explicit stream → publication-scope binding in control state and check that.** This is the airtight version — it would catch a conflicting stream whose slot happens to be momentarily inactive. Deferred as a follow-up (see residual risk); the slot-activity proxy covers the realistic operator sequence at a fraction of the complexity.
...
**Residual risk.** The slot-activity check is a proxy, not a registry. A conflicting stream whose slot is momentarily inactive — stopped mid-migration, or reaped and not yet re-established — would not be detected, and the rescope would proceed. The refusal therefore reduces the silent-loss window sharply but does not close it to zero; the explicit stream→scope binding in control state (Alternatives) is the follow-up that would. Worth filing as a roadmap item alongside this ADR rather than leaving implicit.
