...
2. **Reconcile after the bulk-copy.** In `runBulkCopyPhases`, **after the copy+index block (BOTH the IncrementalIndexBuilder overlapped branch AND the non-IIB fallback branch) and before the constraints phase**, drain the tracker. For each touched table: `TRUNCATE` the target table (via `ir.TableTruncator`, which the MySQL `RowWriter` implements) then re-copy that **one** table **serially** from the source via `copyTable` (within-table parallelism = 1 — the single-stream pace that never outruns replication). Placing it after the block rather than inside a single branch is load-bearing: **every production target (PG and MySQL) implements `ir.IncrementalIndexBuilder` and therefore takes the overlapped branch**, so scoping the reconcile to the non-IIB fallback `else` made it dead code — it never ran for a real MySQL target (the v0.99.160 miss, caught by live PlanetScale re-validation: a real run hit 112 reparent-retries but logged 0 reconciliation rounds). Running before constraints keeps the `TRUNCATE` free of FK dependencies; the touched table's already-built secondary indexes (built during the overlapped copy) are maintained by the re-`INSERT`. The serial redo reuses the top-level writer, which carries the observer, so a redo that itself reparents re-marks for another round.
...
- Engine scope: **MySQL first.** The touch-observer is wired engine-neutrally, but only the MySQL `RowWriter` implements `ir.ReparentObserverSetter` today (the confirmed-affected PlanetScale-MySQL path); on PG it is a no-op, so although the reconcile call runs on both targets' (overlapped) path, PG never has a touched table and the reconcile drains empty (zero cost). The machinery is engine-neutral (all in the pipeline package via the optional `ir` interfaces) so PG can adopt the observer if a PG reparent-loss is ever demonstrated.

## Correction (v0.99.161)

The first cut (v0.99.160) placed the reconcile call inside the non-`IncrementalIndexBuilder` fallback `else` branch of `runBulkCopyPhases`, on the mistaken premise (from a stale interface comment) that MySQL takes that branch. In fact **both PG and MySQL implement `ir.IncrementalIndexBuilder`** and take the overlapped copy+index branch, so the reconcile was dead code and never ran for a real MySQL target — the fix was inert. Live PlanetScale re-validation caught it: a real migrate into a reparenting non-Metal PlanetScale MySQL hit 112 reparent-retries and 81 grow-gate trips yet logged **0 reconciliation rounds** (the grow-gate alone happened to preserve every row that run, so no data was lost — but a run that dropped acked rows would not have been recovered). v0.99.161 moves the reconcile to **after the copy+index block, covering both branches**, and the unit test now uses an `IncrementalIndexBuilder` fake so it exercises the overlapped branch that production engines actually take (the original test used a plain non-IIB fake and so validated only the dead branch). This is the "validate end-to-end before building more" tenet doing its job.
