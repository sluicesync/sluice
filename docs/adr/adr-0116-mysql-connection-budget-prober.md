...
### Correction (v0.99.122): tier cap is PlanetScale-flavor-only

The first cut (v0.99.121) folded the Part-B buffer-pool tier cap into the budget for **every** MySQL flavor. That was wrong: the buckets proxy PlanetScale's *fixed plan tiers*, but a vanilla MySQL (`mysql`) or a **self-hosted Vitess (`vitess`)** sizes its buffer pool to its own hardware, so `@@innodb_buffer_pool_size` is not a CPU/tier signal there. The effect was that a common 128 MB-default buffer pool capped parallelism to 2, collapsing parallel `backup`/`restore`/`migrate` on non-PlanetScale MySQL (caught by the `-race` integration shard's `TestBackupParallel_MySQL*` / `TestRestoreParallel_PGToMySQL` assertions — not the unit tests). v0.99.122 gates the tier cap on `== FlavorPlanetScale` **specifically** (not `usesVStream()`, so self-hosted Vitess is excluded); the connection-slot budget (Part A) still applies to all flavors. Pinned by `TestComputeConnectionBudget_TierCapPlanetScaleOnly` (cap binds for PlanetScale, full budget preserved for vanilla/Vitess). Lesson: a parallelism-resolver change must be validated against the integration shards that assert parallelism *engages*, not just the unit math.

### Correction (v0.111.1, roadmap item 123): a sub-floor reading is UNREADABLE, not a tiny tier

A field report showed the Part-B premise failing: a branch the operator believed was PS-160 returned **32 MiB**, which bucketed to the tightest cap of 2 — a silent 4× throughput loss on a large instance. `bufferPoolParallelismCap` now treats any reading below PS-10's measured 128 MiB floor as **not a tier answer** and applies no cap at all, with a WARN naming the value and the floor. The direction is deliberate: an absent cap falls back to the connection budget (a real slot bound), whereas a wrong tight cap throttles the copy and looks like sluice being slow. A real PS-10 returns EXACTLY the floor and the check is strictly-below, so the smallest legitimate tier is still capped.

### Correction (roadmap item 144): the tier signal failed OPEN on the write fan-out

Item 123's fix was right for the connection budget and left a hole one layer over. "Not a tier reading" meant **no bound of any kind**, so the one target class whose capacity sluice cannot establish was the class it drove at the **widest** ADR-0097 write fan-out. Measured on real PlanetScale (2026-08-05, reproduced 2026-08-06 on a freshly created PS-10): a **production** branch reports exactly the 128 MiB floor and a **dev** branch on the same database reports 32 MiB, below every tier the table records. So a production branch got the tier cap and a dev branch — the weaker infrastructure — got nothing.

Two things were wrong, and the second is what made the first invisible. (1) `ir.ConnectionBudget` carried no fan-out axis, so "how many connection slots are free" was the only question the report could answer; on a vtgate-fronted target the two diverge sharply, because slots are abundant (`@@max_connections` measured 210–250) and write capacity is not. (2) The sync cold-start's own connection-budget preflight **ran the probe, computed the tier verdict, logged it as `capping bulk parallelism … effective=2`, and discarded the result** — with a comment saying so, and a second comment asserting the step was "a no-op on engines without a connection-slot model (MySQL)", which stopped being true in v0.100.0 when MySQL gained `ProbeTargetConnectionBudget`. That stale premise is why the discard read as harmless for three minor versions.

**Decision.** The report gains `CopyFanoutCeiling`, a SEPARATE axis from `CopyBudget`, declared by the target engine. MySQL sets it on the PlanetScale flavor **only when the probe's reading is below the floor** — i.e. only when the tier is unknown — and derives that condition from `bufferPoolParallelismCap` itself, so the two decisions cannot drift apart (`TestCopyFanoutCeiling_SharesOnePredicateWithTheTierCap` is what fails if they do). An on-scale reading, **including a real production PS-10 at exactly 128 MiB**, declares no ceiling and is byte-identical to before; so is every other engine, every other flavor, and every degraded probe. The ceiling is never folded into `CopyBudget`, and that is what keeps this from re-opening item 123: a sub-floor reading still leaves the copy budget at the connection-derived value, so migrate's table × within-table product is untouched.

**Why the known-tier path is deliberately left alone** — measured rather than assumed, on a fresh PS-10 with 1.70M rows / 787 MB across four tables so the native-concurrent path engages. On the **production** branch degree 4 ran 80.9 s / 87.0 s and degree 2 ran 89.0 s / 91.2 s: capping a branch whose tier we CAN read would have cost about 7% for no measured benefit. On the **dev** branch degree 4 ran 95.0 s / 115.8 s and degree 2 ran 101.2 s / 103.2 s — the same mean within noise, with the run-to-run spread collapsing from ~21 s to ~2 s, so the ceiling is free there even in the healthy case. And it is worth 2.5× in the case it exists for (item 144's A/B/A: 579 s with a connection-drop storm at degree 4 versus 234.8 s with zero drops at degree 2, storm restored on re-raising). Note what that pair of results means: **fan-out is not intrinsically harmful** — it is harmful once the target starts shedding connections — which is exactly why the ceiling is scoped to the one class whose capacity sluice cannot establish rather than applied across the tier cap's whole range.

**Named residual.** The ceiling's VALUE (2) is calibrated on measurement, not derived. Every sub-floor reading observed so far came from a PlanetScale dev branch; if some other target class ever reports sub-floor, the ceiling is conservative for it on no evidence. That error costs a narrower copy — slower at worst, and LOUD (the budget WARN names the reading, the floor and the ceiling; the cold-start logs the degree it resolved and why). The opposite error is the measured drop storm. Also unchanged: W, the cross-table reader count, still sits outside the preflight (ADR-0101 §5), so the ceiling bounds D and not the W × D product.
