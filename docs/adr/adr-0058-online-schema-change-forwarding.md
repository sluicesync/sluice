...
**Its synthetic Updates now follow the boundary's snapshot instead of preceding it**, because the applier drops its cached view of the table on the snapshot, and a value ahead of it was encoded against the pre-ALTER table (measured: a refused `integer[]` on Postgres, a `BIT(8)` written as its 8-character text on MySQL). Every change after the boundary still follows the backfill, which is what keeps a concurrently updated row at its later value. Two further defects in the loop were fixed: a page whose read failed part-way was taken for the end of the table (the readers report that failure only through `Err()` after closing the page); and on VStream every table was refused as keyless, because the FIELD event carries no primary key — it is now read from the source catalog.

**§1c's "re-running the backfill after a crash re-issues the same UPDATEs" and §2c's "the backfill resumes from the last successfully-applied PK cursor" were never true.** No cursor is persisted, and a reopened stream — a new process, or an apply retry in the same Run — never sees the boundary as an ADD COLUMN again (the intercept's cache is seeded once, at cold start). Measured on Postgres 16: a stream stopped 12 rows into a 60,000-row backfill restarted cleanly with 59,989 pre-existing rows NULL. What stands in for the resume is a ledger (`schema_forward_backfill_ledger.go`): a backfill owed by a forwarded boundary must be shown to have reached the target — finished, and the target's persisted position at or past the first change after it — or the attempt ends with the terminal `ADD-COLUMN-BACKFILL-INCOMPLETE`, recorded through the `UNFORWARDED-SCHEMA-CHANGE` store so every restart refuses until acknowledged. A process killed without running its exit path settles nothing, so the owed backfill is also recorded durably on the target before the ALTER. This needs no new column: the ledger writes the same `ADD-COLUMN-BACKFILL-INCOMPLETE` text into `sluice_cdc_state.unforwarded_refusal`, the record the startup door already reads. It does so before the single-stream `AlterAddColumn` and before the Shape A lease holder's apply; a Shape A stream that only observes a peer's ALTER writes it at plan time. The ledger removes the record with a compare-and-clear once every backfill the record covers is proven durable, checked every 10 s while the stream runs and again at attempt end, so a genuine refusal that replaced the record is never cleared. A record that cannot be written refuses the boundary. A hard kill inside a backfill therefore restarts refusing, pinned on Postgres by `TestStreamer_AddColumnBackfill_HardKillRefusesOnRestart_PostgresToPostgres`. Residuals: the false-refusal window after a hard kill (the durability interval plus checkpoint lag, a crash between the write and the ALTER, or a failed ALTER); and on Shape A, a kill after a peer's ALTER has filled this shard's rows but before this stream sees its own boundary. §2c's actual resume, a persisted PK cursor, is still not built. The record makes an interruption loud; it does not continue the backfill.

§2a is unchanged: a volatile DEFAULT the source still declares when the boundary is probed is refused, not forwarded-then-backfilled. The backfill would correct the pre-existing rows, but the ALTER itself still has to be expressible on the target (`nextval` of a sequence the target lacks is not), and a refused ALTER is loud where a backfill interrupted by a hard crash would leave the target's own clock in those rows looking plausible.
