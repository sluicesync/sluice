...
- **Maximum attempts: 8.** After 8 consecutive failures of the same `(batch_id, source_position)` pair the pipeline gives up loudly with a terminal error that preserves the most recent transient. The deliberate backoff is 12.7s (100ms doubling to 6.4s; the eighth failure exhausts the budget without sleeping) — enough to absorb a tx-killer or a momentary blip, NOT enough to ride out a vtgate restart or a Patroni failover, and short enough that a *real* stuck batch doesn't hide for hours.
...
- **Retry budget is bounded and observable.** A stuck batch surfaces after 12.7s of deliberate backoff with defaults — plus however long each failed attempt takes to return, which for a half-open target can reach `--apply-exec-timeout` (60s) apiece — not silently forever.
...
- **Position-and-data atomicity (ADR-0007) is preserved** in its amended directional form (2026-08-12): batched flushes write position with the data in one tx; the per-change path defers mid-source-transaction positions to the TxCommit. Either way a persisted position never covers un-applied data, so a retry can only re-apply idempotently — never skip.
