# ADR-0163: `backup export-as-parquet` — the analytics exit surface
...
- **Row groups align 1:1 with source chunks** — the operator-visible chunk concept survives into the file, and each file's footer metadata records the full source-chunk list (`sluice:source_chunks`: file/sha256/row_count) so operators can cross-reference `backup verify`. Further provenance keys: `sluice:backup_id`, `sluice:source_engine`, `sluice:backup_created_at`, `sluice:schema`, `sluice:table`; enum/set value universes in `sluice:enum_values` / `sluice:set_values`; a GeoParquet `geo` block for WKB geometry columns.
