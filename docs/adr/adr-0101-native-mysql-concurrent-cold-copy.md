...
**Implementation note (2026-07-04, perf-parity gap 3 — default flipped to auto).** The paragraph above shipped with a default of 1; the perf-parity survey (matrix gap 3: a MySQL user's first `sync` cold-start ran ~4× slower than the `migrate` they had just run against the same database) re-examined it and flipped the absent-knob default to `defaultNativeCopyTableParallelism = 4` (migrate's `defaultTableParallelism` twin), still clamped by the same resolver to `min(4, nTables, 32)`. The flip is confined to this native path because every property that made 1 "the cautious choice" turned out to be cost, not correctness: the N-snapshot is consistency-identical to serial (one FTWRL cut, one position, §3), the serial opener already attempts the same FTWRL on every cold start, FTWRL-denied sources keep the loud serial fallback (§4), and — decisively vs the VStream sibling — this path has no cross-process resume whose table→reader partition a changed default could silently re-derive (ADR-0111 cursors are in-memory only). ADR-0099's VStream default stays 1 for exactly that resume-stability reason (its §5 operator contract); `--copy-table-parallelism=1` / `copy_table_parallelism=1` is the serial opt-out. The default lives in the resolver chain (CLI override → DSN param → constant), never a struct zero value.
...
- **Gaplessness is by construction, not by argument.** All N readers are at the identical cut = the one recorded position. CDC resumes from that one position; every change committed after it is in the binlog tail, every change before it is in every reader's snapshot. There is no per-reader window to reconcile — the seam is a single point, so there is nothing to stitch and no "is set A ⊆ set B" obligation.
  ...
  **Correction (2026-08-07 — the construction is now CHECKED; until this date it was not).** "By construction" is a claim about ONE thing: that the N `START TRANSACTION`s and the position read all happen inside one held FTWRL. That was written down and nothing verified it. The package's AST capture-order roster (`TestSnapshotCaptureOrder_EveryUnlockedSiteCapturesBeforeTheSnapshot`) *discovered* `acquireConsistentSnapshot` and then **exempted** it, with this very sentence as the exemption's stated reason — so moving the position read below `UNLOCK TABLES` left the entire `internal/engines/mysql` unit suite GREEN (mutation-verified, mutant confirmed applied). Three gates now hold it, each doing something the others cannot: the roster **grades** the bracket instead of exempting on it (a listed site must show `FLUSH TABLES WITH READ LOCK` < capture < `UNLOCK TABLES` in source order); `TestConcurrentSnapshotCapturesPositionUnderTheLock` reads `mysql.general_log` and requires that order on **one thread_id**, because source order is not execution order and a position read on a different connection is not covered by this connection's lock; and `TestConcurrentSnapshotIsGaplessUnderChurn` runs four concurrent committers across the open and asserts all N readers pinned the identical cut and that every committed row is in the snapshot or the CDC tail and never in neither — with the test's own committed-id set as the independent expected value. The last one caught the §4 rejected design directly: with the FTWRL removed, two readers pinned cuts 40 and 38 rows apart.
