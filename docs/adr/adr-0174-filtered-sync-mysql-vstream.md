...
- **PAD SPACE / collation faithfulness (v0.99.278–282).** `evalengine.NullsafeCompare` is NO-PAD regardless of a collation's real `PAD_ATTRIBUTE`, but every legacy MySQL collation (`utf8mb4_general_ci`, `_bin`, `latin1_*`) is PAD SPACE — so the client comparator right-trims trailing spaces on PAD-SPACE collations. UCA collations (`_0900_as_cs`/`_cs`) fold canonical equivalence (NFC/NFD) and ignore UCA-ignorables, so only `_bin`/`binary` is genuinely byte-exact; the rest route through the Vitess comparator. MariaDB `*_nopad_*` is NO-PAD (the `nopad` NAME token is the version-robust signal — MariaDB's `PAD_ATTRIBUTE` catalog column is absent through the 11.x LTS line + 12.0 and was only added in 12.1, so the catalog can't be relied on). A non-UTF-8 charset or an unresolvable collation refuses loudly; `--where-strict-collation` refuses the fold path (byte-exact only).
