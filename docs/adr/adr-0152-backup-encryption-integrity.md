...
The residual unsigned manifest-edit boundary has three named shapes, in increasing subtlety: (1) **whole-backup rollback** (recoverable — an older-but-coherent state); (2) **partial change-chunk tail-truncation** — dropping the tail entries of an unsigned incremental's `ChangeChunks` list, whose survivors keep ordinals `0..k` so every GCM AAD still validates, leaving a short apply with an intact `EndPosition` that overstates the data and poisons a resumed CDC stream (the *unrecoverable* variant). The replay path (both offline `chain_restore` and the live-apply broker) now backstops (2) by asserting the applied change-chunk tail REACHES `EndPosition` and refusing with `SLUICE-E-BACKUP-INCOMPLETE` on a shortfall — a *signing-independent* net that converts the unrecoverable truncation into a loud refusal; a fully-coherent edit that also LOWERS `EndPosition` stays the recoverable rollback of (1). (3) **coherent two-sided position rewrite** — moving a link's `EndPosition` forward AND the next link's `StartPosition` to match (`ValidateBoundary` enforces only *relative* seam contiguity). Every chunk still applies (positions don't gate data), but the forged `EndPosition` is the CDC resume anchor, so a post-restore `--position-from-manifest` resume skips the gap; this is NOT caught by the tail-reach backstop (the tail does reach the forged `EndPosition`) and remains signed-only. All three fold into ADR-0154's signed canon (change-chunk ordinal + count + both positions), and `--require-signature` refuses an unsigned chain outright.
