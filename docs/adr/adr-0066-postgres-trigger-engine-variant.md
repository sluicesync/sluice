...
- **The capture-shape door graded the function's NAME and never its DEFINITION, so a `CREATE OR REPLACE` that records nothing passed every check (2026-08-31, SL-5).** §3's "one shared function, one trigger per table" is the whole capture mechanism, and the door added for F2 verified the *triggers* — presence, `tgenabled`, `tgtype`, and `proname` — while its own header called it the mirror of the sqlite-trigger door, which compares the installed CREATE statement against the statement the binary renders. So `CREATE OR REPLACE FUNCTION sluice_capture_change() … AS $$ BEGIN RETURN NULL; END $$` left every trigger correctly named and shaped, pointing at a function that captured nothing, and every subsequent DML on every replicated table was silently absent from the stream at exit 0. The same gap covered the *vintage* population, which is larger and needs no tampering: `CREATE OR REPLACE` is the only thing that installs a function fix, so an install set up before `SET bytea_output = hex` (audit B-1), `SET extra_float_digits = 3` (Bug 194), the SEC-1 `search_path` pin or the Bug 257 suppression keeps capturing through the old body — the exact corruption those pins exist to stop — and nothing said so. The door now reads `pg_proc.prosrc` **and `proconfig` and `prosecdef`**: the GUC pins live in proconfig, so a prosrc-only comparison (the shape the finding proposed) would have missed its own headline scenario. **Refuse vs WARN is decided by PROVENANCE, not by the body**, because the body alone cannot distinguish old from edited: setup records the installed definitions' digest (`capture_fn_digest`, schema_version 5), a definition that no longer matches what setup recorded REFUSES, and one that matches it but not this binary's render only WARNs (`STALE-CAPTURE-FUNCTION`) — the SEC-1 posture, so a binary upgrade never strands a running sync. Independent of provenance, a definition that no longer writes into the change log refuses at any vintage: no sluice version has ever rendered such a body (checked against the engine's first commit `9f220c47`), and that arm is what covers the entire pre-v5 population against the attack. **Residual, stated rather than implied:** on a pre-v5 install a subtle edit that still records lands on the WARN. Pinned by `TestGradeCaptureFunctionShapes_EveryFunctionEveryDriftShape` (row × three payload modes, truncate, `ddl_command_end`, `sql_drop` × four drift shapes × both provenance states), `TestGradeCaptureFunctionShapes_OldButUntamperedWarnsRatherThanRefuses`, and `TestCaptureFunctionBodyDoor` on a real PG — which executes the attack rather than simulating it, ground-truths that the gutted function really records nothing, and CHECKS the environmental premise the comparison rests on (prosrc byte-identical and proconfig in the door's normalized form, for all four functions). **One existing pin needed its fixture corrected, and the correction is the point:** `TestInsecureCaptureFunctionWarn_AtCDCOpen` (SEC-1) simulated a pre-fix install by hand-planting the old function body on a CURRENT install — which, once setup records provenance, is indistinguishable from tampering and now correctly refuses. A real pre-fix install carries no provenance at all, so the fixture now also presents what an older sluice's setup would have left behind (NULL digest at its own lower `schema_version`). The old fixture was testing a shape no upgrade can produce.
...
- **The §2 change-log indexes were removed, and the §3 TG_ARGV PK bake is now the implemented shape (N-16, 2026-07-09).** §2's sketch creates `sluice_change_log_id_idx ON (id)` — an exact duplicate of the BIGSERIAL PK's implicit index — and `sluice_change_log_table_idx ON (schema_name, table_name, id)`, which no shipped query ever read (poll, anchor, settle-clamp, prune, and stats all key on `id` and `txid` alone; re-verified against the full query inventory at fix time). Both were maintained on every captured source DML for zero read benefit, so setup no longer creates them and instead emits idempotent `DROP INDEX IF EXISTS` for both names, converging any pre-existing install on the next setup run (no schema_version bump — pre-production simplification). Separately, v1's capture function re-derived the PK column list from a `pg_constraint`/`pg_attribute` join per fired row (this ADR's §3 sketch — TG_ARGV[0] as a JSON array of PK column names — was deliberately deferred); the bake is now real: setup's preflight loads each table's PK columns (conkey order) and `CREATE TRIGGER ... EXECUTE FUNCTION sluice_capture_change('["col",...]')` carries them, composite PKs included. Staleness posture: the baked list no longer tracks a post-setup `ALTER TABLE` PK change live — on the event-trigger tier the ALTER's op='X' row already refuses the stream before any post-ALTER DML row is consumed (drained-model → setup re-run re-bakes), and for the polled-fingerprint tier / manual trigger surgery the function's projection guard refuses any write whose row image no longer matches the baked list, so a stale bake fails loudly rather than capturing rows keyed on the wrong columns. Pinned by `TestRenderSetupDDL_NoChangeLogIndexes` / `TestRenderSetupDDL_BakedPKArgs` / `TestRenderCaptureRowFunction_BakedPKList` (unit) and `TestSetup_ChangeLogIndexDiet` / `TestCapture_CompositePK_EndToEnd_AllModes` / `TestCapture_StalePKBake_RefusesLoudly` (integration).
- **The anchor's gap-freedom claim in §2's sketch is narrower than written — CLOSED at the handoff by a settle-wait + clamp (2026-07-08; the hole itself was live-confirmed the same day, epoch-independent).** A still-uncommitted transaction's change-log row is invisible to the anchor query (MVCC), so the `MIN(id) − 1` arm cannot anchor below an in-flight txn's already-allocated id when that id is lower than every *visible* not-yet-settled row (repro: A inserts change-log id=1 and stays open, B inserts id=2 and commits → anchor = 1, but gap-free needs 0 — A's change lands in neither the bulk copy nor the `id > 1` tail; the exact loss reproduced through the real Streamer cold start). `OpenSnapshotStream` now, after the anchor query on the pinned snapshot conn: exports the snapshot's visibility horizon (`pg_current_snapshot()::text`), assigns a txid **upper bound** on the pool (`pg_current_xact_id()` — a freshly ASSIGNED xid, the same own-xid bound logical-decoding slot creation waits on), waits until every pre-bound transaction settles (fresh snapshot per poll; bounded at 5 min with a loud refusal naming the stuck txids + the pg_stat_activity remedy — a wedged writer must not hang cold-start forever), then clamps the anchor below `MIN(id)` of change-log rows not `pg_visible_in_snapshot` in the copy's horizon. The gap-freedom invariant — *anchor ≤ min{id : allocating txn not visible in the copy snapshot} − 1* — and the case split (pre-bound txns settled+clamped; post-bound txns' ids are post-snapshot-allocated, hence above any anchor the queries can return, BIGSERIAL CACHE 1 monotonicity) live on `waitForPreSnapshotTxnsToSettle`. **Do NOT rebuild this on `pg_snapshot_xip` alone** (this ADR's original follow-up sketch): a snapshot's `xmax` is latestCompletedXid+1, so the only running transaction can sit AT/ABOVE xmax and appears in NO xip list while already holding an invisible low change-log id — live-probed (snapshot `744:744:` with backend_xid=744 idle-in-transaction). Snapshot `xmin` (which PG keeps at the oldest still-RUNNING txid) unioned with xip, bounded below the assigned upper bound, is the sound in-flight enumeration. Pinned by `TestSnapshotStream_InFlightTxnAnchor_NoGap` (engine), `TestPGTriggerStreamer_InFlightTxnSpansColdStart_NoLoss` (through the real Streamer), `TestSettleWait_TimeoutRefusesLoudly` + `TestSettleWait_QuiescentSourceFirstPollExits` (refusal + no-overlap latency). A narrower steady-state cousin remains open and is NOT changed by this fix: xid-assignment order can invert change-log-id order across multi-statement txns (inherent to the Debezium-style design; the §2 safety-lag hold-back bounds it). **That last sentence was wrong, and the steady-state cousin was a CRITICAL silent-loss defect — see the next note.**

- **The §2/§6 steady-state hold-back is a CONTIGUOUS-PREFIX rule, not a per-row predicate; the per-row form silently dropped committed changes (2026-08-01).** The claim closing the note above — that the §2 safety-lag hold-back "bounds" the xid-order-vs-id-order inversion — did not hold, and the shipped poll lost data under ordinary concurrent OLTP. The poll held back per row (`txid < pg_snapshot_xmin(...)`) but advanced its watermark to `max(id)` of the rows it actually fetched: two different correctness arguments, agreeing only when id order matches xid order. `T_A` (xid 100) writes change-log id 1, `T_B` (xid 101) writes id 2 and stays open, `T_A` writes id 3 and commits: a poll sees ids 1 and 3 (both `txid < xmin = 101`), emits both, and advances past the invisible id 2 — which is then never emitted, on any poll or any restart, with exit 0 and a green `sync status`. Two transactions and one multi-statement write are enough; no unreplicated table and no pre-assigned xid are required. The poll now fetches an id-ordered batch WINDOW and consumes only the CONTIGUOUS run out of it, stopping at the first missing id, because "missing from the snapshot" is exactly "allocated by a transaction that has not committed". Because contiguity alone would wedge the stream on the first rolled-back write to a captured table, a hole is skipped only once PROVEN permanent, by the same freshly-assigned-xid bound + settle probe the cold-start handoff uses — observe, then assign the bound, then probe, then re-observe; that order IS the proof. The anchor and the poll now render one shared ceiling expression (`settledCeilingSQL`) so they cannot drift back into two rules, and the previously-implicit **BIGSERIAL `CACHE 1`** premise (monotone global id allocation, which both the contiguity rule and the abort proof rest on) is preflighted at every CDC open and refused loudly. Pinned by `TestCDCReader_GapFreedom_OverlapMatrix` (every xid-order × id-order × commit-order cell against real overlapping sessions, asserting completeness — its single-orientation predecessor `TestCDCReader_SafetyLag_OverlappingTxns` could not expose this and never did), `TestCDCReader_GapFreedom_InterleavedMultiStatement`, `TestCDCReader_GapFreedom_WaitsOnAnInFlightHole` / `..._SkipsAnAbortedHole` (both directions of the hole proof, mutation-verified), `TestCDCReader_RefusesSequenceConfigThatCanReissueIDs` (see the amendment below — this cited `TestCDCReader_RefusesNonUnitSequenceCache` until the preflight was widened), and `TestPollAndAnchorShareTheSettledCeiling`. The sibling trigger engines (`sqlite-trigger`, incl. the D1 backend) are EXEMPT and stay on the plain `id > lastSeen` scan: SQLite holds the write lock from a transaction's first write to its commit, so a committed higher id above an uncommitted lower one is unconstructible there — recorded at that pump's doc comment together with what would invalidate it.
- **The sequence preflight covered ONE of the four settings that break monotone allocation, and the sharpest of the other three was asserted rather than checked (2026-08-07).** The note above says the `CACHE 1` premise "is preflighted at every CDC open and refused loudly", which was true and narrower than it read: `verifyChangeLogSequenceCache` graded `seqcache` alone, while `readChangeLogAnchor`'s negative-anchor clamp carried the *assertion* "MIN(id) − 1 can only go negative if the lowest in-flight id is 0, which BIGSERIAL never allocates (it starts at 1)". Ground-truthed on PG 16, that assertion is false of a sequence anyone can ALTER: `MINVALUE 0 RESTART WITH 0` allocates change-log id 0 and `MINVALUE -100 RESTART WITH -100` allocates negative ids, both of which sit at or below the watermark floor (`readChangeLogMaxID` returns 0 for an empty log, and that clamp forces a negative anchor to 0), so the change is never emitted on any poll or restart. A negative `INCREMENT` and `CYCLE` reach the same end by different routes. The preflight is now `verifyChangeLogSequence` and grades all four — `seqcache = 1`, `seqmin >= 1`, `seqincrement >= 1`, `NOT seqcycle` — at both CDC doors. **Stated scope, so the name cannot be read as broader than the truth:** it grades CONFIGURATION at open, and a mid-stream `ALTER SEQUENCE … RESTART WITH <low>` re-issues low ids under a perfectly legal configuration and is outside it — the same limit the CACHE check always had. Pinned by `TestCDCReader_RefusesSequenceConfigThatCanReissueIDs` (one cell per setting, both doors, plus a restore leg and an untouched-sequence positive control) and by the defect proof `TestChangeLogAnchor_MinValueZeroStrandsTheChange`, which binds the two halves that were each plausible alone: PG really allocates id 0, and sluice's poll really skips it. **The `sqlite-trigger` sibling stays exempt from THIS check** (there is no `pg_sequence` row to read; its id is `INTEGER PRIMARY KEY AUTOINCREMENT`) — but note that `sqlite_sequence` is an ordinary writable table, so the equivalent tamper is constructible there and is NOT preflighted. Filed, not fixed here.
