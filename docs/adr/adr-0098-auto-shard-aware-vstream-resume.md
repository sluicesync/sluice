...
- **New pins.** Unit: `resolveResumeAutoShard` (single in-scope cursor → table; multi-table / out-of-scope / empty → loud refuse); the resume routing engages the auto-shard pump and opens the in-progress table SEEDED from the cursor while re-copying the prefix and copying the tail fresh (incl. the in-progress-table-at-index-0 edge the constructor's pre-opened table[0] complicates); the stitched handoff position is the per-shard set-min. Integration (`vstream`): cold-start an auto-shard copy of a multi-table keyspace where Σ(tables) far exceeds a tiny `--max-buffer-bytes`, INTERRUPT mid-copy after ≥1 table completes while another is in flight, then `--resume` under the SAME tiny cap — assert **NO ADR-0071 buffer-cap error on any table** (the exact crash-loop), the in-progress table resumes strictly past its cursor (no row-0 restart) and delivers its full tail (no gap/dup at the seam), the re-copied/fresh tables deliver in full, and the stitched handoff position is non-empty for a clean CDC tail. The seam zero-loss is asserted at the row-identity level on the snapshot stream (min-id > cursor, exact tail count, full per-table counts); the apply-side idempotent absorption of the re-copied prefix + checkpoint-lag overlap is ground-truthed by the existing no-PK process-restart resume test (`TestVStream_CopyResume_NoPKTable_CheckpointLagReSendUpserts`, real MySQL applier into a pre-populated target → final `COUNT(*) == source`) and the cross-engine pipeline suite. This is the regression pin for the exact crash-loop.
