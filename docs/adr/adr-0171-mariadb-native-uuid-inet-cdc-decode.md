...
- **Residual risk (stated explicitly) — now substantially closed (roadmap item 73 matrix expansion, 2026-07-17):** the decode was originally ground-truthed against `mariadb:11.4` and `mariadb:10.11`. If a future MariaDB line changed the binlog storage layout for these types (byte order or the length-prefix framing), the decode would diverge silently, and the over-width refusal catches a framing change that produces MORE bytes than the width but NOT one that merely reorders within the width — the reorder guard is the empirical pin (the live oracle), not a runtime check. **That matrix has since been expanded**: the byte-layout was re-ground-truthed and confirmed **byte-identical on `mariadb:11.8`, `mariadb:12.3`, and the `13.1` preview** (uuid canonical big-endian, and the four netip-divergent inet6 shapes `::1.2.3.4` / `::0.1.0.0` / `::100` / `::ffff` all render exactly as `mariadbInet6Text` targets). The required engines-mysql shard now runs the value-fidelity pin (`TestMariaDB_CDCReader_NativeUUIDInet_Decode`) across the full LTS spread **10.11 / 11.4 / 11.8 / 12.3**, and a non-required continue-on-error leg (`TestMariaDB_Preview_NativeUUIDInet_Decode`, `mariadbpreview` tag) extends it one line ahead of GA to `13.1`. A future line that reorders within the width will now trip a live pin, not corrupt silently. (Aside, same probe: `13.1` additionally accepts `SHOW BINARY LOG STATUS`, which `11.x`/`12.x` reject with 1064 — the `masterStatusSpellings` fallback already handles it, so no code change.)
