...
**Amended 2026-09-02 (audit 2026-09-01 SLM-2, VStream arm; v0.138.0):** every check in this ADR asks only "has the source thrown away anything the position still needs" — which a fresh, reset or rebuilt keyspace answers with an empty `gtid_purged`, a subset of everything, so a position captured on one cluster resumed against an unrelated one passed Part 4. Measured on the real cluster rig: vttablet's own uvstreamer does refuse a resume set that is not ⊆ its `gtid_executed` (`GTIDSet Mismatch`), but the refusal did not reliably reach sluice — vtgate marked the refusing tablet ignorable and blocked waiting for another, a `backup incremental` window deadline expired into a clean close, and the chain committed a link with an empty end position; the next link started from the unrelated cluster's *current* position, silent loss at exit 0. Three closures, all in the code this ADR describes: (1) `vstreamCDCReader.verifyVStreamPositionReachable` (`cdc_vstream.go`) now asks each shard `GTID_SUBSET(<resume>, @@global.gtid_executed)` **before** the Part-4 `gtid_purged` check, over the same shard-scoped probe connection; a resume set naming a source UUID the shard has never executed refuses with `ir.ErrPositionInvalid` and takes this ADR's Part-2 route (cold-start re-snapshot by default, a loud stop under `--no-auto-resnapshot`). A shard where every UUID is present but sequence numbers are lower is replica lag, not lineage — the probe routes through `keyspace:shard@<tablettype>` and can land on a tablet behind the one the position was streamed from — so it logs at INFO and leaves tablet selection to vtgate's picker. A failed lineage probe logs the `UNVERIFIED-INSTANCE-IDENTITY` WARN and proceeds to the retention check (Part 4's degrade posture, now visible). (2) The reactive `GTIDSet Mismatch` (`isVStreamGTIDSetMismatchError`, `reader_errors.go`) is classified `ir.ErrPositionInvalid`, sibling of Part 1's purged matcher, so the case where vttablet's refusal *does* reach sluice takes the cold-start route instead of a terminal stop. (3) Engine-independent, found by the same measurement: a chain extension whose parent link recorded no end position (a quiet or DDL-only window) resumes from the nearest ancestor that recorded one rather than the legacy start-from-current branch. The MySQL binlog arm gained the same lineage door in the same release (`verifyGTIDLineageContinuity`; ADR-0022 amendment), and MariaDB an anchor-based one (ADR-0170 amendment).
