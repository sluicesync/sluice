...
- **Status:** Accepted — experiment validated 2026-08-18; MySQL-source implementation shipped v0.129.0 (`migrate` + `sync` cold-start index phase); the Postgres source-size estimator (`ir.TableByteSizeEstimator` via `pg_table_size`) shipped v0.131.3 — see the source-engine enumeration in the Amendment (SQLite/D1 filed, not implemented). The PlanetScale leg ran on a real PlanetScale branch (2026-08-18): the wire behaviour is confirmed, but it exposed a size-gate bug (the split sized off the freshly-copied *target's* stale `information_schema` stats, so it never engaged on PlanetScale). Fixed by sizing off the SOURCE estimate — see the Amendment below.
- **Date:** 2026-08-18
- **Related:** [ADR-0080](adr-0080-mysql-index-build-overlap.md) (the **combined-`ALTER`** this conditionally splits — its single-table-scan win is why the default stays combined); [ADR-0148](adr-0148-planetscale-deploy-request-index-build.md) (the deploy-request index build, the safe-migrations-ON escape this complements); [ADR-0182](adr-0182-planetscale-query-timeout-raise.md) (raising the ~900 s wall to 3600 s — the same errno-3024 wall, a different lever); roadmap item 109 / `SLUICE-E-CONSTRAINT-STATEMENT-TIME-LIMIT` (the FK half of the wall family).
...
**Source-engine enumeration (batch-C content sweep, 2026-08-23 — this cell was left implied above, which is exactly the sibling shape the working agreements forbid).** `ir.TableByteSizeEstimator` is per-SOURCE-engine, and the split is inert for any source that does not implement it — the same "inert on the platform it exists for" defect one axis over. Status per live source: **MySQL** — implemented v0.129.0 (source `information_schema` `DATA_LENGTH`); **Postgres** — implemented in the batch-C sweep (`pg_table_size`, the closest `DATA_LENGTH` analogue; pinned-reader posture mirrors `CountRows`), closing the PG→PlanetScale large-table cell that still walled; **SQLite/D1** — NOT implemented, filed: a per-table byte size needs the `dbstat` virtual table (compile-time optional in some builds) or a page-count heuristic, and D1's HTTP surface has no per-table size API — a >8 GiB SQLite→PlanetScale table keeps the combined `ALTER` and may wall (the pre-existing behavior, now stated rather than implied); **mydumper/flatfile** — archives, "unknown" by design (same follow-up shape as restore).
