...
**Real-cloud validation deferred (N-9):** both are pinned against faithful in-process fakes that return each provider's NATIVE wire format (GCP CRC32C fields + DER; Azure raw r‖s + JWK), so the conversions are genuinely exercised — there is no first-party local emulator for GCP KMS / Azure Key Vault signing, so a live pin is a flagged follow-up alongside the AWS `kmsverify` localstack leg.
