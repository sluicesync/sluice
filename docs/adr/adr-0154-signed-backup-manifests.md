...
**Deferred to Phase 3b (flagged, not silently dropped):** GCP KMS (incl. its Ed25519 signing) + Azure Key Vault signer adapters — each normalizing its own Sign request shape, signature encoding (Azure returns raw r‖s, not DER — a real divergence the adapter must handle), and public-key export (GCP PEM, Azure JWK) into the `crypto.KMSSigner` seam; unit-fake-pinned, real-cloud validation deferred (N-9 pattern). The pure verifier already covers ecdsa/rsa/ed25519, so 3b is signer-adapter-only.
