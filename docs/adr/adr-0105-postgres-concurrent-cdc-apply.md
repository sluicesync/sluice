...
**Proposed (2026-06-20).** Brings the [ADR-0104](adr-0104-mysql-pipelined-cdc-apply.md) concurrent key-hash CDC apply to the **Postgres** `ChangeApplier`, closing the cross-region apply-throughput gap that ADR-0104 closed for MySQL targets but left open for Postgres. Realizes roadmap [item 26](../dev/roadmap.md). The MySQL path is GA and live-load-validated; this ADR's central engineering risk is therefore **extracting the engine-neutral lane+frontier core without perturbing that GA path** — handled by the pin-first discipline below. **Concurrency chunk** → the `-race` integration gate must pass on CI before any tag.
