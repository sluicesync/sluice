...
- The **key-hash lane router** (`hash(qualified table, ordered PK values) mod W`) gives the same-key-closed guarantee — all changes to one row land on one lane, applied in source order — so the dependent-row hazard (INSERT then DELETE/UPDATE of the same key racing on two transactions) is structurally impossible. This is pure hashing over `ir.Change`; nothing MySQL-specific. **CORRECTED — see [Addendum 3](#addendum-3--the-route-is-not-always-the-primary-key-roadmap-item-131-2026-08-05): "structurally impossible" holds for ONE row and is FALSE across two.** Two changes with different primary keys that collide on a table's secondary UNIQUE index are dependent on each other, and PK-hashing puts them on different lanes with unconstrained commit order. The router now takes the per-key hash only where a catalog probe has PROVEN the table carries no non-PK uniqueness constraint (`RouteScopeKey`); everything else, including a failed probe, routes the whole table to one lane (`RouteScopeTable`, the zero value).
...
The `sync from-backup` broker replays each incremental's change chunks through the engine applier's `ApplyBatch`, identical to the live streamer. Until this addendum it opened that applier and never wired the lane count, so every incremental replayed through the single-stream ADR-0092 pipelined path — RTT-bound on a cross-region link, the same wedge this ADR lifts but on the broker-replay path. The broker now resolves `--apply-concurrency` (`SyncFromBackup.ApplyConcurrency`, the `sync from-backup run --apply-concurrency W` flag) through the same `applyApplyConcurrency` helper the streamer uses, so incremental replay fans across W in-order PK-hash lanes. The exactly-once contract is inherited verbatim: every change in an incremental carries the same broker chain-position token, so the lanes persist the identical position the serial path did — the frontier still only checkpoints at a fully-durable boundary, and the broker's idempotent re-replay-from-parent recovery is unchanged. The shared replay resolver (`resolveReplayApplyConcurrency`) does **not** run a connection-budget probe (these paths open a single applier up front; per-lane backpressure handles a tight target), so auto resolves to the fixed conservative default (4) rather than the streamer's probe-bounded count.

## Addendum 2 — chain-restore incremental replay reuses the same lane path (2026-06-24)

`ChainRestore` (used by standalone `sluice restore --from <chain>` of a multi-incremental chain, AND by the broker's `--reset-target-data` cold-start, which absorbs the chain before transitioning to polling) replays each incremental's change chunks through its OWN applier — a path distinct from both the live streamer and the broker's polling applier. It had the identical gap: it wired `applyMaxBufferBytes` / `applyTargetSchema` but never the lane count, so chain-incremental replay ran single-stream serial. Found live on the Track-C program (2026-06-24): a cross-region MySQL→PlanetScale-Postgres cold-start restored the full backup fine (parallel COPY, ADR-0112) then **wedged at ~1 change/s** applying an 8M-change incremental — the exact pre-fix broker-polling stall, on the chain-restore leg. `ChainRestore` now resolves `ApplyConcurrency` (threaded from the broker cold-start's own `ApplyConcurrency`, and from a new `restore --apply-concurrency W` flag on the standalone path) through the same `resolveReplayApplyConcurrency` + `applyApplyConcurrency` helpers, so chain-incremental replay fans across W lanes with the same inherited exactly-once contract. The full-restore COPY axis (`--table-parallelism` × `--bulk-parallelism`, ADR-0112) is unchanged and orthogonal — this closes only the incremental-apply leg. Fast-by-default: the unset value resolves to auto:4, so every chain restore (CLI, broker cold-start, tests) now gets concurrent incremental apply unless it opts out with `--apply-concurrency 1`; the existing chain-restore + broker integration suites exercise this under `-race` and assert convergence (exactly-once preserved).

## Addendum 3 — the route is not always the primary key (roadmap item 131, 2026-08-05)

The `LaneApplier` seam sketched above returned only the qualified table and the ordered PK values, and this ADR's Context asserts that hashing them makes the dependent-row hazard "structurally impossible". That claim is true for **one row** and false across two. Two changes with DIFFERENT primary keys that collide on a table's **secondary UNIQUE index** are dependent on each other, and PK-hashing puts them on different lanes with unconstrained commit order. Source order `DELETE(pk=1, uniq='x')` then `INSERT(pk=2, uniq='x')` — an ordinary natural-key reassignment on the wire — then loses a row on a MySQL-family target: the lane INSERT's `ON DUPLICATE KEY UPDATE` fires on the *uniq* index and mutates pk=1 in place (the PK is excluded from its SET list) instead of inserting pk=2, and the DELETE lane then removes pk=1. Zero rows where the source has one, both lanes green, checkpoint advanced past the loss. On PG the same routing raises `23505` and takes the run down — loud, and equally broken for the operator. Found by the 2026-08-05 audit (A-1); reproduced on real MySQL 8.0 and real PG before the fix.

The seam method is accordingly renamed `PKValuesForRouting` → **`RouteForChange`**, returning a `laneapply.Route{Qualified, PKVals, Scope}`. `RouteScope` is the new concept: `RouteScopeKey` hashes (table, PK) as before; `RouteScopeTable` hashes the table alone, putting every change on it onto one in-order lane. **The zero value is `RouteScopeTable`** — an engine may take the fast path only where it has PROVEN the target table carries no non-PK uniqueness constraint, and "the catalog probe failed" is not a proof. `Router.LaneForRoute` is the single point where a `Route` becomes a lane. Both engines probe once per table and cache the verdict alongside the PK/column caches, invalidating it on the same schema boundary (safe because the barrier drains every lane first — now pinned by `TestBarrier_DrainsAllLanesBeforeApply` rather than asserted).

**Cost, measured, so the trade is on the record.** Whole-table routing removes lane concurrency on exactly the tables that most want it. Measured on a local MySQL 8.0 target at W=4 (8,000 inserts into ONE unique-carrying table): 3.9s per-key vs 10.2/11.1s whole-table — **~2.6× slower** on that pathological cell. On a schema with several unique-carrying tables the effect reverses, because whole-table routing improves ADR-0140 coalescing locality (each lane sees fewer distinct tables): 4 tables × 500 rows measured 23.8s per-key vs 3.9s whole-table (~6× faster), and 8/32 tables were within ±10%. The unaddressed residual is unique-value-aware sequencing — barrier only on an *observed* value collision — which would restore per-key concurrency on unique-heavy hot tables at materially more code and risk. Not built; single-lane is the deliberate first cut.
