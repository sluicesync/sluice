...
The strict-correctness fix is reserved for a follow-up roadmap entry. Two viable paths:
- **Slot-pause approach**: temporarily stop the slot's apply ack so the slot retains WAL across the publication-add boundary, then re-decode the retained WAL with the updated publication. Lighter than Strategy B but requires plumbing into the streamer's apply loop.
- **Strategy B (dual-slot)** as originally sketched in `docs/dev/design-mid-stream-add-table.md`: run a second slot at the publication-add LSN that delivers events on the new table from that point forward; main slot continues without scope change. Heavier; the LSN race ADR-0030 wanted to avoid is the price.
