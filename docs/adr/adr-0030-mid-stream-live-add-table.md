...
**Hazard: in-flight-event loss during the publication-add window (best-effort, NOT pinned).** This is a real edge case discovered during v0.24.0's CI integration testing. The under-load test (`TestAddTable_LiveMode_PG_UnderLoad`) drives sustained INSERTs on the new table during the live add and observes that a small number of in-flight INSERTs (typically 1–3 out of ~20 over a sub-second window) don't land on the target. Cause: pgoutput evaluates publication membership per WAL record at decode time; events on the new table that the slot decoded-and-filtered BEFORE the publication-add commit took effect are gone — the slot's confirmed_flush_lsn advances past them as the slot processes other events, and pgoutput doesn't redeliver retroactively when the publication later includes the table.

Phase 2 documents this as a **best-effort property**: snapshot rows are delivered exactly-once-effectively (proven by the test); events at LSN > publication-add LSN are delivered exactly-once-effectively (proven by the post-add sentinel pin); events on the new table inserted DURING the brief publication-add window may not be delivered. Operators with high write rates on the new table at the moment of live-add should either:
- Use the drained add-table flow (`sluice sync stop --wait` then `sluice schema add-table` without `--no-drain`), which has zero-loss semantics by construction.
- Quiesce writes to the new table for the seconds-long window of the live add (operator-coordinated; tooling deferred).

**Status update (v0.27.0 cycle):** The strict-correctness fix's first picked path — **slot-pause** — was empirically falsified during Phase A verification. See ADR-0033 for the experiment and the verdict. Slot-pause's load-bearing claim ("hold the slot at LSN_pre, ALTER PUBLICATION, then restart pgoutput at LSN_pre to re-decode the retained WAL with the updated publication scope") does not hold: pgoutput's per-LSN historical catalog snapshot pins publication membership at decode time, so a second decode pass at the same LSN produces the same filtering result regardless of subsequent publication changes. The verification test (`internal/engines/postgres/slot_pause_verify_integration_test.go`) lands as a permanent regression artifact for any future revisit. **Update (2026-05-18, status audit): RESOLVED.** ADR-0033's recommended Path D ("characterise the actual v0.24.0 loss surface first") was taken by [ADR-0036](adr-0036-mid-stream-loss-surface-characterization.md): the real loss was an applier-side `errUnknownTable` silent drop, and the **strict-zero-loss fix SHIPPED in v0.32.0** (create target table before publication-add + idempotent bulk-copy; the under-load test is now a strict zero-loss assertion). The "What could go wrong" item 3 best-effort gap is **closed**; Strategy B (dual-slot) / source-quiesce were shed for this surface. The prior "no strict-zero-loss path has shipped" wording predated v0.32.0.
