...
## Amendment 2026-08-12 — the per-change path defers mid-transaction positions to the TxCommit (audit CDCPOS-2)

"Progress and applied data can never diverge" was written when every per-change apply carried its own position, and that atomicity turned out to be the SILENT-SKIP MECHANISM for rows inside a source transaction: in file/pos mode (which `gtid_mode=OFF`, MySQL's default, auto-selects) every row of one RowsEvent carries the event's END LogPos, so atomically persisting row K's position and crashing before row K+1 resumed PAST the un-applied tail. The per-change path on BOTH appliers now applies rows between the delivered TxBegin/TxCommit through the position-free seam and persists only the TxCommit's own (post-commit) position, in its own transaction. The invariant that survives — and the one this ADR actually needs — is directional: the position write runs strictly after every row of its transaction committed, so **a persisted position never covers un-applied data**; the reachable divergence is data-ahead-of-position, which a resume replays idempotently (ADR-0010). Changes outside any source transaction keep the original same-transaction write. Pinned per engine by `TestApply_PerChangePositionDeferredToTxCommit`, in the xid dimension by `TestPositionAndDataLandInOneTransaction` (whose serial cell now grades both shapes), and rostered by `TestPerChangeApplyMidTxPositionPins` (internal/appliershared).
