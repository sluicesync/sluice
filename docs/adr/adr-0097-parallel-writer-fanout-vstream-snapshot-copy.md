...
The ADR-0072 mid-COPY durable-write watermark does **not** compose with fan-out — it is **disabled** on the fan-out path (§3): the flat flushed-row count is not order-equivalent to the reader's enqueue-order frontier across N workers, so a mid-COPY breadcrumb could checkpoint past an un-flushed early row (silent-loss-on-resume). On a fanned-out table the only durability guarantee is the whole-table join at COPY_COMPLETED (no position advances until every worker is durably committed). The interrupted-cold-start *process-restart* resume (`OpenSnapshotStreamFromPosition`, a position carrying a `TablePKs` cursor) drives the [ADR-0098](adr-0098-auto-shard-aware-vstream-resume.md) auto-shard-aware resume — one single-table COPY at a time, exactly as the fresh path. Because a fan-out table never persisted a mid-COPY cursor (the checkpoint is disabled on it), such a table is simply re-copied from-beginning on resume (the idempotent upsert absorbs the overlap); a table whose cursor *was* persisted (the serial, non-fanned-out in-progress table) resumes from it. Either way exactly one table is in flight, so the resume is bounded-memory like the fresh copy — this superseded ADR-0095's single-stream resume v1 limitation, which (left unfixed) crash-looped a large multi-table resume on the ADR-0071 cap.
