...
Amended three times under Consequences (2026-09-02, 2026-09-07, 2026-09-15): every *different-lineage* refusal on the MySQL-family lanes is deliberately OUTSIDE this contract — terminal, carried as `ir.ErrPositionForeignLineage` (v0.148.2), never `ir.ErrPositionInvalid` — and since v0.153.2 the binlog GTID arm's empty and behind shapes take this fall-through only when the server's `@@server_uuid` is one the position names. The contract is scoped to *the same source has moved past the position*.
