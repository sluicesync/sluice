...
5. **Operator changes `--target-schema` mid-flight (warm resume).** Today's resume path reads from `sluice_cdc_state` (key: stream-id) — the persisted row is target-schema-agnostic. Phase 1 doesn't add a fingerprint for the *target* schema, only the *source* DSN. So a warm-resume with a different `--target-schema` replays CDC events into a different schema than the cold-start populated. Net effect: data corruption (rows split across two schemas, neither complete). Mitigation: the operator's intent here is unclear (intentional? typo?); v0.25.0 ships without explicit detection but documents the caveat in the `--target-schema` help text. A future refinement could add `target_schema TEXT` to `sluice_cdc_state` and refuse on mismatch (same shape as the source-DSN-fingerprint check); deferred until a real operator surfaces the gap.
