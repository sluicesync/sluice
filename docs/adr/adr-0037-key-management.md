...
- ~~**`--decrypt-verify` mode** for `sluice backup verify`.~~ **SHIPPED** (roadmap item 97 / Bug 215, 2026-07-28). It arrived under the existing `--encrypt` flags rather than a new one: supplying key material to `backup verify` now makes it authenticate-open every encrypted chunk, and it reports `decrypted=N` alongside `chunks=N` so the coverage is legible. **The implementation is NOT the "decrypt + re-hash plaintext" shape this bullet anticipated** — it is an AES-GCM *authenticated* open under the same CEK and AAD binding `restore` uses (`verifyChunkAuthenticated`, `internal/pipeline/backup/restore.go`). GCM authenticates the entire ciphertext, so the open is already the complete answer to "can restore read this"; re-hashing the plaintext afterwards would add nothing the tag has not already proven, and skipping the codec layer keeps the cost to one AES pass over bytes verify already holds for the SHA. The driving defect was also not the "extremely rare" ciphertext-corruption case named here: it was a chain sealed under a *different key* per rotation-born segment, which sha256 cannot see at all.
