...
**Resume must target PRIMARY when carrying a COPY cursor (the silent-loss bug the warm-resume path hid).** The standalone `vstreamCDCReader`'s `buildVStreamRequest` originally pinned the resume to `TabletType_REPLICA` unconditionally (to keep CDC tailing off the primary's hot path). But on a COPY-resume, vtgate's `uvstreamer.buildTablePlan` enumerates the still-copying tables via the **resolved tablet's schema engine** — and a REPLICA's schema engine can be **COLD** (replicas lag the schema-tracker on a quiet binlog). When it is, `buildTablePlan` finds no copy plan, the COPY-resume branch is **silently skipped**, and the stream degrades to plain CDC tailing: the `TablePKs` cursor is ignored and the un-copied rows are **NEVER replayed (silent loss)**. This is a PRODUCTION reach, not a test artifact — the orchestrator's mid-COPY crash recovery goes `runOnce` → `warmResume` (`streamer.go:2203`) → `openCDCReaderWithOptionalSlot` → `vstreamCDCReader.StreamChanges` → `buildVStreamRequest` (`cdc_vstream.go`), exactly this REPLICA path, so a real mid-COPY crash that warm-resumed would silently drop the un-copied tail. **PROVEN against real vttestserver:** flipping the resume `TabletType` REPLICA→PRIMARY made vtgate resume from the cursor (rows from `id=cursor+1`, ascending, no restart from row 0). The fix: `buildVStreamRequest` targets `TabletType_PRIMARY` **iff the resolved position carries a non-empty `TablePKs` cursor** (a COPY-resume), and keeps `TabletType_REPLICA` for pure CDC tailing (no cursor → off the primary's hot path). This mirrors the snapshot cold-start (`cdc_vstream_snapshot.go:115`) and Phase-C in-place `reconnectCopy` (`cdc_vstream_snapshot.go:536`), both of which already target PRIMARY — so the standalone reader's resume is now consistent with both the snapshot's PRIMARY-taken GTID anchor and a warm schema engine. Pinned by `TestBuildVStreamRequest_TabletTypeDependsOnCopyResumeCursor` (unit) + both `TestVStream_CopyResume_*` integration tests asserting `id > cursor` start + zero loss against real vtgate.
...
- **A COPY-resume must target `TabletType_PRIMARY`.** `buildVStreamRequest` selects the tablet type by inspecting the resolved position: a non-empty `TablePKs` cursor → `PRIMARY`; an empty cursor (pure CDC tailing) → `REPLICA`. vtgate's `buildTablePlan` enumerates the COPY tables via the resolved tablet's schema engine, which on a REPLICA can be cold — silently skipping the COPY-resume and degrading to plain CDC tailing (silent loss). Targeting PRIMARY guarantees a warm schema engine AND a read consistent with the PRIMARY-taken snapshot. See the load-bearing note above for the proof and production reach.
...
- **Resume reads from the PRIMARY (a deliberate, cursor-scoped exception to "tail off the REPLICA").** A COPY-resume now hits the primary's schema engine + read path, where a pure CDC tail stays on a REPLICA. The extra primary load is bounded to the resume's catch-up COPY window (it ends when COPY completes and the stream transitions to CDC tailing on REPLICA), and it is the only tablet whose schema engine is guaranteed warm and whose read is consistent with the PRIMARY-taken snapshot. Skipping it was a CRITICAL silent-loss class on the production warm-resume crash-recovery path — see the load-bearing note in Implementation notes.
