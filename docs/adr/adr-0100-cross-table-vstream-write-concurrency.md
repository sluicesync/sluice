...
**Why resume exactly-once holds without a resume-specific consumer code path:** the partition the consumer drives (`ConcurrentCopyGroups()`) is a deterministic pure function of `(sorted tables, K)` ([ADR-0099] §5), surfaced as the **same** `concurrentGroups` field for cold-start and resume — the engine computes it identically whether `start` is nil or carries a cursor (see `cdc_vstream_snapshot.go`: `concurrentGroups` is derived from `(tables, K)` *before* and independent of the resume seed). The only resume-specific logic is **producer-side** (seeding the in-progress table's per-table COPY past its cursor, `resolveResumeAutoShard` → `reopenForTableSeeded`), already covered by the ADR-0098/0099 resume tests. The W-goroutine **consumer** is resume-agnostic: it reads the same surfaced groups and drains each table's queue to completion regardless of where the producer started. So resume exactly-once is **structurally inherited** from ADR-0098/0099's deterministic partition + producer-side seeding — it is not a separate consumer invariant to prove. The integration pin composing ADR-0098 resume × ADR-0100 W>1 write concurrency (`TestVStream_AutoShardResume_ConcurrentWrite_*` in `internal/engines/mysql`) exercises the strongest feasible approximation of this seam end-to-end (see §Testing); a true mid-copy process-kill interrupt is approximated by resuming from a persisted mid-COPY cursor with K = 2, the coverage boundary noted in the test.
...
- **Integration (`vstream` / `vitesscluster` tag):** cold-copy a multi-table keyspace with W = K > 1 through the FULL pipeline (real target) → (a) assert MULTIPLE tables receive rows in an overlapping window (poll the target's per-table row counts and assert >1 table advancing concurrently, the PROCESSLIST analogue) OR wall-clock << sum-of-per-table-serial; (b) target `COUNT(*)` + content checksum == source per table (no gap/dup); (c) clean CDC handoff from the stitched position. Plus a **resume × concurrent-write** pin (`TestVStream_AutoShardResume_ConcurrentWrite_*`): cold-start a multi-table keyspace with `vstream_copy_table_parallelism=2`, capture a mid-COPY cursor while one table is in flight (≥1 already complete), then `OpenSnapshotStreamFromPosition` with K = 2 over the full table list and assert the resume engages the concurrent path (`ConcurrentCopyGroups()` returns ≥2 groups — the W-goroutine consumer drains two groups), every table lands exactly once (in-progress table resumes past its cursor, the rest re-copy/fresh — no gap/dup), and the stitched handoff position opens CDC cleanly. **Coverage boundary:** the harness models the process-restart boundary by a full `Close()` + resume-from-persisted-cursor, not an OS-level kill of an in-flight write goroutine; the resume exactly-once it pins is the structurally-inherited one (§7) — deterministic partition + producer-side seeding — which is the surface the value-fidelity review flagged.
