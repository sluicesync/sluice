...
Where an EARLY answer is needed — refusing before a copy starts rather than at emit time — the pattern is the emit-preflighter family: the target engine dry-runs its own emit against the schema and reports what it cannot render. One source of truth, asked ahead of time, and it cannot drift from the emitter because it IS the emitter. The members are `ir.IndexEmitPreflighter`, `ir.ViewEmitPreflighter`, `ir.TableEmitPreflighter` (table NAMES), `ir.TableNameFoldPreflighter` (the one that needs a connection) and `ir.ColumnTypeEmitPreflighter` — the column-TYPE member, which is the direct answer to the deleted capability table: it runs each engine's own `emitColumnType` over the schema, so it works for engine pairs nobody hand-coded and cannot report a fidelity the emitter does not have. Each is one question so no method's name is broader than what it checks; all five are asked at the same six copy entry points, which `TestIndexEmitPreflightReachesEveryCopyEntryPoint` enforces.

Two things the column-type member deliberately does NOT answer, because it takes no connection and carries no flags: whether the PG target has PostGIS, and whether the run passed `--enable-pg-extension`. It runs the emitter under the most permissive configuration the run could have, so what it refuses is refusable under every configuration; both remaining refusals still fire from the emitter in the create-tables phase, which is phase one.
...
**Schema drift during continuous sync.** MySQL binlog carries DDL; PostgreSQL logical replication does not, so the Postgres lanes detect drift from relation messages (native) or record it through event triggers (`postgres-trigger`). What happens next is the `--schema-changes` mode: `forward` (the default) translates additive and type-changing DDL through the IR and applies it to the target at the boundary, `refuse` stops the stream with the drained-model remedy, and a small set of shapes always refuse because forwarding them would silently change stored values: any cast to or from a session-normalised timestamp (MySQL `TIMESTAMP`, Postgres `timestamptz` — the server resolves those through the executing session's zone, so a forwarded ALTER re-casts the target's existing rows against a different one), a `RENAME COLUMN`, a volatile `DEFAULT`, the projection-invisible typmod shapes, and a drop of a synced table. ADR-0091 is the design and `docs/schema-change-runbook.md` the operator procedure; the "pause and require operator action" posture this paragraph originally described is what `refuse` mode still does.
