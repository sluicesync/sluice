...
**A forwarded `ADD COLUMN` is backfilled from the source by default.** The target fills the rows it already holds with the DEFAULT the forward carried, which is read when the change reaches sluice — after Django's follow-up `ALTER COLUMN … DROP DEFAULT`, after a `SET DEFAULT`, and re-evaluated by the target for `now()` or `gen_random_uuid()`. So sluice then reads the new column's value for every row from the source and writes it to the target, ahead of every later change. It costs one pass over the table on the source, the stream waits behind it, and it logs its progress; `--no-backfill-added-column` opts out and leaves the target's fill in place with a WARN. A backfill that is interrupted does not resume, so it stops the stream with `ADD-COLUMN-BACKFILL-INCOMPLETE` until you repair and acknowledge it. The details, including what a hard crash leaves uncovered, are in [cdc-streaming](operator/cdc-streaming.md).
