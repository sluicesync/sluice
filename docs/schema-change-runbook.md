...
- **`--verify`** runs a whole-table remaining-count on `--where` after the walk: 0, *over a run that actually moved rows*, prints the safe-to-contract signal; >0 exits with `SLUICE-E-BACKFILL-INCOMPLETE` (rows written behind the walk's cursor during the run — re-run to catch up, then verify again). Zero remaining over a run that moved **nothing** — this walk, the run whose cursor it resumed, and the completed run it no-ops over all at zero — exits with `SLUICE-E-BACKFILL-VERIFY-NO-EVIDENCE` instead of authorizing the contract step: the count and the UPDATE render the same `--where`, so a predicate that is valid SQL but selects nothing agrees with itself at every step and proves nothing. **Re-running the identical `--verify` command after a successful run is not that shape and exits 0** — the completed run's row count is persisted and carried into the no-op, so an idempotent cron/CI step stays idempotent. (The one exception is a spec a sluice at or below v0.108.0 marked `complete`: those state rows never recorded the count and it cannot be recovered, so such a spec re-verifies as no-evidence — use `--verify-only` for it.) **`--verify-only`** is the standalone scriptable gate for deploy pipelines: no walk, no UPDATEs, no control-table writes, no PK requirement, `--set` optional. It never raises the no-evidence refusal — with no walk it has nothing to have evidence about — and its report says so: it states the remaining count as a fact and that the count alone does not show a backfill ever ran.
...
**After upgrading to v0.156.0, an existing `sluice_cdc_state` needs one new column.** The `unforwarded_refusal` column holds a recorded `UNFORWARDED-SCHEMA-CHANGE` refusal. A fresh `control-tables ddl` bootstrap already includes it. On an existing table, sluice adds it with a detect-then-ALTER, which safe migrations refuses. A start without `--schema-already-applied` stops with `SLUICE-E-PS-DIRECT-DDL-BLOCKED`, naming the statement. With `--schema-already-applied`, the start fails too: every start first makes sure a refusal could be recorded, and a stream that could not record one would have it accepted silently by the next restart. Ship it once before restarting your streams:

```bash
sluice deploy-ddl --org myorg --database mydb \
...
**Default behavior: unambiguous DDL forwards online.** Since v0.92 (`--schema-changes=forward`, the default), the streamer applies every unambiguous source schema change on the target through the live CDC apply path — `ADD`/`DROP COLUMN`, `ALTER COLUMN TYPE`, and on a MySQL source `ALTER` nullability — logging each applied DDL at INFO. The stream stays online through routine schema evolution; usually there is nothing to coordinate.

**A forwarded `ADD COLUMN` is backfilled from the source by default.** The target fills the rows it already holds with the DEFAULT the forward carried, which is read when the change reaches sluice — after Django's follow-up `ALTER COLUMN … DROP DEFAULT`, after a `SET DEFAULT`, and re-evaluated by the target for a `now()` or `gen_random_uuid()` default dropped in between (a non-constant DEFAULT still declared when the change reaches sluice is refused, not backfilled). So sluice then reads the new column's value for every row from the source and writes it to the target, ahead of every later change. It costs one pass over the table on the source, the stream waits behind it, and it logs its progress; `--no-backfill-added-column` opts out and leaves the target's fill in place with a WARN. A backfill that is interrupted does not resume, so it stops the stream with `ADD-COLUMN-BACKFILL-INCOMPLETE` until you repair and acknowledge it. The details, including how a hard crash is handled, are in [cdc-streaming](operator/cdc-streaming.md).

**What never forwards, on any source: `CREATE`/`DROP INDEX` and `ADD`/`DROP`/`MODIFY CHECK`.** A shape forwards only if the source CDC reader produces a boundary carrying it, and neither reader does — the MySQL reader's DDL-boundary projection carries columns and the primary key only, and pgoutput's relation message carries columns and the replica-identity flag only ([ADR-0091 §1d](adr/adr-0091-default-on-schema-change-forwarding.md) is the matrix, and the source of truth). On a Postgres source the same holds for nullability. Plain indexes are still a silent gap: a source `CREATE INDEX` leaves the target without that index, un-WARNed. Add it on the target out-of-band (`CREATE INDEX CONCURRENTLY` on PG; a deploy request on PlanetScale).

**Constraint, RLS, policy and default changes stop the stream (`UNFORWARDED-SCHEMA-CHANGE`, v0.156.0+; Postgres and MySQL/MariaDB binlog sources).** The streamer does not forward these changes, so the target is left without them. The classes are constraint changes (primary key, `UNIQUE`, foreign key, `CHECK`, plus `EXCLUDE` on Postgres), RLS and policies, `NOT NULL` on Postgres, `DEFAULT`, and identity, `EXTRA` or generation changes. Through v0.155.1 they passed silently, an `ADD CHECK` or `CREATE POLICY` included. Since v0.156.0 the stream stops at the next write to the affected table, and the refusal is recorded so every restart refuses again. To recover, apply the change on the target, then start once with `--accept-unforwarded-schema-change=<fingerprint>`, using the fingerprint the replayed refusal prints. The full procedure is in [cdc-streaming](operator/cdc-streaming.md). To avoid the refusal altogether, use the drained model below. A change applied on both sides while the stream is stopped is part of the next start's baseline, so it does not refuse. PlanetScale and Vitess (VStream) sources are not covered: the gap there is still silent.
...
**A cast to or from a session-normalised timestamp refuses, in either direction and at any precision.** The original shape is a MySQL `TIMESTAMP` ↔ `DATETIME` `MODIFY`, and since v0.139.0 the refusal covers the wider measured class: a cast between `TIMESTAMP` and `VARCHAR`, `DATE`, `TIME` or `BIGINT` (and Postgres `timestamptz` to text or date) resolves through the executing session's zone in exactly the same way. The `time` / `timetz` pair is MEASURED asymmetric, and since v0.142.0 the refusal matches the measurement rather than covering both halves. `time` → `timetz` refuses: an offset is invented, and Postgres takes it from the executing session. `timetz` → `time` FORWARDS: `timetz` stores its offset alongside each value, so dropping it consults no session zone at all, and the cast measured byte-identical under `UTC` and `Asia/Tokyo`. The array shapes follow the scalar ones (`time[]` → `timetz[]` refuses, `timetz[]` → `time[]` forwards). The `timestamp` family is unchanged and still refuses in both directions, because `timestamptz` is stored normalised to UTC and so re-renders through the session zone whichever way the cast runs. Measured on MySQL 8.0.46 and Postgres 16. `ALTER TABLE t MODIFY c DATETIME` on a `TIMESTAMP` column (or the reverse) makes MySQL convert every stored value using the *executing session's* `time_zone` — not the server's, and nothing the binlog or the VStream wire carries. The source operator's ALTER ran under their own session, whose shipped default is `time_zone=SYSTEM` (the host zone), while sluice pins `time_zone='+00:00'` on every connection it opens; so a forwarded ALTER would re-cast the target's pre-existing rows against a different zone whenever the source host is not UTC. Every pre-existing target row of that column would end up off by the host's UTC offset, every row applied after the ALTER would be correct, row counts would match, and the sync would exit 0 — which is why the shape refuses (the message names the column, both types and the mechanism) rather than forward. Since v0.138.0 the refusal also fires at a table's **first** DDL boundary after a cold start or a warm resume: it had compared each boundary against a prior shape that only the boundary emitter itself wrote, so the first `MODIFY` per table after any start went unchecked, and under `--inject-shard-column` was forwarded (observed on a `+09:00` source: every pre-existing target row read nine hours off at exit 0). The MySQL reader is now seeded with each in-scope table's shape at start — the source schema on cold start, and on warm resume the target's own current column types read back through the target engine (the independent witness of the last shape the stream committed under; a target you have already ALTERed to the new family therefore lets the replayed DDL through as the no-op it is, which is what makes the drained-model recovery below resumable), with the retained schema-history version as the fallback for a table the target does not hold, a `--type-override` on a temporal column, or a target whose read-back does not distinguish the pair — and keeps its last decoded shape across a DDL cache clear, on all three lanes (binlog, VStream, VStream snapshot stream); the pipeline's boundary routers carry the same door as a belt. Precision-only changes within one type (`DATETIME(3)` → `DATETIME(6)`), and casts with no zoned side at all (`DATETIME` → `VARCHAR`), carry no zone conversion and keep forwarding. The widened class is enforced at the pipeline door, which is reached when a boundary is FORWARDED — the mode that re-casts the target's existing rows; the reader-side checks that refuse at a table's first boundary still carry the original pair. The remedy is the drained-model workflow below: stop with `--wait`, run the same `MODIFY` on source **and** target from your own client so both casts happen under a session zone you chose, then restart with the same `--stream-id`. Stated residual: a fallback table with no retained history row cannot be checked at its first boundary after a warm resume, so a swap performed while the stream was stopped on such a table is primed rather than refused. A SECOND residual, established as fact by the v0.143.0 fan-out work and stated here because the list read as complete without it: on a MySQL MULTI-DATABASE stream this refusal does not fire at all, regardless of seed or history. The reader is handed a seed on that lane, but the refusal is armed on `schemaDeltaAppliesToTarget`, which is false in multi-database mode by construction — so the seed is delivered and inert. Postgres multi-schema IS covered as of v0.143.0; MySQL multi-database is not. The Postgres lane's twin of that gap was closed in v0.139.0 — its reader now takes the same target-witness seed and checks a table's first relation message against it. The `postgres-trigger` lane has no zone-SPECIFIC door and does not need one: a Postgres event trigger fires whether or not sluice is running, so a swap made while the stream was stopped is recorded as a DDL marker at the moment it happens and the resume refuses on it. Its residual is narrower and not specific to this class — an `--allow-polled-fingerprint` install has no event triggers, so no source DDL is visible to it at all. **On the MySQL family the refusal honours the sync's table filter as of v0.148.3, on all three of its CDC lanes.** A `TIMESTAMP`⇄`DATETIME` `MODIFY` on a table you excluded with `--exclude-table` no longer kills the stream, because nothing this sync emits for that table reaches the target — the same posture the XA refusal has carried since Bug 246. Before that the two VStream lanes could not honour a filter under any circumstance, having never declared the interface the pipeline hands it over through, so `--exclude-table` is a working mitigation on PlanetScale and Vitess only from this release.
...
`sync stop --wait` guarantees the in-flight batch is committed and the CDC position is persisted past the last applied event — on a Postgres source that is the last transaction's end LSN, so the resumed stream starts at the *next* transaction rather than re-delivering the one it closed. Re-running `sync start` with the same `--stream-id` warm-resumes automatically (there is no `--resume` flag on `sync start`); the post-restart CDC schema cache rebuilds from the first event after that position, so the new shape is the anchor on both sides. The corollary is that a schema change applied to the source *only*, while the stream was stopped, is not classified at resume — this workflow applies it on both sides before restarting, which is why it works. (Before audit finding A2-1 was fixed, a Postgres resume re-delivered the last applied transaction with its pre-DDL relation shape, and every refusal re-fired on the restart the hint prescribed.)
