...
- **Being single-node is NOT an exemption**, however intuitive that is. The hazard is not only promotion of a standby: a cluster **resize** or a maintenance rollout replaces the node, which strands a primary-local slot exactly as a promotion would, and replica count is irrelevant to it. PlanetScale's console states this directly — it asks for these same two parameters so slots *"survive failovers and cluster changes"* — and **holds cluster changes for the branch** until they are set (observed on a single-node instance, 2026-09-10). sluice also could not tell whether you are single-node even if it mattered: `pg_stat_replication` shows a non-privileged role only its own rows, so an empty result is equally consistent with "no standby" and "a standby this role cannot see".
