...
- **A single-node instance with no standby has no standby to promote**, so the classic failover case cannot arise — but do not read that as "nothing to fix". A cluster **resize** plausibly replaces the node, which would strand a primary-local slot exactly as a failover would, and that is **untested**: PlanetScale's API refuses a Postgres branch resize outright (`Resize is not supported`, confirmed slot-independent by a zero-slot control), so it could not be measured. Treat single-node as *probably* fine, not certainly. sluice also cannot tell whether you are single-node: `pg_stat_replication` shows a non-privileged role only its own rows, so an empty result is equally consistent with "no standby" and "a standby this role cannot see". Suppressing the warning on that would silence it on exactly the HA clusters that need it, so the warning names the single-node case instead of guessing.
