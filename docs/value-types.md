...
**One documented exception to this table: the `postgres-trigger` change stream.** Its reader decodes the JSONB capture log with `UseNumber`, so every non-integer number — a `Decimal`, a `Float`, a numeric array element, a number inside a jsonb document (which stays a `map[string]any`) — reaches consumers as a `json.Number` carrying the source's exact text, and integers as `int64` ([ADR-0066](adr/adr-0066-postgres-trigger-engine-variant.md) §4). Every applier accepts `json.Number` for these families. The backup chunk codec preserves it: a `postgres-trigger` chain's change chunks are read with `ChangeChunkReader.PreserveNumbers` (`internal/pipeline/blobcodec`), so restore, `sync from-backup` and smart compaction see the same `json.Number` the live stream produces. Before v0.156.4 that read decoded every bare number as a `float64`, and a chain restored any number a `float64` cannot carry exactly altered (`10.50` → `10.5`, long `numeric` values truncated, `1e-400` → `0`) — see CHANGELOG. Because the chunk bytes are unchanged, a segment whose chunks carry such a number is stamped backup `FormatVersion` 12 so older binaries refuse it ([backup-format-versioning.md](backup-format-versioning.md)).
