...
The `d1` reader and the `sqlite-trigger` / `d1-trigger` capture path defeat both by projecting, per column, a pair: `typeof(col)` plus `CASE WHEN typeof(col)='blob' THEN hex(col) ELSE CAST(col AS TEXT) END` (REALs use `format('%!.20g', col)` — SQLite's alternate-form-2 `!` flag, which raises the precision until the rendering is lossless on SQLite ≥ 3.43). The reader reconstructs the `ir.Row` value from that `(storage-class, text-or-hex)` pair through the engine's shared faithful decoder, so integers above 2⁵³ and binary BLOBs round-trip **exactly** ([ADR-0132](adr/adr-0132-d1-query-api-reader.md)). The same encoding is used by the trigger-CDC capture triggers (they must NOT use `json_object()`, which reintroduces the float64 rounding) so cold-start and CDC share one proven seam ([ADR-0135](adr/adr-0135-sqlite-trigger-cdc.md)). *(Corrected 2026-08-22: this originally read `format('%.17g', col)` "for shortest-exact round-trip" — true of C printf, but SQLite 3.43 rewrote `format()`'s float rendering so a plain `%.Ng` is no longer digit-faithful at any N (`0.30000000000000004` renders as `"0.3"`); every REAL through the D1 reader, D1 staging, and both trigger-CDC lanes was silently altered — and a REAL at the float64 max rendered as an out-of-range 16-digit string that killed the stream loudly. Found by the SQLite/D1 adversarial corpus; the per-PR gate is `TestCapturedValueExpr_RealRenderRoundTripsExactly`, and the CDC reader now refuses at stream start if installed capture triggers still carry the old expression.)*
