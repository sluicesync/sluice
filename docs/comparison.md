...
**Capability claim.** sluice carries schema constructs most data-movement tools drop on the floor. PG Row-Level Security policies are captured into the IR and re-emitted on a same-engine target. The cross-engine PG → MySQL case has no RLS equivalent to land on: the MySQL writer does NOT refuse — it drops the policies and logs exactly one WARN per run naming every affected table (ADR-0063's recorded contract, pinned by `TestRLSWarn_PGtoMySQL_DropsPoliciesAndWarnsOnce`), so the rows arrive without per-tenant filters and the operator is told once, not per table. Treat that WARN as a stop signal on a multi-tenant schema. Policies are carried at copy time (migrate, and the sync cold start); a policy, RLS or constraint change made on a Postgres source *mid-sync* is not forwarded by the change stream. Through v0.155.1 such a change went missing silently. Since v0.156.0 the stream refuses under `UNFORWARDED-SCHEMA-CHANGE` (GC-2) until the change is applied to the target and acknowledged. MySQL/MariaDB binlog sources get the same refusal for constraint and default changes. PostGIS geometry round-trips with SRID. PG `DOMAIN` `CHECK` constraints translate to MySQL 8.0.16+ table-level `CHECK` where the shape is safely translatable; where a DOMAIN's CHECK cannot be re-emitted, the column lands as the DOMAIN's base type and the writer logs one WARN per run rather than refusing. Generated columns and many `CHECK` idioms translate via the ADR-0016 expression translator — with `--expr-override` as the escape hatch for generated columns, and `--exclude-table` / fix-on-source for CHECK sites (the override rewrites only generated-column bodies).
