...
**PostGIS Z/M-dimensional columns toward MySQL abort mid-copy with a raw server error (queued).** MySQL 8 supports no Z/M geometry; a `geometry(POINTZ, …)` column passes every preflight today and the copy dies with the server's own error 1416, naming nothing about dimensionality. Loud, but late and unnamed — the preflight arm is a queued audit item (SPAT-2). Workaround: exclude or flatten the column at the source.

**`postgres-trigger` sources with PostGIS columns: cold copy works, the first spatial DML wedges the stream (queued).** The trigger capture renders geometry as GeoJSON, which the apply path cannot decode, so a spatial INSERT/UPDATE/DELETE after cold start fails the apply loudly and the stream stops. The capture-format fix is a queued audit item (SPAT-4). Workaround: exclude spatial tables from trigger-CDC replication, or use the native `postgres` engine where logical replication is available.
