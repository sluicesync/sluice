...
The expressions a table stores — an expression `DEFAULT (…)`, a generated column, a `CHECK`, a functional index — are rendered unfaithfully by both engines' `information_schema`, in opposite ways. MySQL renders the expression's bytes as Latin-1 re-encoded as UTF-8, so `DEFAULT ('é')` reads back as `_utf8mb4'Ã©'`; that is lossless (a four-byte character survives it), and MySQL's `SHOW CREATE TABLE` prints the same expression faithfully except that a character beyond the Basic Multilingual Plane appears there as `????`. MariaDB's `information_schema` is faithful for the BMP but writes one `?` per byte for a character beyond it, while its `SHOW CREATE TABLE` is faithful throughout. sluice reads each expression holding a non-ASCII byte (MySQL, PlanetScale, Vitess) or a `?` (MariaDB) from the faithful surface and requires it to agree with the other one on that object's own `SHOW CREATE TABLE` line; an expression the two do not agree on — for example one written under a non-UTF-8 charset introducer — refuses the schema read, naming the object. A mydumper dump of a MySQL source carries each expression as `SHOW CREATE TABLE` printed it, so a character beyond the BMP in an expression is already `????` in the dump.
