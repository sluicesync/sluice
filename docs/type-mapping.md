...
|-------------------------------|----------------------------------------------------|-------|
| `TINYINT(1)`                  | `Boolean{}`                                        | Convention. A value outside `{0,1}` would be collapsed to `true` by the boolean mapping, so the reader REFUSES loudly (`SLUICE-E-VALUE-TINYINT1-RANGE`) at the first such value — a fail-fast preflight catches it before the copy, and the per-read-path decode guard is the floor. Remedy: change the source column type away from `TINYINT(1)` (e.g. `MODIFY … SMALLINT`), which works on every source incl. PlanetScale/Vitess; or, for a bulk migrate from a non-Vitess MySQL source, `--type-override col=smallint`/`int` (which does not apply to a VStream source — see below). |
...
| `interval` (any declaration)      | `Interval{}`                                  | **The typmod is deliberately dropped**, so a PG→PG migrate lands every declaration as bare `interval` — `interval(0)`, `interval(2)`, `interval day to second` and `interval year to month` all arrive unrestricted. The VALUES are byte-identical (bare `interval` is the widest interval type and PG rounds on STORE, so the source's own declaration had already rounded before sluice read it — measured on real PG 16); what is lost is the target's CONSTRAINT, and the harm arrives at CUTOVER, when an application writing to the migrated target stores values the source would have rounded or discarded. `interval year to month` is the sharpest: the source discards days and time entirely, the target keeps them. Carrying the typmod would move `backup.ComputeSchemaHash` and repartition every existing chain containing an interval column, so this is a recorded deferral, not an oversight — see `TestIntervalTypmodIsDroppedDeliberately` and the `ir.Interval` doc. |
...
| `geometry`                        | `Geometry{Subtype: ...}`                      | PostGIS only. Toward a MySQL-family target, a value with NaN/Inf coordinates — including `POINT EMPTY`, whose WKB IS a NaN/NaN point — refuses with `SLUICE-E-VALUE-UNREPRESENTABLE`; MySQL would otherwise store it silently as a value it cannot read back. A Z/M-flagged column (POINTZ, …ZM) toward MySQL-family refuses at preflight (v0.124.0) — MySQL 8 has no Z/M geometry, and the writer would otherwise emit a 2D column and die mid-copy on the server's raw 1416. A 2D `geography` lands as planar geometry (bytes + SRID byte-exact) with a geodesic→planar semantic-flatten note in `schema preview`. |
...
- **Identity columns** (`GENERATED ... AS IDENTITY`) round-trip as identity columns. Their implicit backing sequences are never surfaced as sequence objects, but their **options** (`START` / `INCREMENT` / `MINVALUE` / `MAXVALUE` / `CACHE` / `CYCLE`, from `pg_sequence`) and the **`ALWAYS` / `BY DEFAULT`** mode (`identity_generation`) ride on the column (`ir.Column.Identity`) and are re-emitted on a Postgres target: options in the identity clause, rendered only where they differ from the factory defaults; `ALWAYS` restored by `migrate` after the copy and kept `BY DEFAULT` by `sync` / `restore` / chain restore with an `IDENTITY-ALWAYS-DOWNGRADED` WARN (see [production-readiness](production-readiness.md#known-limitations)). Before the 2026-09-22 fix both were dropped silently: every identity re-created `BY DEFAULT` with factory options.
...
### MySQL-family character defaults outside the Basic Multilingual Plane

MySQL and MariaDB store `information_schema.COLUMNS.COLUMN_DEFAULT` in utf8mb3, which cannot hold a character encoded in four UTF-8 bytes (emoji, CJK extension ideographs, historic scripts), and the server stores each one as `?`: `VARCHAR DEFAULT '😀x'` reads back as `?x`, and MariaDB's `TEXT DEFAULT '😀z'` as `????z`. So when a character column's literal default reads back containing a `?`, sluice re-reads its true value: on MySQL and MariaDB with `SELECT HEX(CONVERT(DEFAULT(col) USING utf8mb4))`, and on PlanetScale and Vitess, whose vtgate does not accept that query, from `SHOW CREATE TABLE`. The re-read value must agree with the catalog text everywhere except where a `?` can stand for a lost character, or the read refuses, naming the column. The same holds for the CDC boundary projection a forwarded `ADD COLUMN` carries, for the unforwarded-schema-change door's DEFAULT comparison, and for a mydumper dump of a MySQL source (whose `SHOW CREATE TABLE` spells such a default as a `0x…` literal).

Three edges, stated. On PlanetScale/Vitess, a column whose charset is not UTF-8 (`utf16`, `utf32`, …) and whose default holds such a character refuses, because `SHOW CREATE TABLE` prints it in that charset's bytes. An `ENUM`/`SET` whose default uses a label holding such a character refuses: `COLUMN_TYPE` loses the labels the same way, so the type itself cannot be read faithfully. A mydumper dump of a MariaDB source cannot be recovered, because MariaDB's `SHOW CREATE TABLE` prints the `?` too, and the dump carries the default as written.

### MySQL-family expression text with non-ASCII characters

The expressions a table stores — an expression `DEFAULT (…)`, a generated column, a `CHECK`, a functional index — are rendered unfaithfully by both engines' `information_schema`.

**MySQL (and PlanetScale, Vitess)** stores each string literal of an expression in the encoding its charset introducer names — `_utf8mb4'…'` normally, `_latin1'…'` for a table created from a latin1 client session — and its `information_schema` renders every stored byte as a separate character, so `DEFAULT ('é')` reads back as `_utf8mb4'Ã©'`. That rendering is lossless, so sluice undoes it and decodes each literal by its introducer: utf8mb4, utf8mb3 and binary bytes as UTF-8, latin1 bytes as their latin1 characters. It refuses the schema read, naming the object, for a latin1 byte in 0x80–0x9F (MySQL's latin1 is cp1252 there, and sluice carries no cp1252 table), for a literal in any other charset (`gbk`, …), and for a non-ASCII literal with no introducer. The decoded expression must also agree with that object's own `SHOW CREATE TABLE` line — which prints each literal either as its stored bytes (a character beyond the Basic Multilingual Plane, or a byte that is not UTF-8, as `?`) or, for a STORED generated column, as its converted value — or the read refuses.

**MariaDB** converts expressions to UTF-8 when a table is created, so its `SHOW CREATE TABLE` is faithful; its `information_schema` is faithful for the BMP but writes one `?` per byte for a character beyond it. sluice takes the `SHOW CREATE TABLE` text that an expression holding a `?` matches, searched only within the object's own clause (`DEFAULT`, `AS (`, `CHECK (`), and refuses on no match or an ambiguous one.
...
A `CHAR`/`VARCHAR`/`TEXT` column declared `latin1`, `sjis`, `utf16` or any other non-UTF-8 charset maps to the same IR string type as a `utf8mb4` one, and its value is always carried as UTF-8 — the IR's string contract. The bulk copy gets UTF-8 from the server (it reads over a `utf8mb4` connection); the change stream gets the column's stored bytes (the binlog row image and the VStream row event both carry them in the column's own charset) and converts them by the declared charset — every 8-bit charset, `latin1` as MySQL's cp1252 variant, the Japanese, Korean and GB charsets, and the UTF-16/UTF-32/UCS-2 family, each table graded against the server's own conversion over its whole code space. A value it cannot convert faithfully — non-ASCII `big5`, and on VStream non-ASCII `gbk`/`big5`/`tis620`/`gb18030`, which vttablet sends with no collation — refuses with `CHARSET-NOT-DECODABLE`. A binlog value is decoded by the charset its TABLE_MAP says it was written in, so a replay across an `ALTER … CHARACTER SET` is exact; MariaDB under `binlog_row_metadata=NO_LOG` and PlanetScale/Vitess carry no such record, so a replay there is decoded by the current charset (`CHARSET-HISTORY-UNRECORDED` WARNs per table) and, for a change into a non-UTF-8 charset, refuses with `SLUICE-E-CDC-SCHEMA-REPLAY-MISMATCH` when it reaches an `ALTER` that sets it explicitly — after the rows before it, which the remedy re-copies (a re-snapshot under `sync`, a fresh full backup for a chain); a replay into a UTF-8 charset carries the stored bytes as v0.156.2 did. The windows that remain are listed in [migrating-legacy-mysql](operator/migrating-legacy-mysql.md). A PlanetScale/Vitess column with a `_bin` collation arrives typed `VARBINARY`/`BLOB`/`BINARY` and is decoded as text by its collation (an `ENUM`/`SET` one through the label resolver). A backup window that decoded rows across such a change without reaching the `ALTER` refuses before it commits. A PlanetScale/Vitess `ENUM`/`SET` cell of such a column arrives as UTF-8 in CDC and as stored bytes in COPY and is resolved against the column's labels (the index-0 `''` is always a member). Before v0.156.3 the change stream carried the stored bytes as they were. Operator detail and repair: [migrating-legacy-mysql](operator/migrating-legacy-mysql.md).
...
1. **Declared temporal / bool spellings override affinity.** A column declared `DATE` → `ir.Date`; `DATETIME` / `TIMESTAMP` → `ir.DateTime` (the tz-**naive** family — SQLite storage has no zone and the platform has no zone type); `TIME` → `ir.Time`; `BOOL` / `BOOLEAN` → `ir.Boolean`. **On a MySQL target this is the difference between `DATETIME` and `TIMESTAMP`, and it matters:** `TIMESTAMP` is an instant type spanning 1970-01-01 .. 2038-01-19 and zone-converted on store, so before this mapping was corrected an ordinary pre-1970 or post-2038 SQLite datetime refused at insert with a bare server `1292`. `DATETIME` carries it. Postgres targets are unaffected either way — both IR families emit PG `TIMESTAMP`. `--infer-types` makes the same split from the data: a value set where every value carries a UTC offset resolves to `ir.Timestamp` with the zone flag set; one where any value does not is naive and resolves to `ir.DateTime`. (`DATETIME` is checked before `DATE`/`TIME` because the string contains all three.) These spellings would otherwise fall to NUMERIC affinity and read as decimals. Temporal types carry `PrecisionUnspecified` — SQLite has no fractional-second precision concept, so targets pick their max-fidelity form (PG bare, MySQL `(6)`; see [temporal precision](#temporal-fractional-second-precision-bare-vs-explicit--restore-parity-triage-3)).
...
**Default policy:** keep the documented `TINYINT(1)`→`Boolean` mapping (changing it would break the overwhelming majority of schemas that *do* mean boolean), but **REFUSE loudly** — coded `SLUICE-E-VALUE-TINYINT1-RANGE` — when a value outside `{0,1}` is read. The boolean decode collapses every non-zero value to `true`, so `2`/`127`/`-1` would lose their real value; earlier releases only WARNed and carried the collapsed bool, which a field report hit as silent data loss on a column holding `0..6`, so it is now a hard refusal. The refusal fires at the *first* out-of-range value, on every read path — the bulk-copy / snapshot reader, the binlog CDC reader, the VStream CDC + cold-start path, and the mydumper flat-file source — *before* the row is written, so no collapsed value ever reaches the target. A **fail-fast pre-copy preflight** (bounded by a `MAX_EXECUTION_TIME` cap so a clean column never full-scans) catches it at planning time, before any target table is created, on both `migrate` and `sync` cold-start; the per-read-path decode guard is the correctness floor for anything the bounded preflight cannot confirm.

**Remedy:** the universal fix is to change the source column's type away from `TINYINT(1)` (for example `ALTER TABLE … MODIFY <col> SMALLINT`), which works on **every** source — including PlanetScale/Vitess, where sluice reads the column type from the replication stream. For a bulk migrate from a **non-Vitess MySQL source** you can instead supply `--type-override TABLE.COL=smallint` (or `=int`/`=integer`, or the per-column `mappings:` hook): it rewrites the IR type the **reader** decodes with, so the cell is read as an integer (not collapsed to a bool) and carried faithfully. `smallint` (16-bit) is the recommended floor: a `TINYINT(1)` value always fits, and — unlike a `tinyint` override — it cannot re-emit a `TINYINT(1)` target column that would re-trigger the boolean mapping on a round-trip. The `--type-override` escape **does not apply to a PlanetScale/Vitess (VStream) source** — there the boolean decision is made from the replication wire's own `column_type`, not the IR schema, so an override re-types the target column while the value is still collapsed at read time; on a VStream source, change the source column's type. (Making the override honor-able on VStream is a filed follow-up.)
...
- **A value that exceeds it is refused, never rounded (v0.156.4+, GC-37 (c)).** Past 35 integer digits MySQL refuses the write itself (Error 1264). Past 30 fractional digits it does NOT — it rounds the value and reports only Note 1265, which even strict `sql_mode` accepts, so through v0.156.3 `0.1234567890123456789012345678901` landed as `0.123456789012345678901234567890` and `-0.99999999999999999999999999999999` as `-1.000000000000000000000000000000`, at exit 0, through the CDC applier (serial and concurrent lanes — which also carries chain restore, `sync from-backup` and the added-column backfill); the bulk-copy writers (`LOAD DATA` and the batched INSERT path) were already loud, because they refuse a strict-mode write whose warning list is non-empty. sluice now refuses any DECIMAL value — and any DECIMAL column's literal DEFAULT, on `CREATE TABLE` and a forwarded `ADD COLUMN` alike — whose significant fractional digits exceed the target column's scale, before the driver sees it, with `SLUICE-E-VALUE-UNREPRESENTABLE` marked `DECIMAL-SCALE-EXCEEDED` naming the column and the digits lost. Trailing zeros past the scale lose nothing and are not refused (`1.5000…0` lands as `1.5`). The same check applies to a constrained `DECIMAL(p,s)` column. The remedy is to round on the source if the digits are not needed, or `--type-override` the column to a text type to carry every digit. A constrained `numeric(p,s)` with `s > 30` fails at `CREATE TABLE` on MySQL (Error 1425, loud); MariaDB accepts scales up to 38 and lands it exact. The check covers every Go type a reader delivers for a decimal column (a string, the postgres-trigger reader's `json.Number`, bytes, a float retyped with `--type-override`), and a target column whose name differs from the source's only in case. **Residual:** a value for a column the applier's descriptor map does not know at all (a cold cache) is not checked; a Postgres target's `numeric(p,s)` rounds over-scale input silently too, reachable only through a narrowing `--type-override` or a narrower pre-existing column (GC-37 (t)).
...
**SQLite / D1 index columns carry their collation, and a target that cannot enforce it refuses a unique key.** SQLite compares a column under the collation it was declared with (`email TEXT COLLATE NOCASE` refuses `'A@X'` after `'a@x'` under a `UNIQUE`), and the reader carries that per index column from `PRAGMA index_xinfo` (`ir.IndexColumn.Collation`, tagged `CollationDialect: "sqlite"`; `BINARY`, the default, is never carried; the entry's `DESC` is recovered from the same catalog). A SQLite target emits `COLLATE` verbatim on the index or `PRIMARY KEY` column. Postgres and MySQL have no spelling for `NOCASE` (ASCII-only case folding) or `RTRIM`, so on a key that enforces uniqueness — a `PRIMARY KEY`, a `UNIQUE`, a constraint-backed unique — the migration is refused at preflight, before any data moves, naming the column and collation; on a non-unique index the collation is dropped with an `INDEX-COLLATION-DROPPED` WARN (it changes ordering and cost, not which rows are legal). A per-column `COLLATE` on a column that is NOT part of any index is still unread (the rest of GC-5).

**SQLite / D1 sources carry an inline or table-level `UNIQUE` as a constraint under a generated name.** SQLite backs each `UNIQUE` constraint with an auto-index named `sqlite_autoindex_<table>_<N>`, and every `sqlite_`-prefixed object name is reserved — a SQLite target refuses to create it (`object name reserved for internal use`), and used to do so in the index phase, after the whole copy. The reader therefore carries such an index as `ConstraintBacked` under `<table>_<col>[_<col>…]_key` (the name Postgres itself generates for the same inline `UNIQUE`): a PG target re-emits `ADD CONSTRAINT`, MySQL and SQLite a unique index, and uniqueness is never relaxed on any of them. The name is sluice's, not the operator's, so `ConstraintNamed` stays false. A source `CREATE UNIQUE INDEX` keeps its own name and is not constraint-backed. A SQLite target also refuses any `sqlite_`-prefixed index name at preflight (before any data moves) rather than after the copy — the shape a manifest written by an older binary can still carry.
...
**`json[]`/`jsonb[]`/`bytea[]` are `migrate`-only against a MySQL target.** A continuous-sync run is **refused at cold-start preflight** naming the column — including when the column is declared as a domain over the array (`CREATE DOMAIN d AS json[]`). The change applier reads column types from the *target*, where both families are the same `JSON` column, so it cannot tell which of the two encodings above applies — and the bytes cannot decide it either, since a `bytea` may legitimately spell a JSON document. Guessing would make the cold copy and every later CDC apply encode the same column differently with no error on either side, so sluice refuses instead. Every other array element family (text, numeric, temporal, uuid, network, native) syncs normally. Use `sluice migrate` for a one-shot copy, or `--exclude-table`.

The preflight runs at `sync` **cold start** and `schema add-table`. Three paths do not reach it, and they do not all behave alike:

- **Warm resume** and a **mid-stream `ALTER TABLE … ADD COLUMN`** read no schema by design. Over the Postgres logical-replication (`postgres`) source the element still arrives as raw bytes and the writer refuses it loudly, per row, naming the column — a late error, not a silent one. A sync started on **v0.115.0 or earlier** carrying such a column will therefore stop on its first resume after upgrading; that is intended for `json[]` (it was landing base64 tokens) and is a behaviour change for `bytea[]` (which was reversible). The refusal carries the same remedies as the preflight.
- Over the **`postgres-trigger` source**, the same warm-resume/added-column case is **silently divergent** rather than loud: that reader decodes its capture payload as JSON with no column types, so a `bytea[]` element arrives as PostgreSQL's `\x`-hex text and lands as `["\\xdead"]` where the cold copy stored `["3q0="]`. It is not refused because on that door the value is byte-identical to a `jsonb` column holding the *document* `["\\xdead"]`, and to an ordinary `text[]`/`int[]` change — refusing would break both. Cold start is covered (the preflight reads the source schema whatever the source engine, and `sluice trigger setup` refuses `json[]`/`jsonb[]` outright), so the exposure is a stream started before v0.116.0 or a column added to a live one.
- The **multi-database cold-start fan-out** has its own schema plumbing and is not wired to the preflight; its sources are MySQL (which has no array type to read) and Postgres (whose door produces raw bytes), so the writer's refusal still makes it loud.

**No `array_strategy` override exists.** Earlier revisions of this page offered `array_strategy: json | concat`; no such config key was ever implemented. Use `--type-override` to land the column as `text` and parse it downstream.
...
Postgres large objects are server-side blobs stored in the `pg_largeobject` catalog, referenced from user tables by an `oid` value (or the `lo` extension's domain over `oid`). **sluice does not copy them** — and an `oid`/`lo` column is itself an unsupported column type, so a source whose `pg_largeobject_metadata` is non-empty gets one of two outcomes (roadmap item 68c; sequencing corrected for Bug 205 — the advisory census runs *before* the schema read, so its WARN always lands first):

- **An in-scope `oid`/`lo` column exists:** the census WARN fires first, naming every suspect `table.column` and explaining that the blobs will not travel, then the schema read refuses the column loudly (`postgres: unsupported data_type "oid"`). To proceed without the table, `--exclude-table` it; to carry the blobs, convert the columns to inline `bytea` on the source first (`ALTER TABLE t ALTER COLUMN c TYPE bytea USING lo_get(c)`) so sluice copies the bytes like any other column, or export them separately (`lo_export`, or `pg_dump --blobs` restored with the native tools).
- **Large objects exist but no in-scope column is typed `oid`/`lo`:** a single quieter WARN that the blobs exist and will not travel, and the run proceeds — the census can't prove anything references them, and only the operator knows whether they matter.

The probe is advisory — if the census queries fail (managed-platform permission variance), the run proceeds without the WARN, and an in-scope `oid`/`lo` column still refuses at the schema read.
...
### Generated-column storage class: STORED vs VIRTUAL

PostgreSQL 18 added `GENERATED ALWAYS AS (…) VIRTUAL` (computed on read, no storage) beside the STORED form every earlier version has, and made VIRTUAL the default spelling when neither keyword is written. sluice carries the axis on `ir.Column.GeneratedStored`: the PG reader classifies it from `pg_attribute.attgenerated` (`s` / `v`; until the 2026-09-22 fix the reader asserted "Postgres only supports STORED" and read every generated column as STORED, so a PG 18 VIRTUAL column silently re-created as STORED on the target — gap census 2026-09-22 S1/D5), and the MySQL reader from `information_schema.columns.extra`. On emit a STORED column is always STORED. A VIRTUAL column — from a PG 18 source or a MySQL VIRTUAL column — stays VIRTUAL on a PG target at 18 or newer and is **promoted to STORED with a WARN carrying `GENERATED-VIRTUAL-PROMOTED-TO-STORED`** on an older target (the invariant survives; the storage tradeoff changes). One named exception on PG 18+: a VIRTUAL column that the source **indexes** (MySQL allows it; PostgreSQL 18 refuses `CREATE INDEX` on a virtual generated column) is promoted to STORED with the same marker so the index can build. Both storage classes are held to the same immutability rule by PostgreSQL, so VIRTUAL is not a way around a `STABLE` function in the body. Mid-sync, PG never publishes a VIRTUAL column over logical replication (nor a STORED one, without a publication option sluice does not create), so it is absent from the wire and re-derived on the target exactly as STORED columns are today.
