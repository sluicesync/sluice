...
**Compaction now refuses a passphrase-encrypted chain whose root manifest is already missing.** If a chain was compacted or pruned past its root by an earlier version, it is already unrestorable, and compaction will now say so up front and name the `cp` recovery rather than merging further into a chain nobody can read. The test was not waiting on a slow walsender — a clean shutdown releases the slot in about two milliseconds — it was waiting for a slot that had already been deleted, which is why it always consumed its entire budget and why no timeout would ever have been long enough. It now passes in 1.5 seconds instead of 181.
