...
The attribute keeps being carried — reverting that would trade an availability defect for a silent fidelity one. Instead sluice checks the target's keys once the stream opens and, before applying any change, refuses with `SLUICE-E-TARGET-DEFERRABLE-KEY` naming every affected table and the remedy. A table with a deferrable primary key *and* a usable immediate unique index streams normally. Worth knowing if you are reading catalogs yourself: `DEFERRABLE INITIALLY IMMEDIATE` is refused as an arbiter too, so the distinction that matters is deferrable-versus-immediate, not when the check is deferred to. The same defect existed on the idempotent bulk-copy writer — the path behind chain replay and `schema add-table` — and is fixed there as well. (An earlier version of this note also named a `restore --data-only` flag; no such flag exists.)

### Compatibility

**Encrypted backups only, and read this before upgrading a fleet in stages.** A backup written by this version records format version 9 and cannot be read by an older sluice. Restoring an existing chain with this version is unaffected — it opens exactly as before, because the format version recorded in each manifest decides the encoding, never the version of the binary reading it. A *resumed* backup also keeps the version its existing chunks were written under.

**But EXTENDING an older chain with this version makes the whole chain unreadable by an older binary.** A single `backup incremental` stamps its new segment 9 while the root keeps its original version, and an older sluice then restores nothing from that directory — including the untouched older segments it wrote itself, because a chain is only as readable as its newest link. This is a correction: the first published version of these notes said existing chains were unaffected in both directions, which is true only for a chain nobody extends. There is no in-product escape today, so if any binary that must be able to restore is still on an older version, finish upgrading it before taking an incremental with this one. The reverse direction is safe by construction — an older binary refuses to extend a version-9 chain and writes nothing. Tracked as roadmap item 90.
