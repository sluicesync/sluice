...
> ⚠️ **Superseded by [v0.99.279](https://github.com/sluicesync/sluice/releases/tag/v0.99.279) — silent-loss fix. Upgrade.** A post-release audit found that `sync --where` **string** filters mis-evaluate on **PAD SPACE collations** (the pre-8.0 / MariaDB / legacy defaults — `utf8mb4_general_ci`, `utf8mb4_bin`, `latin1_swedish_ci`, …): the client-side CDC comparator ignored the collation's trailing-space (PAD) semantics, so a value differing only by trailing whitespace (`'EU'` vs `'EU '`) could be silently dropped or leaked in continuous filtered sync. **Not affected:** the modern MySQL-8 default `utf8mb4_0900_ai_ci` (NO PAD) and Postgres default collations; **`migrate --where`** at all (source-evaluated); non-string filters. **Fixed in v0.99.279** (with a real-MySQL family-matrix gate). Details: audit `workspace/repo-audit-2026-07-18.md` F0-1.
