...
Both carry the marker `UNVERIFIED-INSTANCE-IDENTITY`, so every chain currently resuming without the identity check can be found in a single grep. The guard itself — `verifySourceInstanceIdentity` — is otherwise unchanged: the same inputs resume, and the same inputs refuse.
...
Choosing the weaker arm silently, on MySQL 8's default, left operators with no way to know they were on it. There is now one INFO per CDC open that says so, and says plainly that file/pos is supported and correct. It carries the marker `POSITION-MODE`, so it is greppable the same way the identity warning is.
