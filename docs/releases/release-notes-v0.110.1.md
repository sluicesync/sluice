...
A zone-scoped IPv6 address (`fe80::1%eth0`) and a MAC literal wider than six bytes (`08:00:2b:01:02:03:04:05`) both render back to themselves, so they passed the canonical-spelling gate and then matched no row for the life of the stream — the same silent-drop shape as the rendering bug, reached a different way. Neither server can store a zone at all, and a Postgres `macaddr` holds six bytes, so in both cases no stored value could ever equal the literal. Both are now refused at sync-start.
...
- **Anyone using `--where` on a `macaddr` column, or on an address with an IPv6 zone.** These predicates were matching nothing and are now refused. Note the `macaddr8` limitation above.
- **Anyone migrating to SQLite or D1 from a MySQL source whose schema uses index prefix lengths.** You will now get a refusal naming the rewrite instead of a silently weaker constraint.
