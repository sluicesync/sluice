...
> **Correction (2026-09-04).** Three sentences in the publication-exposure entry below are wrong, and two of them are wrong in the direction that hides harm. Both are **prose about the Postgres publication warnings only** — the `d1-trigger` fix that is the reason for this release is unaffected, and nothing about it needs re-reading. Fixed in **v0.141.2**.
>
> **"Both warnings now say not to, and point at `sluice sync decommission` instead."** One does. The other — the engine-local message, which serves both `backup full --chain-slot` and *any stream recreating a missing publication, warm resume included* — never named `sluice sync decommission` at all, and spoke in backup vocabulary throughout. A plain `sync start` printed "a chain slot needs a database-wide publication" and "`--chain-slot` keeps the publication after the run" to an operator who had run no backup and passed no such flag. Found by this release's own regression cycle (Bug 269). v0.141.2 gives each door its own wording and adds an AST roster over the call sites.
>
> **"Doing that on a running stream pins the replication slot's restart position behind the drop, and the stream then fails to resume."** Not always, and the exception is the dangerous one. The cycle isolated it on a single variable — whether anything was **written** between the drop and the resume. With a row written, the resume dies as described. With nothing written, it **succeeds**: the publication silently widens to `FOR ALL TABLES`, the stream replicates the next change normally and reports nothing wrong, and every keyless table in the database begins refusing `UPDATE` from that moment. So the entry described the loud branch of a fork as though it were the whole outcome, which left the quiet, worse branch unmentioned (Bug 270). The advice not to drop the publication stands, and is now stronger, not weaker.
>
> **And the remedy that sentence points at does less than it says.** "…which retires the slot and the publication together" is true only for a stream that had its OWN publication. The shared default `sluice_pub` is deliberately never dropped — other streams may read through it — and that is the dominant configuration at these doors. So an operator who ran `sluice sync decommission` got exit 0, a report saying the slot was removed, and the `FOR ALL TABLES` publication that was the whole subject of the warning still in place, still refusing `UPDATE` on every keyless table in the database. Check with `SELECT pubname, puballtables FROM pg_publication`. The decommission report itself was always honest — it says the shared default was not removed — so what was wrong is the warning that sent you there. This claim is specific to v0.141.1 and to the multi-schema `sync start` warning: v0.141.0's version of that remedy named no command at all. Found by the pre-tag review of v0.141.2, which caught it being copied onto a second door.
>
> Worth saying plainly, since this is the second banner in two days: both of the first two errors are absolutes. The v0.141.0 comment said this conversion happened "at exit 0", v0.141.1 corrected it to "the stream then fails to resume", and each rewrite replaced one certainty with another when what the behaviour needed was a condition.
