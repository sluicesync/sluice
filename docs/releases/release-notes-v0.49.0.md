...
Note: **FK constraint names are NOT affected by this fix.** Empirical testing on the validation rig (two rounds — synthetic + the operator's literal Vitess-renamed examples) showed sluice's PG FK emitter does NOT prepend the table name, so FK names land verbatim from source. No parallel `pgConstraintName` helper needed. See `C:\code\sluice-validation\BUG-CATALOG.md` entry 3 for the full diagnostic.
...
- **End-to-end re-verification via the validation rig** at `C:\code\sluice-validation\`. Both `start_sync_mysql_dest.ps1 -AllTables` (re-triggers #25 pre-fix) and `start_sync_pg_dest.ps1 -AllTables` (re-triggers #26 pre-fix) should now succeed end-to-end with the v0.49.0 binary.
