...
- **Two test-only repairs, both caught by this release's own tag run.** The grow-gate evidence pin compared wall-clock sleeps with a tolerance funded from one of them, which fails on Windows where the timer granularity is most of a 20 ms budget — it now drives the gate's existing clock seams, so the assertion is an equality rather than a band, and a mutation run showed the old band would have *accepted* a gate that halved every early hold. And the Neki verification suite now preflights its credentials: a PlanetScale org-scoped `404` means wrong-org, bad-token, or token-scoped-elsewhere indistinguishably, and the suite reported it once per test, each time attached to a billable create.
