...
The reported sentence is matched, and so is the CLASS it belongs to — because the sentence itself is unconfirmed. It appears nowhere in upstream Vitess v0.24.2 (checked), and the report's text reads as two messages merged, so anchoring on that literal alone risked a fix that is simply inert. A message that names a Vitess component AND carries a transport-loss phrase is now treated as a dropped connection whatever the surrounding prose. Both halves are required deliberately: an `Error 1105` can echo the statement that failed, and a false transient is worse than a missed one here — it also arms a duplicate-tolerance path on the retry. One classifier serves both the cold-copy retry and the CDC applier, so both are covered.
...
**A PlanetScale DEV branch was mistaken for a tiny instance, throttling the copy to a quarter of its parallelism, silently.** sluice infers a copy-parallelism ceiling from `@@innodb_buffer_pool_size`, which scales by plan tier and is the only credential-free tier signal available. A branch that returned 32 MiB — far below the smallest tier sluice has ever measured — was read as the smallest instance, capping table parallelism at 2 instead of 8.

The cause is now measured rather than inferred. Against real PlanetScale: a **production** PS-10 branch reports 134217728 (128 MiB), matching the tier table exactly, while a **dev** branch on the same database reports 33554432 (32 MiB) — the field report's value precisely. A dev branch reports a small fixed pool that does not track the database's plan tier — the two branches even report different Vitess versions (8.4.6 vs 8.4.9), so they are not the same infrastructure. Every measurement the heuristic was built from had been taken on a production branch, which is why the exception went unnoticed.
...
- **Anyone running sluice against a PlanetScale DEV branch** — you were being throttled to a quarter of your copy parallelism, and nothing said so. Point sluice at the production branch for tier-sized parallelism; either way it no longer caps on a dev branch's reading, and it now names the tier decision in the log when it does cap.
