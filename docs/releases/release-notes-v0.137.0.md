...
> **Correction (2026-09-01, recorded alongside v0.137.1; no binary change was involved — the shipped behaviour was already the narrower one):** the headline above and the Compatibility section below both said "Until setup is re-run, an existing install has no recorded provenance and opens with a `STALE-CAPTURE-FUNCTION` warning at every resume." That over-claims. The warning fires only when an install's capture function definitions **differ** from what this binary renders. An install created by a recent sluice whose definitions are already byte-identical — a v0.136.0 install, for instance — opens **silent**, with no prompt at all, even though setup never recorded provenance for it. Those installs are not broken and capture correctly; the residual is that a later hand-edit of a capture function can only be **warned** about there, never refused, because there is no recorded digest to fail against. Re-running `sluice trigger setup` once still records the provenance and closes that gap, and the SEC-1 `INSECURE-CAPTURE-FUNCTION` warning already steers most affected installs to the same re-run. Found by the v0.137.0 regression cycle (Bug 259). Whether that case should warn is filed as an open design call rather than changed silently.

**The trigger-CDC capture door stops taking a trigger's word for it.** It verified that each capture trigger existed and pointed at the expected function, then trusted that function's contents entirely — so anyone able to replace a function body could leave every name and binding intact and silently stop recording changes. The door now grades what the triggers actually execute, and tells an old install from a tampered one using provenance rather than guesswork. Plus a database-wide echo-loop probe, a `trigger setup` that covers the whole install, and a statement-DML refusal that no longer prints operator data. **Every existing pgtrigger install warns until `sluice trigger setup` is re-run once — see Compatibility.** (Over-claimed; see the correction above.)
