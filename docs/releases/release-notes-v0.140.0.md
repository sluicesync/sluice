...
>
> **A second, narrower correction to the same entry.** It says v0.139.0's refusal "only fired on a standby someone had set to logical", which reads as a claim about every path. The v0.140.0 regression cycle measured otherwise: `sync start` already refused correctly at any `wal_level`, landing zero rows on both binaries, because it reaches the coded refusal at publication-scope time. The ordering defect was specific to `backup full`, which is the path the rest of the entry describes. The fix is real and worth having; it did less than the opening sentence implies.
