...
**And the residual that matters most on upgrade day, which the first cut of these notes left to inference: a chain signed by v0.107.0 or earlier is still schema-forgeable.** Signatures are verified under the canonicalization *they were written with*, which is what keeps every existing chain readable — and it means a pre-v5 signature covers no schema bytes, exactly as before. So if all your chains predate this release, verifying them here reports `signature valid` and you have the old exposure, not the new protection. Post-release testing confirmed this end to end: a forged v4 manifest verifies rc=0 under `--require-signature` and restores with constraints stripped and row-level security flipped off. **The remedy is to take a fresh full with this release, or re-sign.** A warning naming this at verification time is landing in the next patch; it should have been in this one.
