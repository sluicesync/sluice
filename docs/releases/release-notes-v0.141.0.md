...
Two paths create that publication and **both** now warn. The multi-schema `sync start` is the obvious one. The other is `backup full --chain-slot`, which is the worse case and had neither a refusal nor a warning: it deliberately keeps the publication so the chain's incrementals can decode through it, so the exposure outlives the run.
