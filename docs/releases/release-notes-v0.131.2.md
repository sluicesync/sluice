...
**Silent data-loss — SQLite / D1 floating-point values were silently rounded on the trigger-CDC capture path.** sluice captured SQLite and Cloudflare D1 `REAL` values by rendering them to text with `format('%.17g', …)`, trusting the C-`printf` guarantee that 17 significant digits round-trip an IEEE-754 double. But SQLite's `printf`/`format()` is not C's `printf`: its `%g`/`%.Ng` conversion **caps output at 16 significant digits** (raised to 26 only by the `!` alternate-form flag), and 16 digits do not round-trip every double — 17 are needed in the worst case. So `%.17g` silently clamped to 16, and **2295 of 5007 sampled doubles failed to round-trip** (measured on both the bundled driver and real Cloudflare D1): `0.30000000000000004` renders as `"0.3"`. This is longstanding SQLite behaviour, not a version regression — `%.16g` through `%.25g` all emit the same 16-digit render, and the pre-existing test passed only because its seed (π) happens to round-trip at 16 digits. Every `REAL` flowing through the affected paths — the D1 reader, `--stage-local` D1 staging, and both the SQLite and D1 trigger-CDC capture lanes — was silently altered at exit 0, and a `REAL` at `float64` max rendered out-of-range and tore the CDC stream down. Fixed by rendering with the alternate-form flag `format('%!.20g', …)`, which lifts the cap so 17+ digits emit — **0 of 5007 misses, confirmed live on D1.** The local SQLite reader that scans `REAL` directly as a `float64` was never affected, and Go-side float rendering already used shortest-exact formatting.
