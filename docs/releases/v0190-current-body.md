...
> **Known issue — Bug 37 (`sluice backup stream stop` cooperative-stop reliability). ✅ CLOSED IN v0.19.1.** The `BackupStream`'s stop-signal observation path (the `--target=<url>` cross-machine `sluice backup stream stop` companion command) did not fire reliably on heavily-contended systems (CI under `-race` reproduces; production-equivalent systems may also be affected). **Root cause** turned out to be a heartbeat-clobber race: the stream's per-rollover heartbeat write to `stream_state.json` was last-writer-wins overwriting the operator's `stop_requested_at` whenever `RequestStreamStop` landed within the rollover-cycle window. **v0.19.1 fix** is two layers: (1) heartbeat read-modify-write that copies any concurrent `stop_requested_at` forward — closes the file-poll clobber across all `BackupStore` backends; (2) an in-process channel registry so same-process stops are signalled instantaneously (~5ms vs 1s+ for file poll). Cross-process stops still go through the file (now race-fixed). The CDC pump, chain correctness, rollover policy, and signal-based shutdown were NOT affected on v0.19.0 — the stream still produced complete chains, and SIGTERM/SIGINT to the process triggered the graceful-drain path correctly. **For v0.19.0 operators** (now an upgrade prompt rather than a workaround): upgrade to v0.19.1 to get reliable `sluice backup stream stop`. The SIGTERM workaround (`kill -TERM <pid>`, k8s `lifecycle.preStop`, systemd `KillSignal=SIGTERM`) still works on v0.19.0 if upgrade isn't immediately feasible — same-machine shutdown was always reliable; only the cross-machine convenience surface was at risk.
