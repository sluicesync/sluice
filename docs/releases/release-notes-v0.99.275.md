...
- **Test & coverage hardening (2026-07-17 confirming-audit tail — no user-facing behavior change):**
  ...
  - The Vitess rolling-upgrade **chaos** cold-image-pull flake was fixed and **confirmed green** (both hardcoded upgrade tags are now warmed into the local cache off the tablet-ping path, and the recreate ping budget is widened). Two other long-red extended-suites legs had their **surface blocker** fixed, each of which then **unmasked a separate deeper pre-existing non-product issue** (both had been hidden behind the surface blocker for ~6 cycles) now filed as follow-ups:
    - **reshard** — the A/B tests' stale pre-`StreamChanges` shard assertion was moved to after the stream opens (the connection-free-reader refactor had left it reading an empty shard set), which revealed that post-reshard discovery now returns the *drained* source shard alongside the two new ones (3 shards vs the tests' `want 2`). The streamer/oracle reshard-follow tests still pass exactly-once (no gap, no dup), so this is a test-expectation / discovery-semantics question, not a streaming defect.
    - **real-world corpus** — the fetch URL was un-404'd (`vitessio/vitess` moved `examples/local` → `examples/common`), which revealed accumulated congruence drift: that suite fetches upstream schemas fresh each run and its benign-diff allowlists have gone stale against evolved upstream DDL. Same-engine dump-parity passes and the loud cross-engine refusals are correct — sluice's behavior is fine; the allowlists need a refresh.
