...
Worth knowing if you run PlanetScale or Vitess: this is not new with the encrypted-format change. CDC segments from a VStream source have raised a chain's floor the same unconditional way since v0.99.228, so a VStream chain rooted before that release has been in this state for a while. The warning is version-agnostic and covers it.
