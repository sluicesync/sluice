...
**What verify was already catching, for calibration:** a corrupted chunk, a deleted chunk, a severed lineage link, an encrypted chain that claims to be plaintext, and — with `--verify-key` — signature failures. Signing a chain (`--sign` / `--sign-key` + `--require-signature`) already covered this class and still does; it is opt-in, and the check it backstops should not have been.

**⚠ CORRECTION (2026-07-29, after publication): the sentence "the manifest fingerprint was the only restore preflight it skipped" was wrong.** It was a claim about a list, and the v0.104.5 regression cycle did what the claim invited — enumerated the list — and found a second one: `verifyBackupIDs`, which restore runs on chain links *and* on a bare full, was still restore-only. A manifest whose recorded `BackupID` no longer matched its content therefore still verified `all chunks OK` rc=0 while restore refused it with rc=3 and zero rows. Filed as Bug 218 and **fixed in v0.104.6**, along with the structural cause: v0.104.5 shared the *predicate* that decides which lineage shapes get checked but left the *list* of checks duplicated between the two commands, so the class stayed reachable while one instance closed. Both checks now come from one function, so the next preflight added reaches both commands or neither.
