...
4. Publish (`gh release edit vX.Y.Z --draft=false`) only after the six-check gate passes: `release.yml` green on the tag; `ci.yml` green on the tag (or its descendant `main` commit); release assets present; curated (non-auto-generated) notes in place; the tag unique on the remote; and the `Filtered move-OUT gate (tag)` job in `vitess-version-matrix.yml` green on the tag. That last one is the check this list omitted while calling itself a five-check gate — it boots a real Vitess cluster and is the only end-to-end gate on filtered sync, so publishing without it is how a filtered-sync Critical shipped once before. Budget ~7–10 minutes; a run finishing in well under a minute is the shape to distrust, since a boot failure that skipped the crux family is exactly what its vacuous-green guard exists to catch. Never publish with a gate check failing or unverified.
