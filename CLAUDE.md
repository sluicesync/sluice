...
6. **Publish via Option B gate.** All six checks must pass before `gh release edit vX.Y.Z --draft=false`:
   ...
   6. **Filtered move-OUT cluster gate** — the `filtered-tag-gate` job in `vitess-version-matrix.yml` (check name `Filtered move-OUT gate (tag)`) on the tag → success. It boots a real Vitess cluster and runs the `TestVitessClusterFilteredSync[WarmResume]` move-OUT crux (both cold-start and warm-resume) — a real wire behavior with no unit pin. It is deliberately a **tag-time publish gate, NOT a branch-protection PR check**: tag pushes aren't PRs, and booting a Vitess cluster on every PR is prohibitive, so the enforcement lives here in the publish checklist rather than in the six required PR-merge checks. This is the audit F-T2 fix — a filtered-sync Critical must not be publishable before its only end-to-end gate reports (v0.99.278 was published ~2 days before the then-weekly-only cluster test could run, and a Critical shipped). It runs on every tag; watch it alongside `release.yml`/`ci.yml` and budget ~30–60m for it. `gh run list --workflow=vitess-version-matrix.yml --event=push` finds the run for the tag; require its `Filtered move-OUT gate (tag)` job green.
