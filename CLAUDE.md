...
**Moving or scoping a REFUSAL is the sibling-miss shape that recurred three releases running (2026-08-17, the Tier-3 audit's top process finding).** The most expensive instances above are all fixes; this is the specific *fix shape* that keeps leaking a sibling — a refusal/door that gets **relocated or narrowed** reaches one path and silently stops covering another that shared it. Three consecutive releases, same shape: CDCPOS-1's XA re-open shipped filter-blind and reached the wrong scope → **Bug 246**; M-2's missing-database halt reached the schema-absent case but fell through to the silent skip on a *probe error* → **SL-1**; H-2's sharded-target door moved from `OpenSchemaWriter` to the create chokepoint and reached migrate but not the sync-cold-start/`add-table` create path → **Bug 253**. Each was a door that MOVED. So when a fix relocates or scopes a refusal, the enumeration is not "which engines" but **"which call PATHS reached the old door, and does each still reach the new one?"** — list the callers of both the old and new site in the commit, and for each state reached-or-now-uncovered. A door that moved without that caller list is the finding. The mechanical form is `TestShardedTargetDoorRoster_EveryCallSiteClassified` (H-2's own AST caller roster) — prefer it to a promise when the door has more than two callers.
...
6. **Publish via Option B gate.** All six checks must pass before `gh release edit vX.Y.Z --draft=false`:
   ...
   4. Release notes body present and curated (not the auto-generated commit-list)
      - **4b. Notes-claims-vs-tag-tree gate** — `sh scripts/check-notes-claims.sh vX.Y.Z docs/releases/release-notes-vX.Y.Z.md` → OK. Extracts the load-bearing identifiers the notes name (SLUICE-E codes, file names, Test names, ALL-CAPS-HYPHENATED markers) and asserts each exists in the TAG's tree, with a ≥3-identifier anti-vacuity floor and an inline `<!-- notes-claims-exempt: … -->` escape for legitimately-not-in-tree mentions. Exists because v0.132.1's published notes claimed code the tag did not carry (a landing session's cwd had drifted into an agent worktree) and check 4 passed — the notes WERE curated, just not true at the tag. Identifier-level only: a behavioral claim about a pre-existing symbol is out of its reach, so the cwd-anchored landing verification (audit backlog 2026-08-27) still applies.
