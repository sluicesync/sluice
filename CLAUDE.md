...
Also glance at the standing **"extended-suites: scheduled run is RED"** issue (auto-filed by `extended-suites.yml`'s `report-red` job, TESTCI-1) before publishing: a red weekly is not by itself a publish blocker — the suites are non-required by design — but it must be *knowingly accepted*, never unseen; three weeks of unconsumed reds is what motivated the job.
