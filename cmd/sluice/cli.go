...
type SyncStartCmd struct {
	...
	NoAutoResnapshot bool `help:"Suppress the automatic re-snapshot when a resume hits a purged/invalid source position. By default (parity with the self-hosted binlog path, ADR-0093) a resume from a position older than the source's retained binlogs — routine on PlanetScale's binlog-retention window — auto-recovers with a fresh, NON-destructive cold-start re-snapshot (the idempotent copy absorbs the overlap; the target is not dropped). With this flag set, sluice instead fails LOUDLY with an actionable error naming the recovery commands (--restart-from-scratch / --reset-target-data), so the operator decides — useful when a full re-snapshot is expensive (very large tables) and should be a deliberate choice. Gates BOTH the pre-flight fall-through and the reactive VStream recovery."`
...
func (s *SyncStartCmd) Run(g *Globals) error {
	...
	streamer := &pipeline.Streamer{
		...
		// position (parity with the binlog path); --no-auto-resnapshot
		// flips it to a loud terminal failure instead.
		AutoResnapshotOnInvalidPosition: !s.NoAutoResnapshot,
