...
type SyncStartCmd struct {
	...
	BackfillAddedColumn bool `help:"ADR-0058 §1c — opt-in source-side bounded backfill of already-shipped target rows after a forwarded ADD COLUMN lands. Default OFF (existing target rows carry the column's DEFAULT, NULL if none). When set, the streamer issues a bounded PK-cursor SELECT (pk, new_col) against the source and emits synthetic UPDATE events to populate the new column with per-row source values. Only consulted while schema-change forwarding is actually running: --schema-changes must not be 'refuse' (it defaults to 'forward', so this is ON by default), and the backfill is a silent no-op under --inject-shard-column (Shape A forwards ADD COLUMN via the lease instead) or on a multi-database stream (ADR-0091 forwarding is single-database only). Large tables: backfill cost is proportional to the table's row count on the source — operators must opt in knowingly."`
