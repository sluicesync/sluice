...
type SyncStartCmd struct {
	...
	ApplyConcurrency int `help:"MySQL target only (ADR-0104 item 23(c)): the key-hash apply LANE count W. The merged CDC change stream is fanned across W in-order apply lanes by primary-key hash (same key → same lane → applied in source order, so dependent INSERT→UPDATE→DELETE on a row never reorder), each lane committing CONCURRENTLY on a dedicated backend with its OWN AIMD batch-size controller. On a high-latency cross-region link a serial applier is RTT-bound and falls below the source write rate, causing the per-shard MinimizeSkew wedge; concurrent lanes lift aggregate apply throughput toward W× and keep both shards drained (live-validated ~4× on a 2-shard Vitess→PlanetScale-MySQL link). The resume position advances only to a source boundary durable across ALL lanes (exactly-once for keyed tables; keyless stays at-least-once). A PlanetScale transaction-killer abort is handled IN-LANE — the lane's controller shrinks and the batch is split-and-retried idempotently without restarting the stream. 0/1 (default) = serial, byte-identical. Set W>1 (e.g. 4) to engage; sluice opens exactly W dedicated backends (the connection budget — no auto-clamp). Keep --apply-batch-size at a sane value (the default is fine): an absurdly high ceiling can make the controller lag on a tx-killer-heavy target (safe — no data loss — but slow). Inert on Postgres targets (ADR-0092 statement pipelining is used there)." default:"0" placeholder:"W"`
