...
type SyncStartCmd struct {
	...
	ApplyPipelineDepth int `help:"MySQL target only (ADR-0104 Phase 1): overlap cross-region CDC-apply commit RTTs across an in-flight window of W independent transactions on a dedicated W-backend pool, committed STRICTLY in source order. On a high-latency (cross-region) link a serial applier is RTT-bound (~1/RTT) and falls below the source rate, causing the per-shard MinimizeSkew wedge; this lifts the aggregate apply ceiling toward W/RTT while keeping the commit linearization point in source order (exactly-once preserved). 0/1 (default) = serial, byte-identical to the pre-ADR-0104 path. Set W>1 (e.g. 4) to engage; the operator-set W is the connection budget — sluice opens exactly W backends (no auto-clamp; MySQL has no connection-budget prober). Keep W conservative on PlanetScale (vttablet limits + the 20s tx-killer interact with W; the AIMD controller still governs per-transaction batch SIZE). Inert on Postgres targets (ADR-0092's within-tx statement pipelining is used there instead). SUPERSEDED by --apply-concurrency (commit-only overlap was measured ineffective — left for fallback)." default:"0" placeholder:"W"`

	ApplyConcurrency int `help:"MySQL target only (ADR-0104 item 23(c)): the key-hash apply LANE count W. The merged CDC change stream is fanned across W in-order apply lanes by primary-key hash (same key → same lane → applied in source order, so dependent INSERT→UPDATE→DELETE on a row never reorder), each lane committing CONCURRENTLY on a dedicated backend. On a high-latency cross-region link a serial applier is RTT-bound and falls below the source write rate, causing the per-shard MinimizeSkew wedge; concurrent lanes lift aggregate apply throughput toward W× and keep both shards drained. The resume position advances only to a source boundary durable across ALL lanes (exactly-once for keyed tables; keyless stays at-least-once). 0/1 (default) = serial, byte-identical. Set W>1 (e.g. 4) to engage; sluice opens exactly W dedicated backends (the connection budget — no auto-clamp). This is the LIVE successor to --apply-pipeline-depth (whose commit-only overlap was measured ineffective). Inert on Postgres targets (ADR-0092 statement pipelining is used there)." default:"0" placeholder:"W"`
