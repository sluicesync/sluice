...
type MigrateCmd struct {
	...
	Redact          []string `help:"Redact a PII column (repeatable). Format: '[schema.]table.column=STRATEGY[:options]'. Strategies: null (NULLABLE columns only), static:<value>, hash:sha256 (deterministic), hash:hmac-sha256 (requires --redact-key-source), truncate:<n> (first N runes; string columns only), mask:inner:<m1>,<m2>[,<char>] (keep first M1 + last M2 runes, mask middle; default char 'X'), mask:outer:<m1>,<m2>[,<char>] (mask first M1 + last M2 runes, keep middle). Examples: --redact users.email=hash:sha256, --redact users.phone=truncate:4, --redact users.pan=mask:inner:4,4. Phase 1.5: bulk-copy + CDC apply paths both honour --redact (v0.54.0+). YAML form available under config 'redactions:' block. See docs/dev/notes/prep-pii-redaction-phase-1.md." placeholder:"RULE" sep:"none"`
