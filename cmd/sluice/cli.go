...
type SyncFromBackupCmd struct {
	...
	PlanetScaleServiceTokenID string        `name:"planetscale-service-token-id" help:"PlanetScale service-token ID (branch + deploy-request scopes) for the ADR-0148 index-build fallback. Prefer the env var so it never lands in shell history." env:"PLANETSCALE_SERVICE_TOKEN_ID" placeholder:"ID"`
	PlanetScaleServiceToken   string        `name:"planetscale-service-token" help:"PlanetScale service-token secret for the ADR-0148 index-build fallback. Set via the env var (never on the command line); never logged." env:"PLANETSCALE_SERVICE_TOKEN" placeholder:"SECRET"`
	PlanetScaleDeployTimeout  time.Duration `name:"planetscale-deploy-timeout" help:"Per-deploy-request wait for the ADR-0148 index-build fallback. DEFAULT 0 = wait indefinitely for the deploy to finish, which is what a large table needs (its index builds via VReplication — async, unbounded by errno 3024, and genuinely hours on a ~100 GB table; progress, ETA and throttling are logged as it runs). Terminal failure states still fail fast. Set a duration to stop waiting after it — nothing fails, the deploy keeps running in PlanetScale and re-running the broker cold start picks up: the index phase re-probes and rebuilds only what is still missing." default:"0" placeholder:"DUR"`
...
type SyncStartCmd struct {
	...
	PublicationName string `help:"Publication name suffix for engines with a shared source-side filter object (Postgres). Same convention as --slot-name: sluice prepends 'sluice_' if absent, so '--publication-name wave1' manages 'sluice_wave1' and every sluice publication is findable with 'pg_publication WHERE pubname LIKE sluice\\_%'. When OMITTED, the stream's publication is resolved by the recorded-name ratchet (ADR-0176 prerequisite): a name recorded in the stream's sluice_cdc_state row is reused on resume; a genuinely NEW stream with a --where row filter gets a per-stream default ('sluice_<stream-id>', normalized); every other stream keeps the shared 'sluice_pub' default. An explicit flag always wins and updates the record (a WARN names both when they differ). REQUIRED (alongside --slot-name) to run multiple concurrent PG-source streams with DIFFERENT --include-table scopes: the slot is per-stream but the publication is NOT, so sharing one makes a second stream's cold start silently de-scope the first — sluice refuses that rescope loudly (SLUICE-E-CDC-PUBLICATION-SCOPE-CONFLICT) rather than performing it. Engines without a publication (MySQL family) silently ignore this flag. See ADR-0175." placeholder:"NAME"`
	...
	CopyFanoutDegree int `help:"VStream/CDC snapshot cold-start (ADR-0097, PlanetScale-MySQL target) only: WRITE-side fan-out — the single incoming snapshot row stream is PK-hash-partitioned out to N concurrent batched-INSERT writer workers, each on its own connection, to beat the single cross-region-RTT-bound INSERT connection vtgate forces (it blocks LOAD DATA). 0 (default) = auto: 4; 1 disables fan-out (serial). Bounded by the target connection budget / --max-target-connections, and — on a PlanetScale target whose plan tier the buffer-pool probe cannot read, which is what a DEV branch looks like — by a conservative fan-out ceiling of 2 that the cold-start logs when it binds (roadmap item 144). Like every other auto-cap, that only ever LOWERS this value. Inert on the FAST cold-start path and on no-PK tables. See ADR-0097." default:"0" placeholder:"N"`
	...
	ApplyRetryBackoffCap  time.Duration `aliases:"retry-backoff-cap" help:"Upper bound on each per-attempt backoff interval. ADR-0038. Defaults to 30s. With 8 attempts and default base the slept sequence is 100ms → 200ms → 400ms → 800ms → 1.6s → 3.2s → 6.4s (7 sleeps totalling 12.7s; the 8th failure exhausts the budget without sleeping, so the cap never binds at the default attempt count). Alias: --retry-backoff-cap (the backup-stream spelling)." default:"30s" placeholder:"DUR"`
	...
	NotifyDeadTupleRatio      float64       `help:"Alert when the Postgres TARGET's worst-table dead-tuple ratio (n_dead_tup/(n_dead_tup+n_live_tup), 0-1) is at or above this fraction — autovacuum falling behind the write load (a sluice bulk copy or long CDC catch-up is exactly that shape). Probed from the target's own catalog (pg_stat_user_tables): UNGATED from PlanetScale telemetry, needs only a --notify-* sink. Postgres targets only (WARNs once and stays inert elsewhere). Small tables below a 1000-dead-tuple floor are ignored so a scratch table cannot page. 0 (default) disables. Edge-triggered + cooldown'd; advisory + failure-isolated." placeholder:"FRAC"`
	NotifyXIDAge              float64       `help:"Alert when the Postgres TARGET database's age(datfrozenxid) — transaction-ID wraparound headroom consumed — is at or above this many XIDs (e.g. 1000000000; Postgres force-stops near ~2.1B, autovacuum normally holds this near 200M). Same probing + gating as --notify-dead-tuple-ratio. 0 (default) disables." placeholder:"XIDS"`
	...
	NotifySlotHealth          bool          `help:"Fire notifications (ADR-0059, roadmap item 64a) to the configured --notify-* sink(s) when the source Postgres replication slot crosses a health threshold — WAL retention pressure at 70% (warning) / 85% (critical) of max_slot_wal_keep_size, 30m slot inactivity (warning), wal_status 'unreserved' (critical — invalidation at the next checkpoint), or the terminal events themselves: wal_status 'lost' and a slot dropped mid-stream each page critical exactly once and then latch (unrecoverable; a re-snapshot is the only recovery, and no false 'cleared' follows). A sustained slot-health probe outage (5 consecutive failures) pages a warning — the net never goes silently blind. The alert carries the slot facts + remediation. On by default (inert unless a sink is configured; only fires for Postgres logical-replication sources — the structured slog WARNs fire regardless). Pass --notify-slot-health=false to keep the slog WARNs only. Advisory + failure-isolated: a delivery problem never affects the running sync." default:"true"`
	...
	AutoPruneChangeLog bool          `help:"Automatically reap consumed rows from a trigger-CDC source's sluice_change_log in-stream, so it doesn't grow unbounded on a continuous sync (ADR-0137 Phase B, Bug 165). Applies only to trigger-CDC sources (postgres-trigger / sqlite-trigger / d1-trigger); a no-op for every other engine (they have no change-log). MULTI-STREAM SAFE (roadmap 115): every trigger-CDC sync records its applied frontier in the source's sluice_change_log_consumers table, and the prune cuts at the MIN across all registered consumers, so a slower sync sharing the source never loses rows it has not read. Two caveats: a source whose change log predates that registry is REFUSED (nothing is pruned) until you re-run 'sluice trigger setup' to migrate it, and a peer sync running an older sluice never registers at all — upgrade every sync on a shared source before enabling this. Off by default — auto-DELETEing source rows is an explicit opt-in. Only durably-applied rows (below the target's persisted CDC frontier, minus --auto-prune-keep) are removed, so warm-resume is never starved; a prune failure is logged and swallowed, never affecting the sync. Equivalent to scheduling 'sluice trigger prune' on a cron, without the cron."`
	...
	BackfillAddedColumn bool `help:"DEPRECATED — the added-column backfill is now ON by default (see --no-backfill-added-column). Accepted so existing scripts keep working; setting it logs a notice and changes nothing. Removed in a future release."`

	NoBackfillAddedColumn bool `help:"ADR-0058 §1c — opt OUT of the added-column backfill. By default, after a forwarded ADD COLUMN lands on the target, sluice reads the new column's values from the SOURCE for the rows the target already held (a bounded primary-key-ordered SELECT) and writes them to the target, because the target's own fill is not what the source holds whenever the source's DEFAULT was dropped or changed after the ADD COLUMN (Django emits exactly that for every AddField with a default) or is non-constant (now(), gen_random_uuid()). The stream waits behind the backfill, which logs its progress; its cost is one pass over the table on the source plus one UPDATE per row on the target. Opting out skips that pass: the rows the target already held keep the target's fill (the carried DEFAULT, NULL if none) and a WARN names each column so affected. Applies to every stream that forwards ADD COLUMN — single-stream (--schema-changes=forward, the default) and Shape A (--inject-shard-column, where each shard's stream fills only its own shard's rows); a multi-database stream does not forward DDL, so there is nothing to backfill."`
...
func (s *SyncStartCmd) run(g *Globals, env *envelopeRun) error {
	...
	streamer := &pipeline.Streamer{
		...
		NotifyDeadTupleRatio:      s.NotifyDeadTupleRatio,
		NotifyXIDAge:              s.NotifyXIDAge,
		...
		// Default-on backfill (ADR-0058 §1c); the flag is the opt-out.
		SuppressAddedColumnBackfill: s.NoBackfillAddedColumn,
