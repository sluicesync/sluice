...
type EncryptionFlags struct {
	...
	EncryptionPassphraseEnv  string `envvar:"name" name:"encryption-passphrase-env" help:"Read encryption passphrase from this environment variable. Recommended over --encryption-passphrase for production." placeholder:"VAR"`
...
type BackupFullCmd struct {
	...
	Redact       []string `help:"Redact a PII column in backup chunks (repeatable). Format: '[schema.]table.column=STRATEGY[:options]'. Strategies: null, static:<v>, hash:sha256, hash:hmac-sha256[:<keyname>], truncate:<n>, mask:inner:<m1>,<m2>[,<char>], mask:outer:<m1>,<m2>[,<char>], mask:ssn, mask:pan, mask:pan-relaxed, mask:email, mask:ca-sin, mask:uk-nin, mask:iban, mask:uuid, randomize:int:<min>,<max>, randomize:email, randomize:us-phone, randomize:uuid, randomize:ssn, randomize:pan[:<brand>], randomize:ca-sin, randomize:uk-nin, randomize:iban[:<country-code>], randomize:dict:<name>, tokenize:dict:<name>[:<keyname>] (Phase 3 v0.61.0+, keyset-sourced Phase 4 v0.62.0+; dictionaries declared in YAML) — same set as 'sluice migrate --redact'. PII Phase 1.5 (v0.55.0+): redaction applies during chunk write, so the on-disk backup this command produces is PII-clean, and restoring it reproduces that redacted shape (restore does NOT re-apply redactions — they were applied at backup time). SCOPE, and it is the whole story: ONLY 'backup full' redacts. 'backup incremental' and 'backup stream' have no --redact flag and apply none — their change events come off the source's CDC pump verbatim — so a redacted chain is a series of FULLS, not a full plus incrementals. Since v0.144.0 the manifest records that this backup was redacted and both of those commands REFUSE to extend it (SLUICE-E-BACKUP-REDACTED-CHAIN) rather than silently appending plaintext; restore/chain-restore/verify/export-as-parquet/the from-backup broker refuse a chain whose links disagree; 'backup compact' refuses a redacted chain outright (it re-attributes one segment's manifest to several segments' merged data, which would launder the marker onto plaintext); and 'sync start --position-from-manifest' refuses unless the sync's own --redact policy matches the chain's, since resuming CDC off a redacted chain without it overwrites the restore's redacted values with plaintext on a live target. See docs/dev/notes/prep-pii-redaction-phase-1.md." placeholder:"RULE" sep:"none"`
	KeysetSource string   `envvar:"spec" help:"Operator keyset source (file:PATH | env:VARNAME | db:DSN) for keyset-using strategies (hash:hmac-sha256, tokenize:dict). PII Phase 4 (ADR-0041). Same forms as 'sluice migrate --keyset-source'." placeholder:"SRC"`
