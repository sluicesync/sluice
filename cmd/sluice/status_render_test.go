...
func TestRenderStatusText_DefaultShape(t *testing.T) {
	...
	err := renderStatus(&buf, streams, nil, nil, nil, statusRenderOpts{Format: "text"}, fixedNow)
	if err != nil {
...
func TestRenderStatusText_Empty(t *testing.T) {
	t.Run("no filter", func(t *testing.T) {
		var buf bytes.Buffer
		if err := renderStatus(&buf, nil, nil, nil, nil, statusRenderOpts{Format: "text"}, fixedNow); err != nil {
			t.Fatalf("renderStatus: %v", err)
		}
	...
	t.Run("with stream-id filter", func(t *testing.T) {
		...
		if err := renderStatus(&buf, nil, nil, nil, nil, opts, fixedNow); err != nil {
			t.Fatalf("renderStatus: %v", err)
		}
...
func TestRenderStatusJSON_Empty(t *testing.T) {
	var buf bytes.Buffer
	if err := renderStatus(&buf, nil, nil, nil, nil, statusRenderOpts{Format: "json"}, fixedNow); err != nil {
		t.Fatalf("renderStatus: %v", err)
	}
	var doc struct {
		Summary struct {
...
func TestRenderStatus_UnknownFormat(t *testing.T) {
	var buf bytes.Buffer
	err := renderStatus(&buf, nil, nil, nil, nil, statusRenderOpts{Format: "yaml"}, fixedNow)
	if err == nil {
...
func TestRenderStatusText_LeaseSummary(t *testing.T) {
	...
	if err := renderStatus(&buf, streams, leases, nil, nil, statusRenderOpts{Format: "text"}, fixedNow); err != nil {
		t.Fatalf("renderStatus: %v", err)
	}
	got := buf.String()
	for _, want := range []string{
...
func TestRenderStatusJSON_LeaseBlock(t *testing.T) {
	...
	if err := renderStatus(&buf, streams, leases, nil, nil, statusRenderOpts{Format: "json"}, fixedNow); err != nil {
		t.Fatalf("renderStatus: %v", err)
	}
