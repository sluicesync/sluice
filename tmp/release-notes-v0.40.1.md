# sluice v0.40.1 — cold-start persists the CDC anchor before the first batch

**Closes GitHub issue #15.** Before v0.40.1, the target's `sluice_cdc_state` row only existed after the first CDC batch committed successfully. Any failure between "bulk-copy complete; entering CDC mode" and that first batch wedged the operator:
...
- **Anyone running `sluice sync start` against PlanetScale-MySQL / Vitess targets**: **upgrade**. The #15 wedge required `--reset-target-data` recovery on every transient; v0.40.1 makes restarts clean warm-resumes.
...
GitHub issue #13 (Vitess tx-killer / PG serialisation transients exit the stream) remains open; the design lives in [ADR-0038](docs/adr/adr-0038-applier-retry-on-transient-errors.md). With v0.40.1 closing #15, a v0.40.0+ operator hit by an issue #13 transient can now restart and warm-resume to retry the failed batch, rather than re-bulk-copying the whole dataset. The retry policy in ADR-0038 is the next surface to ship — implementation in v0.41.0.
