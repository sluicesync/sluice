...
1. **Branch on the exit code first** (`AGENTS.md` / `docs/operator/error-codes.md`):
   ...
   - **1** — runtime failure, OR a `verify`/`diff`/`sync health` **drift/stale** result, OR `sync health` reporting durably **skipped tables** (`skipped_tables_tripped` in the JSON — the target lacks a table the stream carries, or the apply role's privileges on it were revoked, which reads identically; `schema add-table` or an explicit filter resolves the missing-table cause, restoring the grant resolves the revoked cause — no threshold applies). A real failure to fix, or a genuine data/lag difference to report (hand a drift to `fidelity-verify`). **Exception:** a `sync`/`backup stream` message containing `UNFORWARDED-SCHEMA-CHANGE` is a refusal that exits 1 because it has no code yet. Treat it as a decision point (step 4).
...
2. **Resolve the `SLUICE-E-*` code** against the table in `docs/operator/error-codes.md` — read off *class* (runtime vs refusal), *cause*, and *remedy*. The `class` drives the exit code (refusal → 3, runtime → 1). Common ones:
   ...
   - `SLUICE-E-RESUME-SOURCE-MISMATCH` (refusal, v0.154.0+) — `migrate --resume` refused before any copy: the recorded state was written by a run reading a **different source** (identity is source engine + database + schema, deliberately not the host). The refusal prints the recorded and live identity side by side. Remedy: re-run `--resume` against the source the id belongs to, or — to migrate the source you pointed at — **drop `--resume`** and pass a fresh `--migration-id` (both halves are needed; `--resume` under an unused id refuses separately). Not destructive, so no approval gate. A resume whose recorded state predates the column is not refused; it proceeds with a `RESUME-SOURCE-UNRECORDED` warning, which means identity could not be checked — confirm the source yourself before trusting that run.
   - `SLUICE-E-CDC-REPLICATION-PERMISSION` (runtime) — `ALTER ROLE x REPLICATION`.

3. **Classify the remedy's blast radius.** If the remedy is a plain re-run, a DSN fix, or a read-only follow-up → safe to propose running. If it names any **destructive flag** (`--reset-target-data`, `--force-cold-start`, `--yes`, `backup prune`/`compact` without `--dry-run`, `slot drop`) or `--accept-unforwarded-schema-change`, it is a decision point. Flag it as needing explicit human approval for that specific invocation (`skills/README.md` safety model). The acknowledgement is gated because it accepts a source/target schema difference permanently unless the change was applied to the target first.

4. **A codeless failure is usually a normal operator state, not a bug — read the `message` before escalating.** An exit-1 `status:"failed"` with **no `error.code` and no `hint`** is most often an ordinary decision point whose fix is stated in the `message`: re-running an already-complete migrate (`migration_id "…" is already complete; drop the target tables to redo, or use a different --migration-id`), a wrong restore/backup passphrase (`unwrap chain cek (wrong passphrase / KMS key?)`), and similar. Read the `message` and hand back its stated fix — do **not** jump to filing an issue. **Since v0.154.0 the already-complete state has a CODED sibling that looks similar and means the opposite, so check the code before reaching for this branch:** a `migrate --resume` whose recorded state belongs to a different source is `SLUICE-E-RESUME-SOURCE-MISMATCH` at **exit 3**, not a codeless exit 1. "Already complete" says *this* source's work is done; the coded refusal says the run was about to adopt **someone else's** copy — which, before the door existed, is exactly what exited 0 having copied nothing. Never answer it with "drop the target tables to redo"; see the code in step 2. **This heuristic does NOT apply to `UNFORWARDED-SCHEMA-CHANGE` (v0.156.0+).** That refusal is also a codeless exit 1, with no `error.code` and no `hint`, but it is a decision point. The source (Postgres, or a MySQL/MariaDB binlog source) changed a constraint, policy, RLS, `NOT NULL`, default or similar that CDC cannot forward, so the target is weaker than the source. The refusal is recorded on the target (for `backup stream`, in `stream_state.json`), and every restart refuses again. The replayed refusal prints `fingerprint <12 hex>`. Recovery: a human applies the same change to the target (for `backup stream`, takes a new full backup), then starts once with `--accept-unforwarded-schema-change=<fingerprint>`. **Approval needed: YES.** Passing the flag without the target change accepts the difference permanently. Never propose it as a plain re-run, and never restart the stream in a loop hoping it clears. A fleet leg shows `failed` and is not restarted. Only when the cause is genuinely unexplained (a real suspected bug, an unexpected mid-run `"failed"`) assemble an operator-bundle: `sluice diagnose --target-driver <drv> --target "$SLUICE_TARGET" --stream-id <id> -o bundle.zip` (add `--source-driver`/`--source`; `--privacy basic|standard|verbose` controls inclusion). Note `diagnose` **requires `--stream-id`**, so it fits a *sync* failure — a failed one-shot `migrate` has no stream id, so triage it from its `message`, not a bundle.
...
- **Approval needed?** YES if the recovery uses a destructive flag or `--accept-unforwarded-schema-change` (name it, and stop for human sign-off); NO for a safe re-run/config fix.
