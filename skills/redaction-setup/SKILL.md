...
1. **Choose per-column strategies.** `--redact '[schema.]table.column=STRATEGY[:options]'` (repeatable). **Prefer the bare `table.column` form.** It matches the table in every namespace the run reads (a single-database MySQL run's DSN database; a Postgres run's schema). A schema-qualified `schema.table.column` rule matches ONLY a table stamped with that namespace — and a single-database MySQL run stamps none, so since v0.138.0 a qualified rule there is **refused at preflight** (before v0.138.0 it passed, redacted the CDC rows, and shipped the column in clear through bulk copy and backup at exit 0 — audit finding NEW-1). To address a MySQL database by name, select it with `--include-database`, which stamps the namespace and makes the qualified rule resolvable; a multi-namespace run refuses a qualified rule naming a namespace outside the selected set. The real strategy families (confirm exact option syntax with `sluice migrate --help` and `docs/cookbook/recipe-redaction-keyset.md`):
...
4. **Run.** The same `sluice migrate` / `sluice sync start` command — redaction composes into the IR pipeline on BOTH the bulk-copy and the CDC-apply paths. The rule preflight (selector resolution, strategy/type checks, keyset presence) runs on every row-emitting lane: `migrate` (single- and multi-database), `sync start` cold start (single- and multi-database), `schema add-table`, and `backup full`. At start, sluice logs one audit line: `redaction configured … columns=N strategies=[…]` (key names elided). An empty redaction set does NOT refuse — a run with no rules produces a fully-plaintext target, so check that line.
