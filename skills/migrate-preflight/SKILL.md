...
3. **Inspect schema translation — the primary risk surface.** This is where the real risk data lives, not the dry-run. `sluice schema preview --format json …` reports how each source type maps to the target — the `translations[]`, `hints[]`, `unsigned_bigint_narrowings[]`, and the full target `ddl`; `sluice schema diff --format json …` (exit 1 = drift found) compares an existing target. **Also capture stderr** — translation WARN lines print there, not in the JSON. Flag every warning and every refusal:
   ...
   - **no-PK / keyless tables** (copied but not CDC-trackable — call out for a sync; the dry-run's `primary_key:false` is the signal). **On a POSTGRES source this is now a refusal, not an advisory** (v0.104.2, `SLUICE-E-SOURCE-REPLICA-IDENTITY`): a keyless table with no `REPLICA IDENTITY FULL` is refused at `sync start` before the publication is touched, because Postgres would otherwise reject the source application's own `UPDATE`/`DELETE` on it. A **deferrable** PK is refused for the same reason — and so is a deferrable PK *plus* a perfectly good immediate unique index, because `REPLICA IDENTITY DEFAULT` resolves to the primary key and nothing else. Remedies: `ALTER TABLE … REPLICA IDENTITY USING INDEX <idx>`, `REPLICA IDENTITY FULL`, an immediate PK, or `--exclude-table`. Unchanged for `migrate` (never scopes a publication) and for MySQL-family / trigger sources;
   ...
   **⚠ Also invisible: the PG replica-identity refusal.** `sync start --dry-run` returns before the cold start runs, so `SLUICE-E-SOURCE-REPLICA-IDENTITY` — which fires against the live source catalog — cannot appear in a dry-run however clean it looks. Check it yourself on a Postgres source before reporting GO: every in-scope table needs an immediate (NOT DEFERRABLE) unique/primary key, a nominated `REPLICA IDENTITY USING INDEX`, or `REPLICA IDENTITY FULL`. `SELECT relname, relreplident FROM pg_class …` plus `pg_index.indimmediate` answers it in one query. A clean dry-run is not evidence here.
