...
3. **Target branch has safe migrations ON? Follow the governed DDL path — NEVER "disable safe migrations" on your own.** A safe-migrations PlanetScale branch refuses EVERY direct DDL statement (Error 1105), so a fresh `sluice migrate`/`sync` into it refuses coded `SLUICE-E-PS-DIRECT-DDL-BLOCKED` — that refusal is the branch's production guardrail working, not a sluice failure. **Read which arm you got before reaching for a remedy**, because two of the three name a statement of yours and the newest does not: the control-table and schema-apply arms echo the exact refused statement, while the v0.151.0 **preflight** arm fires BEFORE the schema phase and names a throwaway probe table. The preflight arm's recovery is a **fresh `--migration-id`, never `--resume`** — nothing of yours was created to resume onto, but the attempt did record its own state row, so re-running the same id is refused as a partial migration. sluice never toggles the setting; disabling it is a human decision with a named tradeoff (surface it, don't improvise it). The governed path, in order:
