...
func requireNoHealEvidence(t *testing.T, store irbackup.Store) {
	...
	recs, defects, err := lineage.ReadHealRecords(context.Background(), store)
	if err != nil {
		t.Fatalf("read heal records: %v", err)
	}
	if len(defects) != 0 {
		t.Fatalf("heal log has unreadable line(s) %v; the fixture must be well-formed", defects)
	}
	if len(recs) != 0 {
...
// TestRestorePathsSurfaceMaintenanceHeals is the audit 2026-08-31 SEC-6
// pin. The A3 heal provenance shipped wired to `backup verify` alone, so
// the RESTORE paths — the moment the data actually lands — reported "all
// manifest + lineage signatures verified" on a chain whose signatures had
// been regenerated, with no hint. Both remaining verification entry points
// now report it, informational and never a failure.
//
// This is the behavioural half; TestMaintenanceHealReportRoster_Every-
// VerifyEntryPointClassified is the structural half that keeps a FUTURE
// entry point from shipping blind. Neither substitutes for the other: the
// roster proves the call exists, this proves it reaches an operator.
func TestRestorePathsSurfaceMaintenanceHeals(t *testing.T) {
	ctx := context.Background()

	// healedChain returns a chain that has been through one no-op
	// maintenance heal, plus its envelope and walked links.
	healedChain := func(t *testing.T) (irbackup.Store, crypto.EnvelopeEncryption, []lineage.SegmentRecord) {
		t.Helper()
		store, env, _ := buildSignedChain(t)
		signer := mustSigner(t, env)
		staleifyCatalog(t, store)
	...
	t.Run("chain restore / broker / export-as-parquet", func(t *testing.T) {
		store, env, links := healedChain(t)
		buf := captureMaintenanceSlog(t)
		if err := verifyChainSignatures(ctx, store, links, verifyMaterial{env: env}, true); err != nil {
			t.Fatalf("verifyChainSignatures on a healed chain: %v", err)
		}
		out := buf.String()
		for _, want := range []string{"regenerated by a no-op maintenance heal", "chain restore", "pre-heal"} {
			if !strings.Contains(out, want) {
				t.Errorf("chain-verify output missing %q — every restore path was blind to the heal (SEC-6)\nlog:\n%s", want, out)
			}
		}
		// Still not a failure: the heal is the documented recovery and the
		// signatures legitimately verify.
		if strings.Contains(out, "level=ERROR") {
			t.Errorf("heal provenance must be informational, never a failure:\n%s", out)
		}
	})

	t.Run("single-manifest restore", func(t *testing.T) {
		store, env, links := healedChain(t)
		buf := captureMaintenanceSlog(t)
		err := verifyManifestSignaturePolicy(ctx, store, links[0].Path, links[0].Manifest, 0,
			verifyMaterial{env: env}, false)
		if err != nil {
			t.Fatalf("verifyManifestSignaturePolicy on a healed chain: %v", err)
		}
		if out := buf.String(); !strings.Contains(out, "regenerated by a no-op maintenance heal") {
			t.Errorf("single-manifest verify output missing the heal-provenance line\nlog:\n%s", out)
		}
	})

	t.Run("never-healed chain: no line on any path", func(t *testing.T) {
		store, env, _ := buildSignedChain(t)
		links, err := lineage.ListAllSegmentManifests(ctx, store)
		if err != nil {
			t.Fatalf("list manifests: %v", err)
		...
		buf := captureMaintenanceSlog(t)
		if err := verifyChainSignatures(ctx, store, links, verifyMaterial{env: env}, true); err != nil {
			t.Fatalf("verifyChainSignatures on a healthy chain: %v", err)
		}
		if err := verifyManifestSignaturePolicy(ctx, store, links[0].Path, links[0].Manifest, 0,
			verifyMaterial{env: env}, false); err != nil {
			t.Fatalf("verifyManifestSignaturePolicy on a healthy chain: %v", err)
		}
		if strings.Contains(buf.String(), "maintenance heal") {
			t.Errorf("a never-healed chain must produce no heal line:\n%s", buf.String())
		}
	})
}

// TestMaintenanceHeal_MalformedLogSurfacesBothHalves is the verify-side
// half of the audit 2026-08-31 SEC-7 posture change, and it replaces a pin
// that asserted the OPPOSITE contract (refuse the whole file on one bad
// line). The old posture handed an adversary with store write access and
// NO signing key a one-byte lever: append a junk byte and every heal
// record disappeared behind a WARN, at exit 0.
//
// Now verify must show BOTH halves — the records that parsed AND a loud,
// counted warning about the line that did not — and still count zero
// failures, because the log is provenance, not validity.
func TestMaintenanceHeal_MalformedLogSurfacesBothHalves(t *testing.T) {
	ctx := context.Background()

	t.Run("a junk line hides neither the good records nor itself", func(t *testing.T) {
		store, env, _ := buildSignedChain(t)
		signer := mustSigner(t, env)
		staleifyCatalog(t, store)
		if _, err := CompactChain(ctx, store, CompactOpts{MergeWindow: time.Hour, Signer: signer}); err != nil {
		...
		// The lever: one junk line appended to a real, well-formed log.
		body := storeReadAll(t, store, lineage.MaintenanceHealLogFileName)
		if err := store.Put(ctx, lineage.MaintenanceHealLogFileName,
			bytes.NewReader(append(body, []byte("not json at all\xff\n")...))); err != nil {
			t.Fatalf("plant junk line: %v", err)
		}
		...
		out := buf.String()
		if !strings.Contains(out, "regenerated by a no-op maintenance heal") {
			t.Errorf("the real heal record was hidden by one junk byte — the SEC-7 defect:\n%s", out)
		}
		for _, want := range []string{"unreadable line(s)", "unreadable_lines=1", "records_reported=1"} {
			if !strings.Contains(out, want) {
				t.Errorf("verify output missing %q — the skipped line must be counted and named, never dropped quietly:\n%s", want, out)
			}
		}
	})

	t.Run("a store-level read failure still says provenance unknown", func(t *testing.T) {
		// The error return is reserved for STORE failures, and that path
		// keeps the original message: nothing could be read at all, so
		// there is genuinely no provenance to report.
		store, env, _ := buildSignedChain(t)
		records, err := lineage.ListAllSegmentManifests(ctx, store)
		if err != nil {
			t.Fatalf("list manifests: %v", err)
		}
		failing := &healLogReadFailStore{Store: store}
		buf := captureMaintenanceSlog(t)
		if failed := verifyBackupSignatures(ctx, failing, records, VerifyOptions{Envelope: env}); failed != 0 {
			t.Fatalf("verifyBackupSignatures = %d failure(s); want 0", failed)
		}
		if !strings.Contains(buf.String(), "heal provenance unknown") {
			t.Errorf("verify output missing the 'heal provenance unknown' WARN on an unreadable heal log:\n%s", buf.String())
		}
	})
}

// healLogReadFailStore fails Exists on the heal log only — the store-level
// failure shape, as distinct from malformed content.
type healLogReadFailStore struct {
	irbackup.Store
}

func (s *healLogReadFailStore) Exists(ctx context.Context, path string) (bool, error) {
	if path == lineage.MaintenanceHealLogFileName {
