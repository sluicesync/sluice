...
func healStaleLineageSignatures(ctx context.Context, store irbackup.Store, cat *lineage.Catalog, op string, dryRun bool, signer *lineage.Signer) error {
	...
	// Durable heal evidence (audit 2026-08-27 A3), BEFORE the re-sign
	// destroys it: preserve the non-verifying signature verbatim and append
	// a maintenance-heal record, so the one-shot WARN below is not the only
	// trace of a re-sign on a typically cron-driven run. Both writes fail
	// the heal closed — refusing to heal is recoverable (re-run), healing
	// while silently losing the forensic evidence is not.
	preserved, err := lineage.PreserveLineageSigForHeal(ctx, store, time.Now().UTC())
	if err != nil {
		return fmt.Errorf("%s: preserve pre-heal lineage signature: %w", op, err)
	}
	if err := lineage.AppendHealRecord(ctx, store, lineage.HealRecord{
		HealedAt:      time.Now().UTC(),
		Operation:     op,
		KeyID:         signer.KeyID,
		VerifyFailure: verr.Error(),
		PreservedSig:  preserved,
	}); err != nil {
		return fmt.Errorf("%s: record maintenance heal: %w", op, err)
	}
	slog.WarnContext(
		ctx, op+": the lineage signature does not verify on a no-op maintenance run — re-signing the chain in place. This is the expected recovery for a maintenance run that crashed between its catalog commit and its re-sign; if this chain should NOT have needed healing, treat the signature failure below as suspect before trusting the restored signatures (the pre-heal signature is preserved for inspection, and the heal is durably recorded in "+lineage.MaintenanceHealLogFileName+" — `backup verify` reports it)",
		slog.String("verify_failure", verr.Error()),
		slog.String("signing_key_id", signer.KeyID),
		slog.String("preserved_sig", preserved),
	)
...
func reportMaintenanceHeals(ctx context.Context, store irbackup.Store) {
	...
	for _, rec := range recs {
		slog.InfoContext(
			...
			slog.String("verify_failure", rec.VerifyFailure),
