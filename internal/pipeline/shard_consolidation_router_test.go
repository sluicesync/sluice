...
func newTestRouter(t *testing.T, store *fakeLeaseStore, streamID string, prober ShardConsolidationProber, applier ir.ShapeDeltaApplier, clock *mockClock) *BoundaryRouter {
	...
	router, err := NewBoundaryRouter(mgr, applier, prober, "postgres", "postgres", sourceDefaultReaders{})
	if err != nil {
...
func TestNewBoundaryRouter_NilGuards(t *testing.T) {
	...
	if _, err := NewBoundaryRouter(nil, &fakeShapeApplier{}, &fakeProber{}, "postgres", "postgres", sourceDefaultReaders{}); err == nil {
		t.Error("expected error on nil manager")
	}
	if _, err := NewBoundaryRouter(mgr, nil, &fakeProber{}, "postgres", "postgres", sourceDefaultReaders{}); err == nil {
		t.Error("expected error on nil applier")
	}
	if _, err := NewBoundaryRouter(mgr, &fakeShapeApplier{}, nil, "postgres", "postgres", sourceDefaultReaders{}); err == nil {
