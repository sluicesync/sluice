...
func TestIntercept_NilRouter_PassThrough(t *testing.T) {
	...
	out := interceptSchemaSnapshotsForCoordination(context.Background(), inRecv, nil, nil, nil, nil, &errStore)
	if out != inRecv {
...
func TestIntercept_FirstSnapshotSeedsCache_NoRoute(t *testing.T) {
	...
	defer cancel()
	out := interceptSchemaSnapshotsForCoordination(ctx, in, nil, router, nil, nil, &errStore)
