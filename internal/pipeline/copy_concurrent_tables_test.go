...
func TestRunConcurrentTableCopy_WindowsOverlap(t *testing.T) {
	...
	go func() {
		...
		done <- runConcurrentTableCopy(ctx, groups, schema, reader, writer, nil, ShardColumnSpec{}, 1, true, false, nil)
	}()
...
func TestRunConcurrentTableCopy_ExactlyOnce(t *testing.T) {
	...
	if err := runConcurrentTableCopy(context.Background(), groups, schema, reader, writer, nil, ShardColumnSpec{}, 1, true, false, nil); err != nil {
		t.Fatalf("runConcurrentTableCopy: %v", err)
	}
...
func TestRunConcurrentTableCopy_ErrorAbortsLoudly(t *testing.T) {
	...
	err := runConcurrentTableCopy(context.Background(), groups, schema, reader, writer, nil, ShardColumnSpec{}, 1, true, false, nil)
	if err == nil {
