...
func (b *BackupStream) alignEncryption(ctx context.Context, parent *irbackup.Manifest) ([]byte, error) {
	...
	case parentEnc == nil && b.Encryption != nil:
		...
		return nil, sluicecode.Wrap(sluicecode.CodeBackupEncryptionMismatch,
			...
			errors.New("stream: parent chain is plaintext but --encrypt was supplied; cannot extend a plaintext chain with encrypted incrementals"))
	case parentEnc != nil && b.Encryption == nil:
	...
	if b.Encryption.Mode != "" && b.Encryption.Mode != mode {
		...
		return nil, sluicecode.Wrap(sluicecode.CodeBackupEncryptionMismatch,
			...
			fmt.Errorf("stream: --encrypt-mode=%q conflicts with the chain's encryption mode %q; "+
