...
// ADR-0060 — Operator-actionable rendering of [ir.SchemaDriftReport]
...
// diff lives in [ir.DiffTable]. The split keeps the engine-neutral
...
func RenderSchemaDriftReport(r ir.SchemaDriftReport) string {
	if !r.HasChanges() {
		return ""
	}
	var lines []string
...
func renderColumnAddedLine(c ir.ColumnDriftEntry) string {
...
func renderColumnDroppedLine(c ir.ColumnDriftEntry) string {
...
func renderColumnRenamedLine(c ir.ColumnRenameEntry) string {
...
func renderIndexAddedLine(c ir.IndexDriftEntry) string {
	kind := "index"
	if c.Unique {
		kind = "unique index"
	}
...
func renderIndexDroppedLine(c ir.IndexDriftEntry) string {
	kind := "index"
	if c.Unique {
		kind = "unique index"
	}
...
func renderCheckAddedLine(c ir.CheckDriftEntry) string {
...
func renderCheckDroppedLine(c ir.CheckDriftEntry) string {
...
func renderCheckAlteredLine(c ir.CheckAlterEntry) string {
...
func renderFKAddedLine(c ir.ForeignKeyDriftEntry) string {
	name := c.Name
	if name == "" {
		name = "<unnamed>"
	}
...
func renderFKDroppedLine(c ir.ForeignKeyDriftEntry) string {
	name := c.Name
	if name == "" {
		name = "<unnamed>"
	}
...
func renderFKAlteredLine(c ir.ForeignKeyAlterEntry) string {
	name := c.Name
	if name == "" {
		name = "<unnamed>"
	}
