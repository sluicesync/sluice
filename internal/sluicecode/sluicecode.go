...
var registry = map[Code]Info{
	...
	CodeCDCLineageMismatch:       {ClassRefusal, "the resume position's GTID lineage does not match the source: either an errant GTID (shares UUIDs with the shard and names one it never executed) or a genuinely different keyspace/instance (shares none) — the message says which; the foreign arm is a terminal refusal that never routes into the automatic re-snapshot, which would drop the target and re-copy from a different database"},
