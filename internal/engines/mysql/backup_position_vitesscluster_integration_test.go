...
// The vtgate half of [binlogEnabledQuery]'s premise, across Vitess
// versions.
//
// Same assertions as the per-PR vtcombo sibling
// (TestVStream_BackupPositionProbesAnswerThroughVtgate), against the REAL
// multi-process cluster — etcd + vtctld + primary/replica vttablet +
// vtgate — so the read crosses a genuine vtgate process to a genuine
// tablet. This is the copy that runs in vitess-version-matrix.yml over
// v21..v24, which is the point: "vtgate serves `@@GLOBAL.log_bin`" is a
// claim about someone ELSE's software, and the version axis is where such
// a claim rots. The original measurement (2026-08-07, v24.0.1) was taken
// on exactly this stack.
//
// What the measurement showed, and why the value is not a coincidence:
// vtgate does not answer the variable locally, it ROUTES the query to a
// tablet — visible as "target: test.0.primary: no healthy tablet
// available" before the primary starts serving. So the answer is the
// tablet's mysqld's, and a Vitess tablet runs mysqld with binary logging
// on because VReplication and VStream both ride it.
...
// TestVitessCluster_BackupPositionProbesAnswerThroughVtgate measures,
// rather than assumes, that a real vtgate serves the system variables the
// backup-position preflight and the table-name-fold probe read.
func TestVitessCluster_BackupPositionProbesAnswerThroughVtgate(t *testing.T) {
...
// vtgate premise pins. It is DUPLICATED verbatim in the vstream sibling
// rather than shared from one file: the two suites' build tags are
