...
type ChangeApplier struct {
	...
	// direct-API / unit constructions, which disables the concurrent path
	...
	// Zero-value-safe: 0 and 1 mean serial (byte-identical to today);
	// concurrency engages ONLY for W > 1 via [SetApplyConcurrency]. See
	...
	// change's source schema to derive the TARGET database; the change's own
	...
	// through on the SOURCE, set by [SetPublicationName] at Streamer
	// startup — slotName's exact sibling (ADR-0176 prerequisite
	// chunk). Same cross-engine parity rationale as slotName: a
	// PG-source sync can target MySQL, and the warm-resume ratchet
	// reads the recorded publication back from the TARGET's
	// sluice_cdc_state row via ListStreams, so the column must
	// round-trip here too. Empty on same-engine MySQL → MySQL streams
	// (no publication concept) and for legacy streams on the
	...
	// the ADR-0104 concurrent lanes reach the skip path from W goroutines.
	...
	// ADR-0104 concurrent lanes record from W goroutines while the coordinator
...
// name so subsequent position-writes populate the sluice_cdc_state
// row's publication_name column — slot_name's exact sibling (ADR-0176
// prerequisite chunk). Symmetric with the PG ChangeApplier's
// counterpart: a cross-engine PG → MySQL stream records which
// publication it reads through on the TARGET's control table so warm
// resume can ratchet onto it.
//
// MySQL sources have no publication concept, so on a MySQL-source
// stream the structural SetPublicationName call lands with an empty
// string; empty input is a no-op via the COALESCE-tolerant shape in
...
// signal, not the log volume) — and accumulates it toward the durable
...
// boundary. This is what removes the per-event fsync that made a bulk-load
// of an unknown table a permanent lag generator. recordSkippedTable can no
// longer fail (no DB I/O), but keeps its error return so the dispatch arms'
...
func (a *ChangeApplier) flushSkippedTables(ctx context.Context) error {
	err := a.skipAccum.Flush(ctx, func(ctx context.Context, key appliershared.SkipLedgerKey, e appliershared.SkipLedgerEntry) error {
		return upsertSkippedTable(ctx, a.db, a.controlKeyspace, key.StreamID, key.Table, e, a.upsert)
	})
	if err != nil {
		return classifyApplierError(err)
	}
...
//     stream): maps the change's source database through nsRename (ADR-0142;
//     identity when nil) to its TARGET database, then qualifies with that
//     target ONLY when it is non-empty AND differs from the bound `schema`
//     — exactly the cross-database case (`app_db` row applied while bound to
//     a control/different database). A change whose (renamed) target is
//     empty or equals the bound database returns the bound database, so an
//     in-bound-namespace change still emits the SAME bound SQL. This mirrors
