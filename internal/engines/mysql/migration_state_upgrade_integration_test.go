...
func TestMigrationStateStoreMySQL_LegacyBlobUpgrade(t *testing.T) {
	...
	// READ-ONLY server must succeed (the old read-time upgrade failed
	// here with a write refused by the server). super_read_only blocks
	// even the SUPER test user's table writes.
	if _, err := db.ExecContext(ctx, "SET GLOBAL super_read_only = 1"); err != nil {
		t.Fatalf("set super_read_only: %v", err)
	}
	...
	_ = roStore.Close()
	// super_read_only=1 implies read_only=1; clear BOTH (disabling the
	// former does not auto-clear the latter).
	if _, err := db.ExecContext(ctx, "SET GLOBAL super_read_only = 0"); err != nil {
		t.Fatalf("unset super_read_only: %v", err)
	}
	if _, err := db.ExecContext(ctx, "SET GLOBAL read_only = 0"); err != nil {
		t.Fatalf("unset read_only: %v", err)
	}
	if roErr != nil || !ok {
		t.Fatalf("Read under read-only server = ok=%v err=%v; want ok=true err=nil", ok, roErr)
	}
	assertUpgradedProgressFamilies(t, roGot.TableProgress)

	// 5. The FIRST WRITE path performs the one-time upgrade before its
	// own statement: format 2, sentinel blob, 7 rows (orphan gone), and
	// the written entry landed on top.
