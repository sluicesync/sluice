...
// a later process), so each must come back byte-exact. The 4-byte row is the
// one a utf8 (utf8mb3) column would refuse or mangle.
var unforwardedRefusalCorpus = map[string]string{
	"ascii":         `mysql: cdc: UNFORWARDED-SCHEMA-CHANGE on db.t: ADD CONSTRAINT "u" UNIQUE (name)`,
	"2-byte utf8":   "UNFORWARDED-SCHEMA-CHANGE on db.café: ADD CONSTRAINT \"ü\" CHECK (x > 0)",
	"3-byte utf8":   "UNFORWARDED-SCHEMA-CHANGE on db.表: ALTER COLUMN \"✓\" SET DEFAULT '…'",
	"4-byte utf8":   "UNFORWARDED-SCHEMA-CHANGE on db.t: ALTER COLUMN \"😀\" SET DEFAULT 'x'",
...
// TestUnforwardedRefusalStore_RoundTrip pins the MySQL persisted-refusal
...
// the column itself, every corpus shape round-trips exactly — including a
// repeated identical Record, which MySQL's changed-rows RowsAffected would
// report as zero rows — a missing stream row is a loud error, and Clear is
// idempotent, all without touching the stream's position.
func TestUnforwardedRefusalStore_RoundTrip(t *testing.T) {
	...
	applyMySQLApplier(t, dsn, "CREATE TABLE `sluice_cdc_state` ("+
		...
		"PRIMARY KEY (stream_id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4")
	applyMySQLApplier(t, dsn, "INSERT INTO `sluice_cdc_state` (stream_id, source_position) VALUES ('s1', 'tok')")
	...
	for name, want := range unforwardedRefusalCorpus {
		...
		if err := store.RecordUnforwardedRefusal(ctx, "s1", want); err != nil {
			t.Fatalf("%s: repeated Record (changed-rows RowsAffected = 0): %v", name, err)
		}
