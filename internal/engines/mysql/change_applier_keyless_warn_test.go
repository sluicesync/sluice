...
// because crash-resume granularity is the SOURCE TRANSACTION (the GTID
// advances only at its commit; for VStream the VGTID arrives after every
// ROW event of the transaction), so every keyless row in one interrupted
...
func TestWarnKeylessOnce_HonestWording(t *testing.T) {
	...
	a := &ChangeApplier{}
	a.warnKeylessOnce(context.Background(), "app.events")
	...
	for _, want := range []string{"at-least-once", "not idempotent", "app.events", "ADR-0089"} {
...
func TestWarnKeylessOnce_OncePerTable(t *testing.T) {
	...
	a.warnKeylessOnce(context.Background(), "app.audit")
	if got := strings.Count(buf.String(), "\n"); got != 2 {
