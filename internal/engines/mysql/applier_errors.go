...
//   - Error 1105 (HY000) carrying a VTGATE CLUSTER-EVENT sentence
//     ("primary is not serving, there may be a reparent operation in
//     progress", "current keyspace is being resharded") — the same
//     failover/cutover class, but emitted by vtgate itself, so it
//     carries NO `vttablet` tag and the branch above cannot see it.
//     See [vitessClusterEventSubstrings].
...
func classifyApplierError(err error) error {
	...
	if errors.As(err, &mysqlErr) {
		...
		case 1105:
			...
			// VTGATE-emitted cluster event (the 2026-07-28 live PS-160
			// 122 GB finding). vtgate answers a query for a shard whose
			// primary is mid-reparent with `Error 1105 (HY000): target:
			// <ks>.<shard>.primary: primary is not serving, there may be a
			// reparent operation in progress` — generated by VTGATE, not by
			// a tablet, so it carries NO `vttablet` tag and
			// classifyVitessMessage above (which requires that tag by
			// design) cannot match it. Pre-fix it fell to the terminal-code
			// shield below and aborted a 122 GB cold copy after 38s, with
			// NEITHER ADR-0108 bound (30m wall-clock / 100000 attempts)
			// reached — while the vttablet-framed sibling in the SAME
			// reparent window was ridden out. The belt the ADR-0108 text
			// legs were supposed to provide has been unreachable for every
			// structured *MySQLError since the shield landed.
			//
			...
			// prevent. See [vitessClusterEventSubstrings].
			if isVitessClusterEventMessage(mysqlErr.Message) {
				return &retriableMySQLError{err: err}
			}
		case 3:
	...
	// `code = Unavailable`) and the vtgate cluster-event shape (1105
	// carrying a canonical [vitessClusterEventSubstrings] sentence) are
	...
	if msg := err.Error(); msg != "" {
		if isVitessClusterEventMessage(msg) {
			return &retriableMySQLError{err: err}
		}
...
// vitessClusterEventSubstrings is the EXACT (lower-cased) set of VTGATE
// CLUSTER-EVENT sentences that mark a structured Error 1105 as a target
// failover/cutover transient — the ONE message consultation the
// terminal-code shield allows for that code beyond the vttablet framing.
//
// Why a second, NARROWER set instead of reusing
// [reparentRetriableSubstrings]: these literals are matched with a
// structured *MySQLError in hand, where the message can echo the offending
// statement (`... (CallerID: x): Sql: "insert into reparent_history ..."`).
// A loose "reparent" / "not serving" token there would let an echoed
// identifier or row value flip a terminal 1105 retriable, and a
// false-positive transient ARMS the cold-copy tolerate-1062-on-retry wart
// on the next attempt (see writeBatchedConn) — the D0-3 silent-batch-skip
// chain. Full sentences cannot be produced by an identifier echo. The
// looser set stays in force ONLY below the shield, where no server
// response exists. [TestClassifyApplierError_TerminalCodeShield] pins 1105
// itself against the loose tokens so the two sets cannot be "unified".
//
// GROUND TRUTH — these are vtgate's own constants, verbatim from
// vitess.io/vitess@v0.24.2 `go/vt/vtgate/buffer/buffer.go:72-73`:
//
...
//
// vtgate raises them at `vtgate/tabletgateway.go` when a PRIMARY target has
// no serving tablet, as `vterrors.Errorf(Code_CLUSTER_EVENT, ...)` — its own
// query buffer BUFFERS AND RETRIES exactly this class, which is the upstream
// statement that they are transient. They reach the client as `Error 1105
// (HY000): target: <ks>.<shard>.primary: <sentence>` with no `vttablet` tag.
// The reparent sentence is split into its two halves so a reword of either
// half still matches. Not matched (deliberate): `ClusterEventMoveTables`
// ("disallowed due to rule") — a routing-rule denial during a MoveTables
// cutover, whose wording is far too generic to match safely and which does
// not self-heal the way a failover window does; it stays terminal and loud.
//
// The matcher lower-cases the error text before comparing, so these MUST be
// lower-case. [TestVitessClusterEventSubstrings_PinDown] pins the literals;
// extend that test and this slice together.
var vitessClusterEventSubstrings = []string{
	"primary is not serving",
	"reparent operation in progress",
	"current keyspace is being resharded",
}

// isVitessClusterEventMessage reports whether msg carries one of the
// canonical vtgate CLUSTER-EVENT sentences ([vitessClusterEventSubstrings]).
// Case-insensitive — vtgate/vttablet wording varies in case across versions.
func isVitessClusterEventMessage(msg string) bool {
	lower := strings.ToLower(msg)
	for _, sub := range vitessClusterEventSubstrings {
		if strings.Contains(lower, sub) {
			return true
		}
	}
