...
func (r *RowReader) ReadRows(ctx context.Context, table *ir.Table) (<-chan ir.Row, error) {
	...
	query := buildSelect(table, r.qualifyBySchema, r.rowFilters[table.Name])
...
func (r *RowReader) stream(ctx context.Context, rows *sql.Rows, table *ir.Table, out chan<- ir.Row, cleanup func()) {
	...
	for rows.Next() {
		...
		for i, col := range cols {
			...
			// A TINYINT(1)/ir.Boolean cell holding a value outside {0,1}
			// would be silently collapsed to true (losing the integer) —
			// refuse loudly before this row is written, pointing at the
			// --type-override remedy (SLUICE-E-VALUE-TINYINT1-RANGE).
			if _, isBool := col.Type.(ir.Boolean); isBool {
				if rerr := checkTinyBoolRange(table.Name, col.Name, scanBuf[i]); rerr != nil {
					r.setErr(rerr)
					return
				}
			}
...
// full-scan SELECT as a parenthesized WHERE conjunct (the sole clause on
// the full scan) — evaluated on the source so only matching rows are read.
func buildSelect(table *ir.Table, qualifyBySchema bool, predicate string) string {
