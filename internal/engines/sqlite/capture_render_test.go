...
// `format('%.17g', …)` arm never had: SQLite 3.43 changed format()'s float
// rendering so a plain `%.Ng` silently drops low bits (0.30000000000000004 →
// "0.3" at ANY N), and the expression shipped lossy for every REAL on the D1
// reader, D1 staging, and both trigger-CDC capture lanes until the 2026-08-22
// adversarial-corpus sweep caught it.
//
...
// (r_17 / r_third); a pre-3.43 app library firing a local capture trigger is
// the one UNVERIFIED PREMISE, named at [CapturedValueExpr].
