...
// D1ReplacementCountExpr renders the server-side count of U+FFFD
// characters stored in colExpr, as a byte-difference over three.
//
// # Why a SECOND number is needed at all
//
// D1 stores invalid-UTF-8 TEXT intact but rewrites every invalid byte to
// U+FFFD in its /query JSON response, SERVER-SIDE, so the mangled cell
// arrives as valid UTF-8 and no client-side inspection can see it. The
// byte-length bracket catches the usual case, because three bytes
// replace one and the delivered text is longer than the stored text.
//
// It does not catch every case. A MAXIMAL INVALID SUBPART of exactly
// three bytes — which is what severing a 4-byte sequence at a byte
// boundary produces, i.e. a truncated emoji — rewrites to a single
// 3-byte U+FFFD and PRESERVES the length. The sums then agree and the
// bracket passes over a cell that was silently rewritten.
//
// Counting replacement characters closes that: the source's count is the
// U+FFFD an operator genuinely stored, the delivered count is those plus
// any the API introduced, and between the two numbers the pair admits no
// rewrite that changes neither.
//
// The division is by three because the count is taken as a BYTE
// difference (`length(CAST(… AS BLOB))`) rather than a character one:
// `length()` over text carrying invalid UTF-8 is not a number worth
// depending on, while the byte length is exact, and each U+FFFD removed
// drops exactly three bytes.
//
// Lives here rather than in either consumer because both D1 text-crossing
// lanes need the identical expression: the trigger lane's change-log
// image check and the reader lane's page bracket. Audit 2026-09-09
// A0909-SLP-MEDIUM-2 is what happens when only one of them has it.
func D1ReplacementCountExpr(colExpr string) string {
	return `(length(CAST(` + colExpr + ` AS BLOB)) - length(CAST(replace(` +
		colExpr + `, char(65533), '') AS BLOB))) / 3`
}
