...
func (s publicationExposureSite) remedy() string {
	...
	if s == exposureSiteStreamOpen {
		return give + "Do NOT drop the publication to restore those writes: depending on whether this " +
			"stream has to decode anything written after the DROP and before this resume recreates the " +
			"publication, that either wedges it permanently or " +
	...
	return give + "The publication can be dropped once the whole chain is finished with it and NO stream " +
		...
		"open, depending on whether that stream has to decode anything written after the DROP and before " +
		"the open recreates the publication"
}
