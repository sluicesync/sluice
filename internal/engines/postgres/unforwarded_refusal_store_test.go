...
func TestUnforwardedChangeError_WrapsTheSentinel(t *testing.T) {
	refusal := unforwardedChangeError("public", "t", []string{`ADD CONSTRAINT "u" UNIQUE (name)`})
	terminal := error(&terminalPGError{err: refusal})
	...
	msg := refusal.Error()
	if !strings.HasPrefix(msg, "postgres: cdc: "+unforwardedChangeMarker+" on public.t: ") {
