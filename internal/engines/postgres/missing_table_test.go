...
// NOTE (audit C-11): this file used to also pin isMissingTableErr, the
// truncate-arm exec-error swallow gate. That gate is gone — the applier
// now resolves an unknown target table BEFORE executing (the same
// colTypesFor probe every DML arm uses), because swallowing the
// exec-time 42P01 left the open tx aborted and turned the position
// write behind it into a 25P02 poison pill. The classifier above
// ([isUndefinedTableErr]) remains the engine's one missing-relation
// classification and still gates the read-side emptiness probe.
