...
// immediately. database/sql is lazy — no connection is dialled by Open
// — so the returned handle never touches the network; a subsequent
...
// without a real Postgres, which is exactly the surface the Fix-A
...
func closedDB(t *testing.T) *sql.DB {
	t.Helper()
	db, err := sql.Open("pgx", "postgres://unit-test-never-dialled/db")
	if err != nil {
...
// dominated by the blocked wait (observed ~31s for ~0.4s of real apply
// work). The controller read that as "apply is catastrophically slow"
// and collapsed batch size 1000→1.
//
...
// guarantee that the PG applier exposes both optional-surface
