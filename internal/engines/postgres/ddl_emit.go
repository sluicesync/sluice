...
func emitIntegerType(i ir.Integer) string {
	typeName := postgresIntName(translate.PGStorageIntegerWidth(i))
	if i.AutoIncrement {
		// BY DEFAULT, deliberately and for EVERY identity column, ALWAYS
		// ones included: the bulk copy lands the source's ids through COPY
		// / INSERT with explicit values, which `GENERATED ALWAYS` refuses
		// (SQLSTATE 428C9) — see docs/dev/notes/prep-postgres-copy-writer.md.
		// The downgrade is no longer silent (gap census 2026-09-22 S4):
		// the writer WARNs once per run naming every ALWAYS column
		// ([identityAlwaysDowngradedMarker]), and migrate restores ALWAYS
		// after the copy ([SchemaWriter.RestoreIdentityGeneration]); sync
		// keeps BY DEFAULT because its CDC applier writes explicit ids.
		return typeName + " GENERATED BY DEFAULT AS IDENTITY"
	}
	return typeName
}

// identityOptionsClause renders the `(sequence options)` tail of an
// identity column from [ir.Column.Identity] — the S5 half of GC-3:
// before it an identity re-created on the target always got PostgreSQL's
// factory options, so `INCREMENT BY 10 START WITH 3` (a sharded-id
// scheme) silently became `INCREMENT BY 1 START WITH 1`.
//
// Each option is rendered only where it differs from the ascending
// factory default for the EMITTED integer type (START 1, INCREMENT 1,
// MINVALUE 1, MAXVALUE = type ceiling, CACHE 1, NO CYCLE), so an
// ordinary identity keeps the bare clause and a descending or
// re-optioned one round-trips exactly (every option that differs is
// present). A `--type-override` that NARROWS an identity's type therefore
// renders the source type's ceiling as an explicit MAXVALUE, which the
// narrower type cannot hold and CREATE TABLE refuses loudly — the
// operator asked for the narrowing, and the alternative is sluice
// re-capping the sequence silently. Empty for a non-identity column, a
// MySQL AUTO_INCREMENT (Identity nil), and a pre-field manifest.
func identityOptionsClause(c *ir.Column) string {
	// Storage type, not declared type (the Bug 233 roster): the identity
	// clause and its ceiling belong to the integer the column lands in.
	intT, ok := ir.UnwrapDomain(c.Type).(ir.Integer)
	if !ok || !intT.AutoIncrement || c.Identity == nil {
		return ""
	}
	id := c.Identity
	typeMax := sequenceTypeMax(strings.ToLower(postgresIntName(translate.PGStorageIntegerWidth(intT))))
	var parts []string
	if id.Increment != 1 {
		parts = append(parts, fmt.Sprintf("INCREMENT BY %d", id.Increment))
	}
	if id.MinValue != 1 {
		parts = append(parts, fmt.Sprintf("MINVALUE %d", id.MinValue))
	}
	if id.MaxValue != typeMax {
		parts = append(parts, fmt.Sprintf("MAXVALUE %d", id.MaxValue))
	}
	if id.Start != 1 {
		parts = append(parts, fmt.Sprintf("START WITH %d", id.Start))
	}
	if id.Cache != 1 {
		parts = append(parts, fmt.Sprintf("CACHE %d", id.Cache))
	}
	if id.Cycle {
...
// postgresIntName names one of the three widths Postgres has. Its input
// is a [translate.PGStorageIntegerWidth] result — that function owns the
// unsigned widening AND the collapse of the IR's 8/24-bit widths onto
// them, so this switch never has to restate either. Keeping the policy
// in one engine-neutral place is what lets the shape-compare retarget
// PREDICT this emitter's output without a second copy of the arithmetic
// to drift (roadmap item 158).
...
func emitColumnDef(table *ir.Table, c *ir.Column, opts emitOpts) (string, error) {
	...
	// The identity clause emitIntegerType rendered ends `AS IDENTITY`; PG
	// grammar puts the sequence options directly after it, before
	// COLLATE (moot on an integer) and NOT NULL.
	sb.WriteString(identityOptionsClause(c))
