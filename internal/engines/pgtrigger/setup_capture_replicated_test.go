...
func TestRenderSetupDDL_CaptureReplicatedWrites(t *testing.T) {
	...
	plain := strings.Join(renderSetupDDL("public", specs, true, false, CapturePayloadFull, false, nil), "\n")
	optIn := strings.Join(renderSetupDDL("public", specs, true, false, CapturePayloadFull, true, nil), "\n")
	...
	t.Run("meta upsert records the posture, both values", func(t *testing.T) {
		...
		if want := fmt.Sprintf("%s, %s) VALUES (TRUE, %d, true,", metaCaptureReplicatedCol, metaCaptureDigestCol, ChangeLogSchemaVer); !strings.Contains(optIn, want) {
		...
		if want := fmt.Sprintf("%s, %s) VALUES (TRUE, %d, false,", metaCaptureReplicatedCol, metaCaptureDigestCol, ChangeLogSchemaVer); !strings.Contains(plain, want) {
