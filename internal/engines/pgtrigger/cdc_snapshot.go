...
//
// The snapshot + anchor half is [openAnchoredSnapshot], shared with the
// full-backup opener ([Engine.OpenBackupSnapshot], roadmap item 163) so
// the gap-freedom argument has exactly one implementation; this
// function adds the CDC poller on top.
func (e Engine) OpenSnapshotStream(ctx context.Context, dsn string) (*ir.SnapshotStream, error) {
	...
	// the one snapshot-pinned connection — there is no parallel option on
	// this flavor (the anchor + snapshot consistency argument is bound to
	...
	stream.ReleaseRowsFn = snap.releaseRows
...
// anchoredSnapshot is a pinned REPEATABLE READ view of the source with
// the change log's SETTLED anchor captured inside that same view. It is
// the one thing both consumers of the trigger-native handoff need — the
// sync cold start ([Engine.OpenSnapshotStream]) and the full backup
// ([Engine.OpenBackupSnapshot]) — factored out so the gap-freedom
// argument (anchor query + settle wait + clamp, see
// [waitForPreSnapshotTxnsToSettle]) is implemented once. Before roadmap
// item 163 the backup had no share of it at all, and a chain rooted at a
// trigger full silently omitted the window between the full's sweep and
// the first incremental's "from now" anchor.
//
// Lifecycle: [anchoredSnapshot.rows] hands out the snapshot-pinned
// reader; [anchoredSnapshot.releaseRows] commits the read-only tx and
// closes the pinned conn (idempotent); [anchoredSnapshot.close] releases
// and closes the pool; [anchoredSnapshot.abort] is the error-path
// teardown (ROLLBACK + close everything, context-independent).
type anchoredSnapshot struct {
	...
	// rowsReleased guards releaseRows against a double commit/close. The
	// orchestrator calls ReleaseRows after bulk-copy, then Close on unwind;
	// Close also calls releaseRows as a safety net. Not mutex-guarded: the
	// orchestrator never calls these concurrently (bulk-copy is
	// single-goroutine for the snapshot tx; the streamer's defer chain
	// serialises Close). Mirrors the postgres engine's cdc_snapshot.go
	// closure shape.
	rowsReleased bool
}

// openAnchoredSnapshot connects, preflights the capture log, pins a
// REPEATABLE READ READ ONLY transaction, and captures the settled
// change-log anchor inside it. Every error path tears the connection
// down; the caller owns the returned snapshot's lifecycle.
func openAnchoredSnapshot(ctx context.Context, dsn, appID string) (*anchoredSnapshot, error) {
	cfg, err := parseDSNCompat(dsn)
	if err != nil {
		return nil, err
	}

	// Dedicated pool for the snapshot read. The CDC poller (sync) opens
	// its OWN pool so the snapshot pool can be released by ReleaseRows
	// independently of the CDC lifetime.
	...
	// surfaces at open time; firing it here too means the caller aborts
	// before any data moves rather than mid-stream.
	...
	snap := &anchoredSnapshot{db: db, conn: conn, schema: cfg.schema}
	...
	snap.position, err = encodePos(pgTriggerPos{LastID: anchor})
	if err != nil {
		snap.abort()
...
// rows returns the snapshot-pinned row reader: the composed postgres
// engine's RowReader value-decode + buildSelect machinery on the pinned
// conn. The reader does not own the conn lifecycle — releaseRows/close do.
func (s *anchoredSnapshot) rows() ir.RowReader {
	return postgres.NewSnapshotRowReader(s.conn, s.schema)
}

// releaseRows COMMITs the read-only snapshot tx (nothing was written;
// COMMIT is the project convention and matches the postgres ReleaseRows)
// and closes the pinned conn. context.Background so a cancelled parent
// ctx doesn't prevent cleanup. Idempotent.
func (s *anchoredSnapshot) releaseRows() error {
	if s.rowsReleased {
		return nil
	}
	s.rowsReleased = true
	var firstErr error
	if _, err := s.conn.ExecContext(context.Background(), "COMMIT"); err != nil && firstErr == nil {
		firstErr = err
	}
...
// close releases the snapshot conn if not already released, then closes
// the pool.
func (s *anchoredSnapshot) close() error {
	var firstErr error
	if err := s.releaseRows(); err != nil && firstErr == nil {
		firstErr = err
	}
...
// abort is the error-path teardown: ROLLBACK the pinned tx and close
// everything. Errors are deliberately dropped — the caller is already
// returning the error that got it here.
func (s *anchoredSnapshot) abort() {
	if !s.rowsReleased {
		s.rowsReleased = true
