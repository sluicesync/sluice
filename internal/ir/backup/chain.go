...
// [ir.Column.Identity] is excluded on the same reasoning (gap census
// 2026-09-22 S4/S5): the Postgres reader sets it on EVERY identity column —
// the `id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY` that heads
// nearly every Postgres table — so folding it in would have moved the
// fingerprint of close to every Postgres chain. It rides the wire, so a
// restore re-creates ALWAYS and the sequence options; what the exclusion
// costs is that chain drift detection does not notice an ALWAYS ⇄ BY
// DEFAULT flip or a re-optioned identity sequence between the backup and
// the restore target.
...
func columnNeedsFingerprintNormalizing(c *ir.Column) bool {
	if c.Default == nil || c.OnUpdateCurrentTimestamp || c.Identity != nil {
		return true
	}
