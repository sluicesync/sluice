...
// IdentityOptions is the Postgres-shaped identity attribute set
// [Column.Identity] carries: the generation mode and the backing
// sequence's options, mirrored from information_schema.columns.
// identity_generation and pg_sequence exactly as [Sequence] mirrors
// pg_sequence for a standalone sequence.
//
// Always is `GENERATED ALWAYS AS IDENTITY` (the source rejects an
// application-supplied value with SQLSTATE 428C9); false is `BY
// DEFAULT`. Start / Increment / MinValue / MaxValue / Cache / Cycle are
// the catalog values, rendered by the Postgres writer only where they
// differ from PostgreSQL's ascending factory defaults for the emitted
// type, so an ordinary identity column still emits the bare clause.
...
// LostOnEngineWithoutIdentityAxis reports whether the identity carries
// something a target with no identity axis (MySQL AUTO_INCREMENT,
// SQLite's rowid alias) cannot hold: ALWAYS, or any sequence option off
// PostgreSQL's ascending default. MaxValue is deliberately not consulted
// — it is the type ceiling on an ordinary identity and translates with
// the type. Writers for such engines WARN on it rather than dropping the
// attributes silently.
func (o *IdentityOptions) LostOnEngineWithoutIdentityAxis() bool {
	if o == nil {
		return false
	}
	return o.Always || o.Start != 1 || o.Increment != 1 || o.MinValue != 1 || o.Cache != 1 || o.Cycle
}
...
type Column struct {
	...
	// Identity carries what [Integer.AutoIncrement] collapses: the
	// Postgres `GENERATED { ALWAYS | BY DEFAULT } AS IDENTITY` axis and
	// the backing sequence's options (gap census 2026-09-22 S4 + S5).
	// nil when the column is not an identity column, when the source
	// engine has no such axis (MySQL AUTO_INCREMENT, SQLite's rowid
	// alias), or when the manifest predates the field — every one of
	// which the Postgres writer renders exactly as it did before the
	// field existed: `GENERATED BY DEFAULT AS IDENTITY` with no options.
	//
	// It lives on the Column rather than on [Integer] for the same
	// reason [OnUpdateCurrentTimestamp] does: Integer is compared by
	// `==` at the shape gates and zeroed by the CDC normalizer, and a
	// column attribute that no shape compare should see belongs beside
	// the other write-behaviour attributes. Like that field it rides the
	// wire and is EXCLUDED from the backup schema fingerprint (see
	// internal/ir/backup.canonicalColumnsForHash) — the Postgres reader
	// sets it on every identity column, so folding it in would have
	// repartitioned every Postgres chain.
